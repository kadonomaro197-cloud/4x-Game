...
| `docs/COMBAT-DESIGNER-GROUND-TRUTH-2026-07-28.md` | ⭐ **BEFORE TOUCHING THE COMPONENT DESIGNER, THE ENTITY ASSEMBLER, OR EITHER COMBAT RESOLVER.** The consolidated record of the 2026-07-28 verification campaign — a 17-pass resolver audit, a 7-pass designer audit, a 7-agent parallel survey of the designer's whole surface, and the cross-audit reconciliation between them, in one file (it replaced four). Holds **six gauges that cannot rot** (template count 96 vs a locked ~37; **shared dials per door = 0**), the **seven open developer decisions** that gate the work, the **36 LOCKED decisions** that bound it, the designer **as-built / intent / gap**, all **89 findings**, the **six corrections** — including a retracted doc self-correction that would have crashed New Game — the **save-compatibility risk register** (adding a ctor overload breaks load on ~12 `*Atb` classes), what is **verified-good**, and the A/B/C/D plan. Headline: *the designer models COMPONENTS; the resolver models TOTALS* — and **the taxonomy was built, the parametric collapse was not.** |
| `docs/economy/DESIGNER-NORTH-STAR.md` | ⭐ **🔒 BEFORE ADDING ANY DIAL, DOOR, OR COMPONENT TEMPLATE — the developer's locked call, 2026-07-29: *"this is how the designer must be."*** The **method**: a door is not invented, it is **DERIVED from the values the simulation actually reads**. Find the input surface in source → group by the question each answers → separate FORCED (becomes a choice/door) from FREE (becomes a slider) → prove it reproduces everything that already exists. **Two tests for any dial: (1) which sim variable does it write? None, and it costs nothing ⇒ a bug in the design. (2) 🔒 THE INTRINSIC TEST — can it be set WITHOUT knowing anything else on the entity? If not it is an ASSEMBLY decision or an EMERGENT readout, never a dial. That second test is the missing boundary between the Component Designer and the Entity Assembler.** Worked end to end for **Weapons** — 5 doors and 41 dial groups collapse to **2 choices + 4 sliders**, reproducing all eleven in-game weapons and reaching 1,073 named designs. Also holds the combat **asymmetry** every system must respect: vs shields the ATTACKER's setting decides (fixed everywhere), vs armour the DEFENDER's plating decides (never publish a fixed number), vs hull nobody decides. |
| `docs/assembler/DESIGNER-DRIVER-PLAYBOOK.md` | ⭐ **BEFORE BUILDING ANYTHING FROM A PLAIN-ENGLISH BRIEF** ("design a capital ship that's a carrier first, a brawler second"). The naval-architect SOP for driving the 12 doors + the Entity Assembler from a one-line mission to a **complete, verified, ORIGINAL** build — start from the mission, derive the requirements, size every supporting system (power/heat/berthing/stores/endurance/detection) until the platform *closes*, and back every line with engine source. Holds **THE LAW — authenticity to limitations** (a build works only through its LIVE wires; never a dial that writes to nothing; a PENDING capability is flagged, not sold), the **two modes** (SYNTHESIS from a role = default, must NOT converge on a franchise ship · REPRODUCTION from a named ship = `VENATOR-BUILD.md`), the **9-step pipeline** (objective → self-interrogation axes → capabilities → components pick/derive → close the gates → authenticity pass → cradle-to-grave → verify → deliver doc+preset), the honesty vocabulary, the **scaling ladder** (corvette→capital→station→megastructure/Death Star), the anti-patterns, and a worked "carrier-first" sketch. Pairs with `DESIGNER-NORTH-STAR.md` (how to derive one dial) — this is how to drive the whole suite to a build. |
...
| ~~`docs/archive/SYSTEMS-STATUS-AND-TEST-PLAN.md`~~ | 🗄 **RETIRED 2026-07-27 — do not open it, do not add to it.** Its three jobs now have single owners: **connections → `docs/SYSTEM-CONNECTION-MAP.md`** (the Prime-Directive blast-radius tool), **tests/gauges → `docs/TESTING-TRACKER.md`**, **doc currency → `docs/DOCS-INDEX.md`**. For planetary/ground build state and what to do next: **`docs/ground/PLANETARY-FUNCTIONAL-PLAN-2026-07-27.md`**. Nothing live was lost — its test recipe duplicated this file's Build/Run/Test section and its open-issues list was fully resolved or stale (verified 2026-07-27). |
...
| `docs/economy/RESOURCES-AND-MATERIALS-DESIGN.md` | Designing anything touching minerals, materials, production, commerce, research, or NPC economic AI. Full system survey — read before changing any part of the economy. |
| `docs/society/DIPLOMACY-DESIGN.md` | Designing anything touching faction relationships, IFF, inter-faction trade, logistics access, NPC doctrine, or diplomatic state — **the EXTERNAL-politics half** (the outward twin of `docs/society/GOVERNMENT-AND-POLITICS-DESIGN.md`'s internal layer). Two halves: the mechanical **substrate survey** (`DiplomacyDB`/`RelationshipState`/IFF/first-contact wiring/commerce — build first) and **"EXTERNAL politics — politics with teeth"** (the decision-engine: first-contact-as-event, the relationship TRACK, costed treaties, casus belli gated by the militarism dial, the INTERNAL⟷EXTERNAL handoff, the Foreign-Minister delegate, and the espionage/gate-control/late-game-crisis frontier), **plus the LOCKED "Making politics FUN + CONNECTION layer"** (the COMMITMENT model — a deal emits real orders into fleets/logistics/money with promised-vs-delivered tracked; the broad data-driven EXCHANGE CATALOG of everything two factions can trade + which system each routes into; REACTIVE diplomacy — the "Are we good?" engine = the internal demand-engine pointed outward; and the **blast radius + 3-keystone prerequisite chain**: fix the GlobalManager-not-iterated trap → fix degenerate detection-quality → wire hostility-from-DiplomacyDB, THEN build politics). Read before adding any cross-faction interaction. |
| `docs/combat/DETECTION-DESIGN.md` | **Before any sensor/detection/fog-of-war/EMCON work** (M1 lever #1). Survey of the existing (rigorous but unwired) sensor engine + the Keep/Cut/Add design: keep the contact track-table + signature model, hide the EM-spectrum math as gameplay, ADD fog-of-war-in-combat + the EMCON (Active/Dark) posture lever. Holds the "what exists / how it works / how it comes together" and the gauged build sequence. The decision is *dark-vs-loud*, not wavelength tuning. |
| `docs/combat/COMBAT-DESIGN.md` | The master space-combat design: the eleven required systems (weapon range → auto-resolution → ground-combat interface). Includes the detailed **Fleet Components & Switchable Doctrine** design (Front Line/Flank/Rear Guard/Artillery as sub-fleets, Offensive/Defensive/Utilitarian options, switch cooldown, commander operational discretion, table-based fleet combat UI) under System 4. Read before any combat-system work. |
| `docs/combat/WEAPONS-DESIGN.md` | Weapon design + the combat-**depth** pass: the two-axis **Nature × Delivery** taxonomy (the governing frame — pick nature × delivery, the triangle position emerges), plus the dodge hit-fraction, computed saturation, the **weapon triangle**, and the **aggregate/bucketed** resolve that keeps 100s-of-ships battles cheap. Read before touching weapon types, evasion, or the dodge resolve. *(Consolidated 2026-07-13 from WEAPONS-AND-DODGE + WEAPON-TAXONOMY.)* |
| `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md` | ⭐ **BEFORE TOUCHING ANY COMBAT RESOLVER — space, ground, or the shared kernel.** The single consolidated record of the auto-resolve engine: **how it is actually built, what it was designed to become, every locked ruling, every build slice and its state, every open question.** Assembled from eight design docs (it replaced all of them) by a four-agent verbatim extraction, each finding re-verified against source. Holds: the **§8 14-row scenario matrix** ("does it handle X?" — the fastest answer in the repo); the **§9 six-man-squad** case + Fix A/B/C; the **§10 ten-row ship-vs-ground divergence table** (range · fog · multi-weapon · **firepower conservation** · retreat · stats · hostility · **the per-tick-vs-per-second `dt` trap** · two space paths · perf); the **§11 NORTH STAR** (*"ground units work the same as ships as far as fleets or battalions are concerned"*); **§12** the ground closing fight (real distance + the W-track); **§13** the 2D group plane + the **two pinned joints** (conserved fire-allocation · combined-theater cadence); the **§14 build ledger across all six tracks**; **§15** battle statistics; **§16** the nine open developer decisions; and **§19** the stale-claim register. Headline: **two resolvers sharing ONE damage kernel, and a ship or a ground unit is "whole-or-dead."** |
| `docs/combat/INFORMATION-DELTA-DESIGN.md` | **Before adding any readout/gauge/UI-number, or when a system "doesn't communicate with the player."** The gap between what the sim KNOWS and what it SHOWS, with the load-bearing distinction: **Failure A** (the number exists, just unwired — cheap) vs **Failure B** (no number exists — build the gauge first, an engine job before a UI job). Holds the EXISTS/MISSING ledger (file:line) and the first build: engagement range, detection range (the reverse-solve), delta-V, ETA. Read before assuming a missing readout means a missing number — half the time the gauge is built and just unwired. |
| **THE AI DESIGN SUITE (6 docs) — `docs/AI-*.md`** | **Before ANY work on the faction/NPC AI — the brain in `NPCDecisionProcessor.cs` (`GameEngine/Factions/`).** As-built truth (2026-07-13): `Tick` is a **full decision engine** (objective selection + planner + reactive-diplomacy + espionage mirror + crisis), **not** an empty stub — but the behaviour arms ship **gated OFF by default flags** (`EnableOrderEmission`/`EnableDiplomaticProposals`/`EnableEspionageMirror`/`EnableIntelLedger`), so a default game runs the loop idle. Durable seats shipped; `GalaxyCrisis` is live. Runtime unverified (CI can't run the client). A full "quark→brane" design, consolidated 12→6 docs 2026-07-13. **Enter through the build tracker.** **Read order:** ① `docs/ai/AI-BRAIN-BUILD-TRACKER.md` — THE hub: build state per slice + the design→code wiring map + §4 SOCKET VERIFICATION. ② `docs/ai/AI-PERSONALITY-IMPLEMENTATION-SPEC.md` — the 12-trait `PersonalityDB` + shared `DecisionScorer` (`PersonalityDB` built at 0.5-neutral; scoring/drift pending). ③ `docs/ai/AI-COMMAND-AND-COMMUNICATION-DESIGN.md` — mission-command (Head-of-State sets destination, delegates decide *how*; the 3-mode Delegate/Advise/Hand-fly dial). ④ `docs/ai/AI-DECISION-ENGINE-DESIGN.md` — the needs-ladder objective engine (destination) + the means-ends planner (how to reach it). ⑤ `docs/ai/AI-EMERGENT-POLITICS-AND-CRISIS-DESIGN.md` — emergent inter-faction politics (Organism pointed outward) + the temporal arc & late-game crisis + the authoring/acceptance layer. ⑥ `docs/ai/AI-CAPABILITY-CATALOG.md` — the NEED→buildables catalog the utility scorer reads. Roster/seats/parity live in `docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md`. Companion: `docs/explore/EXPLORATION-CONTENT-DESIGN.md` (the field-site loop). |
...
| L12 | **A DataBlob that forgets `Clone()` fails SILENTLY, returning a bare `object`.** `BaseDataBlob.Clone()` is `public virtual object Clone() { return new object(); }` — **virtual with a garbage default, not `abstract`** (its own source carries `// FIXME: … this seems bad`). So omitting `Clone()` on a new `*DB` still compiles, and the blob silently becomes a `System.Object` whenever an entity is cloned (moved between managers, e.g. a ship jumping systems). `ColonyHexMapDB` is the live example: zero `Clone` references. | Every new `*DB` gets a `Clone()` (and a copy-ctor) — plus `[JsonProperty]` on save-carried fields. If you touch an existing blob, grep it for `Clone` before assuming it round-trips. | `Engine/Datablobs/BaseDataBlob.cs:30`; `CONVENTIONS.md` (DataBlob copy-ctor/`Clone()`) |
| L13 | **Adding a CONSTRUCTOR OVERLOAD to an `*Atb` can break SAVE-LOAD.** Newtonsoft auto-uses a parameterized ctor **only when there is exactly one public ctor**; two public ctors with no parameterless ⇒ `JsonSerializationException: Unable to find a constructor to use`. ~12 classes are at risk (`CargoStorageAtb`, `EnergyGenerationAtb`, `NewtonionThrustAtb`, `OrdnancePayloadAtb`…). Separately, adding a ctor **PARAMETER** is an exact-arity break that crashes New Game — a C# *optional* parameter does **not** help, because `Activator.CreateInstance` is called without `OptionalParamBinding`. | Add a **new overload** (never a new parameter) **and** a `[JsonConstructor] private XAtb() {}` in the same change — the pattern `IndustryAtb.cs:20-21` uses. | `docs/COMBAT-DESIGNER-GROUND-TRUTH-2026-07-28.md` §12 |
| L14 | **`Window.End()` inside the `if (Window.Begin(...))` block bricks the WHOLE CLIENT in a Debug build.** `Window._beginCount` is **static**, decremented only in `End()`, with **no per-frame reset** — so one collapsed or throwing window makes every subsequent `Window.Begin` throw for the rest of the process, blanking all ~52 wrapper windows. 16 windows have this shape. ⚠ The wrapper does **NOT** call `End` for you, whatever `Pulsar4X.Client/CLAUDE.md` used to say. | `End()` goes **AFTER** the block, always — for both `ImGui.Begin` and the `Window.Begin` wrapper. | `docs/COMBAT-DESIGNER-GROUND-TRUTH-2026-07-28.md` §7.2 R-3 |
...
**This applies to every subsystem listed in the Subsystem Index, every system in `docs/combat/COMBAT-DESIGN.md`, every column in the game.** Economy connects to industry connects to population connects to research connects to ship production connects to fleet strength. Sensors connect to contact model connects to IFF connects to doctrine connects to auto-resolution. None of these are islands.

**Operationalize this with the connection map — every single time.** `docs/SYSTEM-CONNECTION-MAP.md` is the connection graph in table form, and it is not a document you read once. **Whenever you go into a system — to read it, change it, debug it, or even just decide whether to touch it — open the map first, find that system's row, and read what it feeds / reads / shares / triggers, and every row it points at.** That is the minimum blast radius; the four questions above extend it one hop further. If you hit a connection the map doesn't list, **add it before you continue** — that is the one upkeep rule, and it's why the graph carries no build/test marks (they can't rot). Test/gauge state goes to `docs/TESTING-TRACKER.md`; doc currency to `docs/DOCS-INDEX.md`. The map is only worth keeping if it is consulted and updated on *every* system dive — that discipline is what stops the reactive pivoting that leaves a game half-built. *(This replaced `SYSTEMS-STATUS-AND-TEST-PLAN.md`, retired 2026-07-27, whose stale status columns were the reason.)*
...
- **Connect** — *the one that matters most* — wire the system into the others and **verify the stacked behavior, in code AND data.** A system is only as done as its connections: detection isn't done when "it detects," it's done when *what you can see decides the fight* (detection × weapons). The **data** half is the same move — when you add a cost/material/tech reference, check the *other end* (gotcha #10): the id is defined, and a starting design's material is stocked. **The real test is always the cross-system stack, never the unit in isolation.**
  - **Investigate before you wire — every connected system, every time (standing process).** When you touch a system, a flagged connection is a **work item, not a footnote**: go open the files on each system it connects to and produce an **EXISTS / MISSING / NEEDS-CHANGE** ledger (file:line, not assertions) *before* designing the change. This is the half of the Prime Directive that's easy to skip — mapping the connection but not looking at what's on the other end. It routinely overturns assumptions and finds half-built scaffolding to finish instead of rebuild. Worked example: `docs/combat/DETECTION-DESIGN.md` §3c (EMCON traced across the emitter / control-heat / detection systems).

**This is how we improve.** Run Keep/Cut/Add/Connect on a system every time it's touched and the game gets tighter — pretty gets trimmed, levers get added, and the connections that turn "100 simple systems" into a *stacking* game get built and verified. Worked example: `docs/combat/DETECTION-DESIGN.md` §3.
...
1. **OPEN the subsystem `CLAUDE.md`** for the code you're touching (Subsystem Index above), AND `docs/SYSTEM-CONNECTION-MAP.md` — find the row and read every system it connects to. That is the minimum blast radius. *(For planetary/ground work also open `docs/ground/PLANETARY-FUNCTIONAL-PLAN-2026-07-27.md`.)*
...
1. **Apply the Prime Directive — through the connection map.** Before touching any system, open `docs/SYSTEM-CONNECTION-MAP.md`, find its row, and read every system it connects to (and their rows). Work those too — don't change a system in isolation. Add any connection the map is missing, in the same commit. Map connections before deciding. See above.
