...
| **THE AI DESIGN SUITE (8 docs) — `docs/AI-*.md`** | **Before ANY work on the faction/NPC AI — the brain that lives in `NPCDecisionProcessor.cs` (`GameEngine/Factions/`), today a working socket with an empty `Tick` stub (~5% built).** A full "quark→supercluster" design of the in-game AI, done 2026-07-10/11. **Enter through the WIRING MAP** — it's the hub that maps every designed piece to a code home. **Read order:** ① `AI-IMPLEMENTATION-AND-WIRING-MAP.md` — design→code bridge + **§4 SOCKET VERIFICATION** (every element → SOCKET-EXISTS / MUST-BUILD / NO-SOCKET; the headline is *the hard infrastructure already exists*). ② `AI-PERSONALITY-IMPLEMENTATION-SPEC.md` — code-ready spec for the 12-trait `PersonalityDB` + the shared `DecisionScorer` (the ONE scoring engine reused everywhere). ③ `AI-COMMAND-AND-COMMUNICATION-DESIGN.md` — the mission-command model (Head-of-State sets destination, delegate officers decide *how*; mandate-down/report-up; the 3-mode Delegate/Advise/Hand-fly dial). ④ `AI-OBJECTIVE-ENGINE-DESIGN.md` — the DESTINATION engine (fractal Survive→Stabilize→Thrive→Ambition needs-ladder + the transition/action-plan engine). ⑤ `AI-SELF-PLAY-DESIGN.md` — the 21-leader org chart + two-axis seat grid. ⑥ `AI-ECOSYSTEM-DESIGN.md` — emergent inter-faction politics (no galaxy-AI; fog-driven, earned alliances, no rubber-band). ⑦ `AI-GALAXY-AND-CRISIS-DESIGN.md` — the emergent arc + the late-game crisis (an *ascension of an existing faction*, not an injected boogeyman). ⑧ `AI-SUPERCLUSTER-AND-AUTHORING-DESIGN.md` — the scenario/franchise-authoring layer. **The three things to know cold:** (a) the **paramount FIRST build** is fixing **seat durability** (`AdminSpaceProcessor.cs:36-46` wipes the seat list every pass — "nothing can be built" until fixed); (b) two load-bearing **NO-SOCKETs** to build on paper first — the fleet-commander *situational read* (fog-limited enemy-strength estimate → fight/flee/last-stand scorer) and the crisis ascension-seed; (c) it's mostly **new logic + wiring**, not new infrastructure — the clock fires, the event bus works, the order rail is headless. Companion: `docs/EXPLORATION-CONTENT-DESIGN.md` (the field-site loop that fills science — "four seasons of Star Trek," incident-response shape). |
