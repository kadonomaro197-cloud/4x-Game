...
Read ${F}/A4-sealift.md (CI gap — ConquerResolverTests hand-place transports, bypassing every link that died). A fixture EfSealiftEndToEndTests ALREADY EXISTS (committed) driving BUILD -> LAUNCH -> LOAD -> SAIL on DevTest Mars, but it FAILED CI (2026-07-20): driving ONLY IndustryProcessor+LaunchComplexProcessor per game-day STALLS the trooper build at status=MissingResources — the material SUPPLY chain (mining -> refining) is never driven, so the components (battery-2t / reactor-2t / NTR1.8) never receive their inputs, the ship never completes, and the launch/charge/load/sail death-zone is never reached. STEP 1 (resolver Rung 2 queues the transport) PASSED and is correct. FIX STEP 2 only, keeping STEPs 1/3/4. Per Tests/CLAUDE.md gotcha #7 ("gauging a BUILD through the industry queue is flaky — drive OnConstructionComplete directly"): complete the queued trooper through its REAL completion entry point — the exact call IndustryTools makes at job completion, ShipDesign.OnConstructionComplete(...) (VERIFY its signature + args in source, e.g. Tests/CLAUDE.md gotcha #7's example) — which for a LAUNCH-COMPLEX colony (Mars) routes the finished hull into the colony's LaunchQueue (NOT straight to orbit — confirm this branch in ShipDesign.OnConstructionComplete source). THEN drive LaunchComplexProcessor.ProcessEntity(mars, ...) (the REAL launch: pad MaxTonnage gate -> TryDeductFuel [the P4.3 fuel] -> CreateShip in orbit -> ProvisionBuiltShip [the P4.2 charge+fuel]) until the transport is an OWNED IN-SYSTEM entity; keep the existing charged/fuelled/at-Mars-body assertions + STEP 3 LOAD (FindOwnedTransport -> LoadInvasion) + STEP 4 SAIL. This exercises the DEATH-ZONE (launch handoff, pad-tonnage gate, fuel gate, reactor charge, load, sail) through REAL paths, and deliberately does NOT gate on the material grind (A4 confirms materials flow under the real full clock — that is not the bug). Do NOT hand-CONSTRUCT a staged entity (no PlaceLoadedTransportAt-style pre-built ShipInfoDB+PositionDB+GroundTransportDB) — OnConstructionComplete is a real production entry point, not hand-building. Keep [Timeout]. REWRITE the committed test file in place; append a short "P4.4 fix" note to LANE-CORE-NOTES.md explaining the supply-chain diagnosis.` },
  ]},
...
Per §13 S2: flag-on, WithinWeaponRange/SeparationOf compute the 2D group-pair distance (per-sub-fleet gaps real); kernel stays 1-D (receives scalar d); flag-off byte-identical. Tests: a long-range group fires a short-range group that cannot answer (directed gate) on the plane; flag-off fixtures unaffected. Do NOT start S3+ (later campaign).` },
  ...
  PW: { lane: 'CORE', title: 'WIRING — resolver rungs onto merged capabilities', batches: [[0, 1]], slices: [
    { key: 'PW.1', title: 'Conquer rungs: beachhead, brain kickoff, hex-infra (bombard TABLED, regions cosmetic)', prompt: `
Prerequisite: GROUND lane merged. DEVELOPER DECISIONS (docs/earthfall/EARTHFALL-DECISIONS.md) reshape this slice — obey them over any older spec:
  * #4 ORBITAL BOMBARDMENT IS TABLED — do NOT build a bombard rung / softening-fire / re-fire wiring at all. Leave the existing one-shot GroundBombardmentTests damage path untouched. The STRIKE rung feeds straight into LAND.
  * #5 REGIONS ARE COSMETIC, THE HEX IS THE UNIT ("regions mean nothing, it's all about hexes") — do NOT build a landing-REGION scorer; landing stays at the default zone (leave ConquerResolver's current land target as-is). All combat / capture / infrastructure resolves per-HEX (GROUND's GroundHex + per-hex capture).
So this slice is ONLY (c)+(d)+(e). Read ${F}/A5-ground-campaign.md (minimal-build list) + docs/earthfall/GROUND-TACTICAL-AI-DESIGN.md §3.8 (the strategy->tactics seam) + the merged GroundCombat surface (grep the new helpers: FormUpLoose, GroundThreat, GroundTactics, EnableGroundTacticalAI, the beachhead build step, the infra order types, GroundHex).
In ConquerResolver (CORE-owned):
(c) BEACHHEAD rung — after the invaders hold ground, land engineer + parts and invoke the G1 build step (verify the merged beachhead API in source);
(d) BRAIN kickoff — flip EnableGroundTacticalAI for menu/DevTest games (beside the other AI gates) + ensure FormUpLoose runs on the landings the resolver drives;
(e) INFRA tasking rung — queue DestroyInfrastructure against an enemy building on a reachable HEX (the G3 per-hex order types + GroundHex), gated on the battalion's tactical-brain posture being Offensive (the design's stance-as-gate rule; FLAGGED aggression policy).
Resolver-driven tests for each rung (the ConquerResolverTests idiom). NO bombard rung, NO region scorer — those are the two tabled/moot parts per the decisions above.` },
...
Prerequisites: GROUND + CLIENT merged. (a) Force Management: battalion RENAME button via GroundForces.RenameFormation; infra "Destroy/Capture infrastructure" order buttons (the G3 order types) in the battalion order surface + PlanetViewWindow city-tile inspect spot (R4 hooks :640/:1074). (b) Sweep every lane-notes REQUEST (docs/earthfall/LANE-*-NOTES.md) and resolve or escalate each to the developer list. CLIENT-TEST-CHECKLIST entries.` },
  ...
  P8: { lane: 'CORE', title: 'EARTHFALL ACCEPTANCE', slices: [
    { key: 'P8.1', title: 'The campaign gauge + the generic player-chain RAILS gauge', prompt: `
DEVELOPER DECISIONS APPLIED (docs/earthfall/EARTHFALL-DECISIONS.md — obey exactly; they OVERRIDE anything below that conflicts):
 - #4 ORBITAL BOMBARDMENT RE-FIRE IS TABLED. The EXISTING one-shot orbital bombardment (GroundBombardmentTests path) stays and MAY soften a defending garrison, but do NOT build or ASSERT a between-waves re-fire cadence — it was never built, no re-fire rung exists.
 - #2 THE SPACE MARINE IS THE DEVELOPER'S LIVE IN-GAME TEST, NOT A CI FIXTURE. Do NOT hard-code a Space-Marine unit (no named sealed + veteran-cadre + power-armor marine). P8.1 (b) proves the GENERIC player unit-creation -> battalion -> defend/invade RAILS are green + generic so the developer's own live-designed marine rides working rails.
 - #5 THE HEX IS THE UNIT. Infra combat is per-HEX (GroundOrder.DestroyInfra/CaptureInfra(region,q,r)); landing still targets a region INDEX (LandTroopsOrder's regionIndex is the engine's real addressing) and capture flips the region/hex owner. No region-transfer logic.
...
(a) OperationEarthfallTests: the WHOLE AI campaign through real paths on DevTest — Conquer held under transient wobble (P3) -> mass+sail -> ONE transport builds, launches fueled+charged (P4) -> the EXISTING one-shot orbital bombardment softens the strong garrison (GroundBombardmentTests path; NO re-fire — tabled) -> land at the SCORED region -> FormUpLoose battalion -> THE TACTICAL BRAIN picks Offensive and advances region-to-region (G2) -> beachhead built by the engineer (G1) -> infra destroyed/captured per-HEX where the brain's posture gates it (G3/PW) -> a SECOND WAVE loads+lands -> planet capture flips the colony; meanwhile Kithrin survey+found (D1-D3) and no faction balance decays structurally (D2). POSTURE ASSERTIONS (the brain visibly alive): when the defenders outnumber the landed force past the Risk bar, the invader battalion reads it and goes Defensive / withdraws toward its beachhead — assert the Stance/Roe/Intent + Reason, not just survival.
(b) PlayerGroundChainRailsTests: the GENERIC player chain (NOT a hard-coded marine — decision #2) — assemble a GENERIC ground-unit design in-engine exactly as the UI does (RegisterAssembledDesign: a frame + a couple of generic base-mod parts, e.g. rifle + plating; do NOT invent a bespoke "Space Marine"), build+field via the real industry path, FormUp into a battalion, set stance/ROE, then a landed UMF invasion force attacks Earth: the player defense battalion defeats it — AND assert the UMF brain REACTS to the counterattack (outmatched => Defensive/withdraw, the exact moment that answers the developer's question); then embark the units (LoadTroopsOrder) and land them on another body / Mars (LandTroopsOrder) — the developer's scenario shape, both directions, on GENERIC units so the rails are proven green for the developer's own live-designed marine to ride.
...
Sweep ALL docs/earthfall/LANE-*-NOTES.md pending rows into the real dashboards in ONE slice: DOCS-INDEX rows (new docs incl. the memos + this campaign folder + GROUND-TACTICAL-AI-DESIGN + EARTHFALL-DECISIONS), TESTING-TRACKER rows (every new gauge), SYSTEM-CONNECTION-MAP (new wires: the beachhead haul, the ground TACTICAL BRAIN, per-HEX infra combat, station income, survey chain, embark/land UI, the resolver brain-kickoff form-up — the one-shot orbital bombardment was already mapped; do NOT add a bombard re-fire wire, it was tabled), subsystem CLAUDE.md rows any implementer missed (audit git log of all lanes), stale-claim retirements. THEN write TWO docs: (1) docs/ground/EARTHFALL-CAMPAIGN-OPS.md — the developer's "EACH INDIVIDUAL STEP AND CLICK" deliverable: the complete invasion flow as BUILT, from BOTH seats — the AI's rung sequence (mass -> sail -> one-shot bombard [NO re-fire, tabled] -> scored landing -> form up -> brain posture -> advance -> beachhead -> per-hex infra -> second wave -> capture, with file anchors) and the PLAYER's actual windows/buttons for the same acts (design a unit in the Entity Assembler -> assemble -> build -> form battalion -> stance/ROE -> move/queue -> embark -> land -> raze/capture infra); note the developer's Space Marine is their OWN live-designed unit riding these generic rails (not a hard-coded fixture). Include the support loop (ammo, resupply, upkeep, fire support) and the honest LIMITS list — plain English, shipboard analogies, per the root CLAUDE.md communication rules; (2) docs/earthfall/CAMPAIGN-REPORT.md: what shipped per lane, the applied developer decisions (point to EARTHFALL-DECISIONS.md), the invasion on-switch gate-flip (EnableGroundTacticalAI + AutoFormUp ON for menu games — easy revert), deferred items (incl. orbital-strike-at-a-specific-hex AND the bombard re-fire cadence, both explicitly deferred/tabled by the developer), and the CLIENT-TEST-CHECKLIST items awaiting the developer's local runtime pass.` },
  ]},
}
