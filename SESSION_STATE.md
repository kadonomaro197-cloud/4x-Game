...
7. **Warp/move:** give any of the six combat ships (Aegis / Picket / Lancer / Bulwark / Wasp / Leviathan) a move order → it should **warp** to the target. A *just-spawned* ship waits for its reactor to charge first (run game-time a bit). If it just sits there, the log now says exactly why: `[WARP] … CAN'T WARP — <reason>`.
...
- **Cradle-to-grave includes the TRAVEL rung — "can it actually get there?"** The live "spawned ship won't move on a move order" symptom had two stacked causes, both now loud via `[WARP]`: (1) the six combat ships had guns/armour/thrusters but **no warp drive**, so `WarpMoveCommand` had nothing to engage and `return`ed silently (the literal `// FIXME: alert the player?`); (2) even *with* a drive, a freshly built/spawned ship boots with an **empty reactor** (`EnergyStored`=0) and can't warp until charge builds — advance time and it goes. Fix: Alcubierre drives on all six (the **same drive the utility ships already carry** → materials already stocked, `BaseModIntegrityTests`-safe; Wasp got the lightest to protect its dodge), and both failure modes made loud. A unit that can't reach the fight is a missing rung, not a footnote — and a silent `return` on a player order is an invisible bug (Visibility Gate). CI-green on `c67b8d2`.
