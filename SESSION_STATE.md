...
**2026-07-16** — **LIVING OPPONENTS**: the AI now MASSES its built warships into a real fleet, SIZES it to its wealth, KEEPS a home reserve, and fields a per-faction fleet AND garrison identity + new AI-used ships — plus an **Event Logger** (auto-pause on enemy detection) and three runtime **freeze fixes**. On `claude/devtest-faction-design-xpfnhe`, **fully CI-green through `cd224a7`, mergeable**. See the ⏩ Session 2026-07-16 block below. (Prior: 2026-07-06 weapon unification — block further down.)
...
## ⏩ Session 2026-07-16 — LIVING OPPONENTS: fleets that mass + fight, faction identity, new AI-used units (READ FIRST)

On branch **`claude/devtest-faction-design-xpfnhe`**. **Every slice CI-green, one slice per push, waited for green before stacking.** Engine + data work; the one client piece (the Event ticker window) is CI-blind → the local-runtime backlog. The branch is **mergeable + fully green through `cd224a7`.**

**The through-line (WHY):** turn the AI/faction layer from "decides but sits idle" into **living opponents** — an NPC that actually MASSES its built warships into a real fleet, SIZES that fleet to its wealth, KEEPS a home reserve instead of throwing everything at you, fields a per-faction fleet AND garrison identity, and uses NEW units authored this session. Plus two player-facing asks: an **Event Logger** that stops the clock (and drops fast-forward back to 1-hour steps) when an enemy fleet is detected, and the **reserve rule** the developer called out mid-build ("don't pull all fleets at the same time").

### What shipped (in order, each its own green commit)

**Freezes first** (the developer's local `[SIM-STALL]` watchdog logs). Three runtime freezes:
- **The WEDGE** (`bd3fd62`) — the fleet-tree walk went exponential on a cycle/diamond in the sub-fleet hierarchy; fixed with a `HashSet<int> seen` visited-set in `CombatEngagement` ship-collection + `FleetTools.AllShipsRecursive`.
- **The parent-walk** (`599a0d9`) — `TreeHierarchyDB.Root` could loop forever on a cyclic parent chain; made iterative + capped at `MaxParentWalk` (4096).
- **The PERF crawl** (`9f2aab9`) — the combat fine-step (5 s sub-stepping to land the auto-pause) false-positived on a flickering detection and crawled game-time to a standstill while `SensorScan` ran per-sub-step; capped at `MaxConsecutiveFineSteps` (24 = 2 game-minutes), after which it runs full steps. A REAL engagement still cancels within a step or two, so the auto-pause still lands.

**Event Logger** (the developer's spec — a top strip that pauses on enemy detection):
- **Slice 1, engine** (`2e9d0ed`): `SensorEvents.OnNewShipContact` rides the same new-contact branch of `SensorScan` as first-contact; a newly-detected ENEMY foreign ship (at-war OR unknown-foreign) publishes a `NewHostileContact` event — "Enemy Fleet detected at [nearest large body]." `FactionEventLog` halts the clock on it **and resets `Ticklength` to 1 hour**, so un-pausing while fast-forwarding months doesn't blast past the danger. Gauge `EventLoggerTests`.
- **Slice 2, client** (`5d1039a`): `EventTickerWindow` — a borderless top-centre strip (5 events collapsed / 20 expanded) that opts the player's log into the halt. CI-blind → local-runtime backlog.

**Fleet Composition** (the holistic-fleet vision + the "AI never masses" bug):
- **Slice 1** (`4c1dea5`): `FleetCompositionTemplate`/`FleetCompositionTier` — the size ladder (Forming→Deployable→Ideal→Perfect, default 3/8/18). Pure data.
- **Slice 2** (`856b362`): `FleetAssembly.AssembleBuiltWarships` — **THE MISSING LINK.** A built NPC warship was parked flat under the faction ROOT fleet (which lives in the GlobalManager, not a star system), so the AI's `ReadyStrikeFleet` — which only sees in-system fleets — never saw it and the strike fleet never reached 3 ships. Now the AI sweeps its loose warships into ONE real, flagship-bearing, in-system fleet each cycle. Gauge `FleetAssemblyTests`.
- **Slice 3** (`ba05781`): resource-gated sizing + the reserve rule. The AI reads treasury + war footing → aspiration (poor→Deployable / 50k→Ideal / 150k→Perfect; war bumps a tier); fleets fill to target then OVERFLOW into a second fleet; and it never commits its LAST organized fleet on an offensive (`HasHomeReserve` keeps a home reserve). A placeholder for the future military-commander delegate. Gauge `FleetCompositionEscalationTests`.

**New units + faction identity** (the developer's "get creative, make new units, AI uses them, per-faction, planetary too"):
- **Slice A** (`581f114`): three new ships filling each faction's real role gap, cradle-to-grave and AI-usable with **zero engine code** (an armed hull is auto-built/fleeted/classified/doctrined): **UMFS Legionnaire** (heavy flak → Line), **UMFS Skirmisher** (light fast flak → Screen), **Kithrin Harbinger** (heavy disruptor/plasma → the long-range Artillery they lacked). Plus a scenario-load gauge (`DevTestFleetRoleReadoutTests`) that builds EVERY faction ship design — closing the blind spot where a `umf.json`/`kithrin.json` registration slip crashes New Game but ships green.
- **Slice B** (`a0864cd`): per-faction fleet composition — UMF grows a **Martian Battle Line** (4/10/20) vs Kithrin's **Raid Swarm** (3/6/12), authored in JSON, read through one choke point (`FleetAssembly.TemplateFor`). Gauge `FleetCompositionPerFactionTests`.
- **Slice D** (`cd224a7`): per-faction ground garrison — the composition logic reaches **planetary units**. UMF's worlds start with a heavier Martian legion (4 Inf / 3 Armor / 2 Arty = 9) vs the default light watch (6), so invading Mars is a harder fight than Earth. Gauge `GroundGarrisonPerFactionTests`.

### Errors made & corrected (the reds, and why)
1. **The Event Logger CI red — `PositionDB` not found** (`0faf625`, reddened ALL shards). `SensorEvents.cs` imported the DEAD `Pulsar4X.Datablobs.PositionDB` (a corpse class that isn't compiled) instead of the LIVE `Pulsar4X.Movement.PositionDB`. There are TWO classes by that name; only the Movement one compiles. One-line `using` fix. **Lesson: when a type "isn't found," check for a dead twin in another namespace — grep the source, don't trust the obvious import.** Before pushing the fix I verified everything the engine failure had masked (the test + client APIs), since a compile break hides all downstream errors.
2. **The reserve REWORK (a design correction the developer caught mid-build).** Slice 3 first shipped as a simple massing cap (stop building at the aspiration target). The developer flagged: *"be sure the AI won't be pulling all fleets at the same time."* The simple cap would build ONE fleet and the strike logic would sail it — the whole navy — leaving home bare. Reworked to reserve-aware: fleets overflow into a second fleet, massing keeps building until strike + reserve BOTH exist, and the strike guard refuses to commit the last organized fleet. **Lesson: "mass a fleet" and "keep a reserve" are both true — the composition must build past ONE fleet or the AI goes all-in.**
3. **Three stale doctrine tests broke when sub-fleet roles landed** (`2693cbc`, `f5e9039` — earlier this session). B-2c2's role sub-fleets changed what `RunFleetDoctrinePolicy` returns; the tests assumed a flat fleet. Fixed by filtering role sub-fleets / selecting the biggest owned fleet.
4. **A DISCOVERY that shaped the plan: support hulls aren't AI-usable without engine code.** An ARMED hull is fully AI-usable for free (the six weapon attributes are what `IsWarship` recognizes; evasion/range auto-classify it). But a zero-firepower support hull (tanker/tender) is built by no resolver, folded into no fleet, AND functionally hollow until a refuel/repair mechanic exists. So we led with armed units and flagged support hulls as a later engine slice rather than ship a unit the AI ignores.

### Lessons (durable)
- **A built NPC ship lands in the faction ROOT fleet (GlobalManager), which `FactionState.OwnedFleets()` — the AI's fleet query — never iterates.** Anything the AI must act on has to be in an in-SYSTEM fleet. This was the entire "the AI builds ships but never attacks" bug.
- **`BaseModIntegrityTests` only covers the COLONY blueprint path, NOT the scenario faction files** (`umf.json`/`kithrin.json` load via `FactionFactory.LoadFromJson`, a different path), so a faction-file registration slip ships green and crashes New Game. `DevTestFleetRoleReadoutTests` now closes that — it builds every faction ship design.
- **An armed hull is cradle-to-grave AI-usable with zero code; a support hull needs new code.** `IsWarship` is a hardcoded six-weapon-attribute check; `ClassifyRole` reads evasion (small+fast = Screen) and weapon range (≥400 km = Artillery). Give a new hull one of the six weapons and the whole conquest chain uses it.
- **Per-faction data flows through ONE choke point.** Both the fleet ladder (`FleetAssembly.TemplateFor`) and the garrison (`GroundStartGarrison.CompositionFor`) read a per-faction number off `FactionInfoDB` set by a JSON node, defaulting to the shared value — "byte-identical off, faction-flavoured on" is one lookup, and the downstream reserve/massing/raise logic needs no change.
- **The `actions_list`/`get_job_logs` results (>430k chars) always exceed the token limit → auto-saved to a file; parse the SHA/status with a `python3` regex over the saved file.** (Confirmed repeatedly this session.)

### Open / flagged (deliberate — WHAT'S LEFT)
- **Screen-bar confirmation.** The Skirmisher is armed + provably more evasive than the heavy Legionnaire, but whether it clears the 0.5 Screen threshold is a one-number read (the gauge prints it) — confirm from the CI printout or a local run; tune thrust/mass if it lands as Line.
- **Support-hull AI-use** — needs (a) a build-rung recognizer + (b) a fleet-fold step, AND a refuel/repair mechanic to make it non-hollow. Deferred.
- **AI ground-REBUILD** — the AI FIELDS its garrison (ships it to invade) but never REBUILDS ground forces; when the garrison's spent it has no path. The next real ground capability (a ground build-rung + a battalion-composition helper — the ground twin of the fleet ladder). Flagged in `Factions/CLAUDE.md`.
- **New ground unit TYPES** (custom stats, e.g. UMF power-armour infantry) — the garrison is per-faction COUNTS today; per-faction STATS + new authored ground designs the garrison raises is the follow-up.
- **Kithrin has no ground presence** (station-only, no region-mapped colony body → raises no garrison). Giving the hive a ground foothold is a scenario/engine gap.
- **Sub-sub-fleets** (the Perfect-tier nested squadron split) — role sub-fleets already form; deeper nesting needs a new mechanism (recursive role-forming on a single-role fleet is degenerate).
- **The military-commander delegate** — the reserve/commit decision is a sensible default with the seam marked; the governance-layer delegate (per-faction reserve/aggression posture) is the eventual owner.
- **Command org 2-tier → 4-tier** (standing task #22).
- **Flagged numbers** (sensible defaults, called out for a balance pass): the aspiration wealth thresholds (`IdealWealth` 50k / `PerfectWealth` 150k); the reserve-guard "keep 1 organized fleet"; UMF's 4/10/20 fleet + 4/3/2 garrison and Kithrin's 3/6/12; the new ships' build costs; the Event Logger halt rule (at-war OR unknown-foreign).

---

## ⏩ Session 2026-07-06 — WEAPON UNIFICATION + the planetary-unit designer wired to the battlefield (READ FIRST)

On branch **`claude/4x-hex-movement-pathfinding-o7qeaa`**. **Every slice CI-green, one slice per push, waited for green before stacking.** All engine work (client UI for the ground-unit designer is a later, CI-blind slice). The branch is **CLOSED** — its A+B work is done and mergeable; the big **combat-resolver MERGE is the NEXT branch** (see "Where to resume").

The through-line: the developer's **ONE weapon designer** vision — you build any weapon by picking Nature × Delivery and turning dials; the triangle corner EMERGES; the same weapon design mounts on a ship OR a planetary chassis, gated only by whether that chassis can SUPPLY it (mass + power + ammo). Then: make a *planetary unit* buildable+fieldable the same way, and give it ammo that depletes and resupplies.

### What shipped (in order, each its own green commit)
- **Weapon-unification clean pass** — deleted the vestigial `WeaponClass` type slot from `WeaponProfile`'s ctor + all ~86 call sites; `Class` is now a pure computed read-out (`WeaponClassifier.Classify(delivery, vel, trk, sat)`). The dials win; there is no hand-picked type.
- **P1 — direct-fire weapons ground-mountable.** Added `ComponentMountType.GroundUnit` to the five direct-fire weapon templates (laser/railgun/flak/disruptor/plasma). Ordnance launcher + defensive deflector deliberately excluded. Gauge `WeaponGroundMountTests`.
- **P2a — reactors ground-mountable.** Same move for the three fuel-burning generators (`reactor`/`rtg`/`steam-turbine-reactor`); storage + solar excluded. Gauge `PowerPlantGroundMountTests`.
- **P2b — the POWER SUPPLY GATE** ("a Titan can, infantry can't"). New `WeaponSupply` (pure): a weapon's **supply mode** Energy/Ammo/Both (smart default from its atbs — laser/plasma/disruptor=Energy, railgun=Both, flak=Ammo), its `PowerDraw_W` = its own energy flux (no efficiency coefficient invented), and reactor `ReactorOutput_W`. `GroundUnitAssembly.Compute` hard-refuses a design whose energy weapons out-draw its reactors, and a non-ground part (weapon/reactor) now counts its mass against the carry budget — so the two gates COMPOSE (infantry can't power the big laser because it can't carry a reactor big enough). Gauge `GroundPowerGateTests`.
- **P2c-a — the AMMO GATE.** `GroundMagazineAtb` (mass-based ammo store) + `WeaponSupply.DrawsAmmo`; `GroundUnitAssembly` hard-refuses an Ammo/Both weapon with no magazine. Gauge `GroundAmmoGateTests`.
- **P2c-b — the buildable magazine.** Base-mod `ground-magazine` (six-point registration); binds the atb, satisfies the gate. Capacity 500 kg + mass=2×cap **flagged** in `installations.json`.
- **Design-lock — ONE combat resolver.** Recorded the developer's call: *"there is no ground tactics, just tactics; one weapon triangle; why a separate resolver?"* → planetary combat is the SAME resolver + physical modifiers (terrain, absolute range, the AIR layer). Terminology: **"planetary," not "ground."** (`WEAPON-UNIFICATION-DESIGN.md` §0 pt 6-7.)
- **Slice A — THE CONNECT.** `GroundUnitAssembly.RegisterAssembledDesign` (assemble frame+parts → register as a buildable faction design); a completed build FIELDS a unit on the planet with the emergent stats. Gauge `GroundUnitFieldingTests`.
- **Slice B — ammo (resolver-independent half).** `GroundUnit.MaxAmmo_kg`/`CurrentAmmo_kg` (snapshot of Σ magazine capacity at raise) + pure `GroundAmmo` (Consume/IsDry/Refill/Fraction); `GroundForces.ResupplyUnit` (manual resupply v1: rearm on friendly-held ground). Gauge `GroundAmmoTests`. Built deliberately resolver-independent so it survives the merge.

### Errors made & corrected (the reds, and why)
1. **P2c-b red — `ground-magazine was not found in the faction data store`** (every colony-build test). Cause: gotcha #10 has **TWO unlock ends** and I only did one — I added the design + listed it in the colony's `ComponentDesigns`, but did NOT add the **template id to `StartingItems`** (which is what UNLOCKS the template for the faction). My commit message even wrongly claimed "StartingItems untouched" — I'd conflated unlocking the *materials* (already done) with unlocking the *template*. Fix: add `"ground-magazine"` to `StartingItems`.
2. **Slice A red #1 — `resources can't cost 0`.** I zeroed the design's `ResourceCosts` to isolate the build mechanic from mineral stock; the industry system rejects a zero-cost job. Fix: a minimal non-zero cost (`stainless-steel: 1`, which the colony stocks in bulk).
3. **Slice A red #2 — `HasDataBlob<GroundForcesDB>` was False.** The queued build didn't complete-and-raise inside the `CreateWithColony` harness — the `installation-construction` queue path isn't reliably driven to completion there, and no other test exercises it. Fix: drive `GroundUnitDesign.OnConstructionComplete` **directly** (the exact call `IndustryTools.ConstructStuff` makes at completion, `IndustryTools.cs:214`) for a deterministic proof; the generic queue is already covered by `ProductionBuildTests`.

### Lessons (durable)
- **gotcha #10 has TWO ends and they're different unlocks.** For a new base-mod buildable: the **materials** it costs must be unlocked (in `StartingItems` or by a starting tech) AND the **template id itself** must be in that colony's `StartingItems` to be available in the faction data store. Miss the template-unlock and `ComponentDesignFromJson` throws "X was not found in the faction data store" on New Game / every colony-build test. Don't conflate the two.
- **The industry system rejects a zero-cost job** (`resources can't cost 0`). To isolate a build gauge from mineral stock, use a *minimal non-zero* cost of a bulk-stocked material, not an empty cost dict.
- **For a ground-unit build→raise gauge, drive `OnConstructionComplete` directly.** The `installation-construction` queue-to-completion path isn't reliably driven by `CreateWithColony` + `AdvanceTime` (no green test exercises it), so a queued-build assertion is flaky. The completion hook is deterministic and is the exact call the processor makes. The generic queue is proven by `ProductionBuildTests`.
- **The industry completion dispatch is single-point:** `IndustryTools.cs:214` calls the design's OWN `OnConstructionComplete` and nothing else stockpiles alongside — so a `GroundUnitDesign` raising a unit (instead of adding to cargo) is clean; no InstallOn needed.
- **Build resolver-independent when a system is slated for a refactor.** Ammo depletion's pool + helper were built pure/data (survive the coming resolver merge); only the in-combat *drain call site* is merge-coupled and was deliberately deferred. This avoids building the same logic twice into a resolver about to be replaced.
- **The huge `actions_list` result** (>430k chars) always exceeds the token limit → it auto-saves to a file; parse it with `python3 -c "json.load(...)"`. Same for big `get_job_logs` (the TRX summary is alphabetical — a failing `G…Tests` is far from the tail, request a large `tail_lines` and grep the saved file).

### Open / flagged (deliberate)
- **Flagged numbers** (sensible defaults in JSON, called out for a balance pass): magazine `Capacity` 500 kg (range 100–5000) + `Mass`=2×Capacity + `CreditCost`=0.2×Mass; the reactor kW→W unit constant (arithmetic, not a dial); resupply v1 source rule = "friendly-held ground" only.
- **Deferred to the merge branch (coupled to the resolver):** in-combat ammo DRAIN + "ammo weapons silent, unit fights on," the **ammo-per-shot WEAPON-DESIGNER spec** (the developer's call), and the whole ground-reads-the-triangle work.
- **Plasma supply mode = the PLAYER's choice** (Energy vs Both) — the overridable designer setting ships with the merge (Energy-vs-Both only bites once ammo is a real in-combat cost).
- **Client UI** for the planetary-unit designer (assemble frame+parts, see/build units) is unbuilt (CI-blind, a later slice).

### Where to resume — the RESOLVER MERGE (next branch, LOCKED framing)
The next branch extracts the shared weapon-triangle / dodge / shield / armour **damage kernel** onto a neutral **combatant view** that BOTH a ship entity and a planetary `GroundUnit` present, and routes both combat paths through it — then deletes the duplicated ground math. Planetary combat contributes terrain + **absolute metric range** (100 km is 100 km everywhere; hexes are just the board — `GroundRangeTools.RealReachKm` maps it) + the **air/altitude layer**. The Armor▸Infantry▸Artillery type-triangle **dissolves** into weapon×armour matchups. Phased slices: (1) shared kernel, additive + tested; (2) route space through it; (3) route planetary through it + delete the duplicate; then the deferred B pieces (drain/silence/ammo-per-shot) land where per-weapon firing lives. Full plan: `docs/WEAPON-UNIFICATION-DESIGN.md` §0 + the P3–P5 phase list. **This is a rewrite of the only green combat code — start it with the developer's explicit go, not autonomously.**
...
7. **Warp/move (the spawned-ship fix):** give any of the six combat ships (Aegis / Picket / Lancer / Bulwark / Wasp / Leviathan) a move order → it should **warp immediately**. A DevTools-spawned ship is now fuelled **and** reactor-charged on spawn (watch the spawn DevLog: `fuel=+… units, energy=+… KJ`) — no more "wait for the reactor to charge." If one ever still sits there, the log says exactly why: `[WARP] … CAN'T WARP — <reason>`.
...
- **Cradle-to-grave includes the TRAVEL rung — "can it actually get there?"** The live "spawned ship won't move on a move order" symptom had **two stacked causes**: (1) the six combat ships had guns/armour/thrusters but **no warp drive** (utility ships do), so `WarpMoveCommand` had nothing to engage and `return`ed silently (the literal `// FIXME: alert the player?`) — surfaced via `[WARP]` + fixed by adding Alcubierre drives (`c67b8d2`); (2) **the deeper one, found when the developer said "it's STILL not moving" after the drives** — a freshly built/spawned ship boots with an **empty reactor** (`EnergyStored`=0), and **warp is paid from stored electricity, not fuel**, so even a drive-equipped ship couldn't form the bubble. The precise *"what do the premade ships have that ours don't?"* answer (the developer's framing — exactly right): the start fleet is **hand-charged to 2,750,000 in `DefaultStartFactory`**; the DevTools/sandbox spawn fuelled the tanks but **never charged the battery**. Fix: **`ShipFactory.ChargeReactors(ship)`** — the energy sibling of `FillFuelTanks` — called by both spawn paths, topping `EnergyStored` to the ship's own `EnergyStoreMax` (a charged base-mod battery holds **2×–4× a warp bubble** at starting tech: battery-2t = 1,000,000 KJ; alcubierre-2k bubble = 1,000,000 KJ, -500 = 250,000 KJ — verified from the JSON formulas, not assumed). Sensor: `CombatTestShipsTests.ChargeReactors_FillsStoredEnergy_SoASpawnedShipCanWarp`. **Lessons: (a) a unit that can't reach the fight is a missing rung, not a footnote; (b) a silent `return` on a player order is an invisible bug (Visibility Gate); (c) "diff the WORKING one against the BROKEN one" beat theorising — the answer was a hand-charge in the start factory the spawn path skipped, invisible until you compare the two build paths side by side.**
- **Spawn parity — a convenience-spawned unit must get everything the start factory hands a native one.** "Ready to fly" is the hull PLUS what `DefaultStartFactory` does *after* the build (fuel, reactor charge — tomorrow maybe crew / ammo / ECM charge). There are **two** convenience-spawn paths — `DevToolsWindow` "Spawn Ship" and `CombatSandbox.SpawnHostileFleet` — and each must mirror the start setup, or a spawned unit silently behaves differently from a native one (here: full tanks but a dead battery → it wouldn't warp *or* fire). **Rule: when you add any new "ship is ready" state to the start factory, extend BOTH spawn helpers too. `FillFuelTanks` + `ChargeReactors` are the pair that exists for this — add the next one beside them, don't bypass them.**
...
### Fog of war for units — detected contacts as blips, undetected hidden (2026-06-26)

**The ask (developer):** "make the detected contact show up like a normal unit but with only the information i have about it which can vary" + "my faction cant see the units from other factions [unless detected]… everyone sees the same star but not everyone can see the massive fleet around it."

**What it does.** With Fog of War on (DevTools › Detection / Fog of War — the same one switch as detection-gated combat), the map now behaves like real sensors:
- The **star and planets always show** (fixed geography everyone has).
- **Your own ships** show normally.
- A **rival faction's ships are HIDDEN** until your sensors detect them — and even their orbit/move trails are hidden, so nothing betrays them.
- A **detected** rival ship shows as a **contact blip**: a red diamond at its last-known position with its name, sized a touch by how loud the return is. Lose the track (it leaves range or is destroyed) and the blip **fades to a "(last known)" ghost** where you last saw it — the grave rung made visible.
- With Fog off, everything draws as before (no change).

**How it's built (all client-side except two tiny engine accessors).**
- `SensorContactIcon` (`Pulsar4X.Client/Rendering/Icons/SensorContactIcon.cs`): a real map `Icon` fed by the engine's `SensorContact`. The contact's last-known position is a `SensorPositionDB`, which is an `IPosition`, so it drops straight into the `Icon` constructor like any unit. The name draws with the same SDL text path the normal entity labels use.
- `SystemMapRendering`: a `_contactIcons` set rebuilt each frame from `GetSensorContacts(faction).GetAllContacts()` (`UpdateContactBlips`), drawn through the per-item SafeDraw guard; plus a guard at the top of `AddIconable` that skips the real icon/label/trail for a foreign mobile unit when fog is on (so it only ever appears as a blip). Both gated on `CombatEngagement.RequireDetectionToEngage`.
- Engine: two public read-only accessors on `SensorContact` (`SignalStrength_kW`, `PositionIsMemory`) so the client can read the loudness + the "is this a memory ghost" flag (the underlying detection fields are internal to the engine). CI-covered.
