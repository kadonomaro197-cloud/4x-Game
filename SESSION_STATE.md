...
Session 2026-06-26 — **live-test hardening on `claude/focused-ritchie-debock`.** The session recorder (`SessionLog` flight recorder) is built and live-confirmed; the dead-ship click-crash and ghost-icons-on-the-Sun are fixed; and the **combat interrupt now stops the clock at first contact** (round 2 below — `CombatReactionStep` fine-stepping in `MasterTimePulse` + the `CombatActiveOrImminent` gate), fixing "combat happened suddenly with no time to react." Open thread: the warp→Sun teleport (a pre-existing movement edge case) is now auto-detected by the teleport heartbeat but not yet root-fixed (needs one more live repro). See the 2026-06-26 entries below. Prior state ↓.
...
**Round 2 — the granularity fix (THIS commit).** Root cause: `RequestCombatHalt` cancels the token, but `MasterTimePulse.SimulateTimeUntil` only checks it at the **master-loop boundary**, and one coarse `Ticklength` (1 hour) is processed inside a single `ProcessSystem` call — so the whole battle resolved before the cancel was seen. Fix: when the interrupt is armed **and** a battle is active/imminent, the master loop advances in fine **`CombatReactionStep` (5 s)** sub-steps instead of a full Ticklength, so it returns to its boundary and honours the cancel **within one 5-second sub-pulse of first contact.** The gate is the new read-only `CombatEngagement.CombatActiveOrImminent(manager)` (any fleet already in combat, or two hostile fleets with ships in range), consulted per active system via `MasterTimePulse.AnyActiveSystemInOrNearCombat()`. **Cheap away from combat** (full Ticklength steps as before — the gate only runs when a cap could apply and the flag is on), and **lockstep-preserving** (all systems still advance to the same capped target each iteration — no cross-system time desync / Temporal Anomaly). Round 2 between leviathans is a *new* engagement, so it re-halts → the developer gets their reaction time.

- **Files:** `GameEngine/Engine/MasterTimePulse.cs` (`CombatReactionStep` const, the per-iteration cap in `SimulateTimeUntil`, `AnyActiveSystemInOrNearCombat`), `GameEngine/Combat/CombatEngagement.cs` (`CombatActiveOrImminent`). **Tests:** `BattleTriggerTests.CombatActiveOrImminent_*` (gate true/false cases) + the existing `Tick_NewEngagement_RequestsCombatHalt_WhenInterruptEnabled`.
- **v1 limits (in `Combat/CLAUDE.md`):** halts on *any* faction's new engagement (no engine-side "player faction" yet); one salvo lands before the halt; a fleet that **warps in from out of range and fights inside one coarse step** is still resolved coarsely (present-state gate, no closing-prediction lookahead); a *step* issued during combat advances ~5 s, not the full slider Ticklength (intentional).
