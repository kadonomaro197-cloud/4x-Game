...
**There is ONE plan, not two.** The march is still `docs/MVP.md` ("take a planet"): Stage 1 space combat ✅ done → **Stage 2 ground combat (next build)** → Stage 3 stitch the loop → Stage 4 UI → v1. The new `docs/REALISM-VS-GAMEPLAY-AUDIT.md` did **not** add build steps — it's the **weight-firewall companion** to MVP's scope-firewall (*name the player decision before building the realism*), and it turned MVP §6 Parking Lot into a **leverage-ranked backlog** for after v1. The stale `PLAN.md` is now marked **SUPERSEDED** (its Phase 1 is done / its per-pixel-combat approach was replaced by the auto-resolve engine) so nobody backtracks onto it.
...
3. **Detection** — answered + **parked as ranked lever #3** (simple model + active/passive/EMCON, NOT the spectrum sim; seam = one method in the combat trigger). **Not built now** — the MVP firewall (known, static defender) doesn't need it, and staying on the spine is what *prevents* the omniscience debt. Do NOT build the sensor gauge/wiring now.
4. **Stage 2 ground combat** — the next MVP build; mirror Stage 1's **doctrine decision-spine**, not the realism (PLAN.md Phase 4 is good *design* reference for the Formation/Element/`GroundForces/` shape; build the minimal MVP slice).
5. **Combat depth built this session** (doctrine/dodge/triangle/interrupt) — done; the audit grades it mostly **EARNS WEIGHT** (doctrine is the real decision; interrupt is its enabler). It's the template for Stage 2.
6. **Economy UI verify** (Stage 0 remainder) — still pending the developer's live check; feeds Stage 4.

**Recommended next move:** two parallel tracks that don't collide — (a) *developer, on your machine:* live-test the combat interrupt + repro the teleport; (b) *Claude, engine-side & CI-verifiable:* start **Stage 2 ground combat** as a decision-spine (`GroundUnitDesign : IConstructableDesign` + `GroundForcesDB` + `GroundCombatProcessor` + tests), pending the developer's go-ahead on opening that subsystem.
