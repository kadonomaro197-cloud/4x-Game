...
1. **One slice per commit; push; WAIT for CI green (all 5 jobs) before the next slice on that branch.**
...
- **G2 (GROUND)**: battalion/campaign engine — `FormUpLoose(body, faction)` helper (AI formation parity; garrison + landed invaders form battalions per `GarrisonComposition`), auto-campaign advance step (flag-gated: invader formations with empty queues advance toward nearest enemy-held region), ammo drain in `ResolveRegionCombat` + resupply caller, UpkeepCredits values at both design-builder sites, manager support micro-helpers (`RenameFormation`, `AllFormationsFor`, `RadarReachHexes`).
...
The 200 Gm homeworld sensor horizon (C2 memo) · built-ships-boot-charged policy for PLAYER ships (P4.2) · recall-vs-press doctrine details (P3.4 defaults chosen, reviewable) · window name (C3) · infra-combat decisions 1-6 (`findings/R4`) · all FLAGGED balance numbers (upkeep, ammo-per-salvo, station tax, cadre costs).
