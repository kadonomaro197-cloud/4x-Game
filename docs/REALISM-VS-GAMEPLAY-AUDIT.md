...
1. **Refined materials → component costs.** Make weapons/shipyards require steel/electronics/etc. Instantly makes **mining → refining → production → combat** one supply-chain decision. Converts 3–4 PRETTY/LATENT systems into a single stacked choice. *Highest leverage in the game.* (Mostly JSON data + a `BaseModIntegrityTests` check.)
