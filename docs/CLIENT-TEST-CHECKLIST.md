...
- [ ] **G3 — heads-up on the map's shape (known limitation).** The current view is a **legible readout** — region columns with terrain bands + percentages — *not yet* a map you navigate units on or click to place bases. That's the deliberate base; the tactical map (click-to-place, units on the surface, terrain-as-leverage) ships with **slice 5 (ground units)** so the map and the things that move on it land together. So for now: confirm it *reads* correctly; the "playable surface" is the next step.
