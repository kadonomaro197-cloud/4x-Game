...
The Planet View is no longer just a readout. These checks need **units on the ground** — get them from **DevTools (SM mode) → "[ Raise Ground Unit ]"**: pick the body in the "Orbit around" picker (up under Spawn Ship), choose a type + count + region, and click **"Raise (your faction)"** (and **"Raise (enemy)"** in the SAME region to test the fight + capture). (There's no build-a-unit-from-industry path in a stock New Game yet — the base-mod JSON template is deferred; this DevTools button is the placement tool for now.) Report what you see either way.
