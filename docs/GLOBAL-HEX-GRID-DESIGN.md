...
- **G5 ✅ built (2026-07-04, ADDITIVE — the VISIBLE slice).** `PlanetViewWindow.DrawGlobalHexWindow` renders the cylinder as a **sliding window** over `PlanetRegionsDB.SurfaceGrid`: centred on a longitude column (`_centerCol`), ~2 region-bands wide (centre band full + neighbours bleeding in), **wrapping at the seam** so a place shows up in every window whose longitude reaches it. Terrain is continuous by construction; a faint seam line + label marks each region-band boundary. Units draw at their **GLOBAL (Q,R)**; click a hex to select your group / march it via `GroundForces.OrderMoveToGlobalHex` (wrapping A\*, **no edge gates**). Uses an **odd-r OFFSET** hex layout (`HexCenterOffset`) so a wide window reads rectangular (not sheared), matching the G2 pathfinder's neighbour model. A **◑ Globe / ▦ Band** toggle keeps the legacy per-region disk view for comparison; the globe is the default when the grid exists. The region-based side panels (selection/build/formation/detail) still drive off `_centerRegion`, which the globe keeps in sync (◀/▶ and clicks recentre the band). Additive — `DrawThreeRegionHexMap` is untouched (fallback). **CI compiles it; render/feel is the developer's local build.** Design consumers: this is the continuous-world payoff the developer asked for ("1 hex map showing 3 regions at once, centre full + neighbours a fraction").
- **G6a ✅ built (2026-07-05, CLIENT) — retire the band view.** The developer validated the globe view live and asked to drop the old per-region "band" view. `PlanetViewWindow` now renders **only** the continuous-globe window: the `◑ Globe / ▦ Band` toggle, `DrawThreeRegionHexMap`, the H4 `DrawHexMap` drill-in, and their now-dead helpers (`HandleHexClick` / `MoveSelectedToHex` / `HexCenter`) are **removed**. `DrawTacticalMap` always calls `DrawGlobalHexWindow`. Plus a **detailed collapsible legend** at the bottom of the globe view (`DrawLegend` + `LegendRow`): unit ownership + type initials + selection ring, and every terrain colour grouped (Water/Lowland/Upland/Cold/Gas, Ocean flagged impassable), and the seam/label/pan cues. The client is now **fully off the disks**.
- **G6b — retire the ENGINE disks (IN PROGRESS — the developer authorized it 2026-07-12: "troops move on planetary hexes not the fine hexes").** Sliced additive-first (this doc's own rule) so CI stays green. **G6b-1 ✅ (additive, 2026-07-12):** added the one genuinely MISSING global mover — `GroundForces.OrderFormationMoveToGlobalHex` (the planetary-grid twin of the per-region `OrderFormationMoveToHex`: marches a whole formation on the continuous cylinder via the tested per-unit `OrderMoveToGlobalHex`). Nothing live-calls it yet → byte-identical; gauge `GroundForcesTests.OrderFormationMoveToGlobalHex_MarchesTheBlockOnThePlanetaryGrid`. **G6b-2a ✅ (2026-07-12, behaviour change):** repointed the processor's queued formation `GroundOrderType.MoveToHex` case to `OrderFormationMoveToGlobalHex` (a queued formation move now marches on the planetary grid; `TargetQ/TargetR` are global cylinder coords), and `LeaderIdle` now checks BOTH the per-region and global paths so the order queue pops on either grid. Re-baselined `Orders_SequentialWaypoints_LeaderEndsAtFinalHex` to global `GlobalQ/GlobalR` (the only test that executed a queued MoveToHex; queue-mechanics + ROE/Hold tests unaffected). **G6b-2b (next):** move the ROE step-back-and-fire (`ApplyEngagementManeuvers`/`HexDist`/`PickStepHex`) — the in-combat tactical micro still on the per-region fine grid — to the global grid (re-baselines the ROE hex-position tests to `GlobalQ/GlobalR`; region-level capture/combat/bombardment tests are SAFE because the global march still maintains `RegionIndex`). **G6b-3:** delete the disk gen + edge-gate `FindPath` + dual coords once no consumer reads them. Audit ledger (file:line for every consumer) captured 2026-07-12. Original scope below:
  The engine still generates + uses the per-region disks (`PlanetHexFactory` disk gen, `Region.Hexes`, the edge-gate `HexPathfinder.FindPath`, the per-region `OrderMoveToHex` + processor fine-march, and W1/C1's disk-addressed variants) as the additive scaffold. Now that no client consumer reads them, retire them: migrate the remaining engine consumers to the global grid, delete the disk gen + edge gates + dual coords, and repoint their tests. Bigger + destructive — do it as its own CI-gated slice.
