...
4. **Survey reveal** — scanning a world flips `Surveyed` and reveals its features; wires into `GeoSurveys`/`Sensors` (the detection-quality fix already landed as the prerequisite). **← next.**
