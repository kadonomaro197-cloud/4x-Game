...
- **5h — base-defends-city coverage + formations.** *Partial: fortification ✅ built (2026-07-04).* A region's located buildings (`Region.InstallationIds` — populated by the tactical-map Build panel) now **fortify its defender**: `GroundForcesProcessor.FortificationDefenseMult` divides the defender's incoming by a capped bonus (each building +15%, max +100%), stacking with terrain cover — so a built-up capital is a fortress and placing a base on the map has a real combat payoff. Gauge: `GroundForcesTests.Fortification_BuildingsFavourTheDefender_InAFight`. **Formations ✅ built (2026-07-04)** — the ground echo of a fleet, mirroring its SHAPE in the data-object model (developer's call: "mirror the fleet as much as possible"): a `GroundFormation` (name + leader + members via `GroundUnit.FormationId`, a new stable `UnitId` = the entity-id echo), the `GroundForces` verbs (`CreateFormation`/`AssignUnit`/`UnassignUnit`/`SetLeader`/`DisbandFormation`), move-as-one (`OrderFormationMove`), and leader-reassigns-on-death (fleet-like, no penalty). Gauge: `GroundForcesTests.Formation_MirrorsFleet_*`. **Fortification is now DESIGN-DRIVEN + base-adjacent ✅ (2026-07-04, the developer's "depending on base design")** — merging the old "adjacent coverage" and "real ground-defence component" items: a building fortifies only if its design carries a `GroundDefenseAtb` (`LocalFortify` + `AdjacentProjection`), so a **Bunker** (a real base-mod installation — the six-point registration, +25% local / +12% adjacent) hardens its region AND projects to adjacent *friendly* regions, while a solar panel does nothing. `GroundFortification.DefenseMult` reads it (colony components resolved by installation id), capped at +100%. Cradle-to-grave: research → build the Bunker → place it on the map Build panel → it fortifies → destroyed = the region softens. Gauges: `Fortification_DesignDriven_*` (math) + `Fortification_BaseModBunker_BuildsAndWinsTheFight` (the real JSON bunker wins a fight). **Still to build for 5h:** formation **sub-nesting** (the fleet tree). **Stance/doctrine ✅ built (2026-07-04)** — a full **moddable catalog** mirroring the fleet's (`GroundStanceBlueprint` → `ModDataStore.GroundStances` from `groundStances.json`; `GroundFormationDoctrine.TrySetStance` with the switch cooldown): the **±25% trade-off** (Offensive +25% dealt/+25% taken · Dig-In −25% taken/−25% dealt · Balanced), applied per-unit in `ResolveRegionCombat` (attack × `AttackMult`, incoming × `DamageTakenMult`), stacking with terrain + fortification. Gauge `GroundForcesTests.FormationStance_*`. Client stance selector is the follow-up half. The formation **client** ✅ built (2026-07-04): a Formations panel on `PlanetViewWindow` — form up idle units in a region, then a selectable formation list with march-as-one + disband (thin callers over the engine API; CI-compiled, runtime-local).
