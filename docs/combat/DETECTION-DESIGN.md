...
> **⚠ Corrected by the 2026-08-09 ledger (`docs/DOCS-CONTRADICTIONS-2026-08-09.md`):**
> - **§3a/b/c "obscuring hazards don't exist yet / masking is v2 / LATENT" is stale — it SHIPPED.** Detection already reads the environment: `SensorScan.cs:83` calls `SpaceHazardTools.CombinedAt`, a gas cloud's SensorJam ×0.35 cuts sight, a solar flare blinds sensors, and a sensor-hardening module counters it (see `docs/combat/ENVIRONMENT-CONDITIONS-DESIGN.md`). "You can't see far in a nebula" is shipped behavior, not a proposal.
> - **`SignalQuality` — the 2026-07-07 "CUT/deleted" banner was NEVER executed** (correction 2026-07-28): the field is live in ~9 files and load-bearing for survey-reveal accuracy. Treat every "SignalQuality is cut" line below as retracted; the open question is *keep-and-rehome*, not delete (ledger blocker #3).
>
...
6. **UI ✅ PARTIAL (client — needs the developer's local build; CI can't see it).** Built: the **EMCON posture selector** (`FleetWindow` Combat tab — Full/Cruise/Silent → `FleetEmcon.SetPosture`) and the **fog-of-war toggle** + **live signature readout** in DevTools (`RequireDetectionToEngage` checkbox; clicked-ship `ActivityMultiplier`). Deferred: **contacts as blips on the map** (the "see what you detect" view — a contact-icon renderer; higher-risk CI-blind work, left for a deliberate pass), the range ring, and the sensor/EMCON **component in the designer** (research → build). See `Pulsar4X.Client/CLAUDE.md` → "EMCON posture + fog-of-war UI".
