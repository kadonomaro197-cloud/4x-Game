...
> **⚠ Keystone C1 was overtaken by the build — reconcile (2026-08-09 ledger).** §5/§10/§12 say "there is ONE bay component … promote `GroundBayAtb` → a general `BayAtb` … do not build a separate hangar … do the promotion FIRST (C1) so nothing builds a second bay type." What actually happened (2026-07-30): a **separate `DockBayAtb`** ship dock was built — "a DOCK, berths for whole vessels … its OWN area, not another cargo hold" (the developer's call) — `GroundBayAtb` was **never renamed**, and the C1 promotion is **unexecuted**. So the "one bay, no second type" plan is superseded by a deliberate two-mechanism reality (a whole-vessel dock ≠ a strike-craft/troop bay). Update C1 accordingly before treating it as the plan.

**Status:** **OFFICIAL / design-locked, ready to build** (2026-07-14; **refined 2026-08-04 — see §13**). The four §9 decisions are made, the bay class includes Shuttle, and the full C1 blast radius is mapped and verified tractable (§12). Prime-Directive investigation done against the live combat/weapons/ship/bay code; the EXISTS ledger + the §12 touch-list are file-cited. Read `docs/combat/COMBAT-DESIGN.md` (the one auto-resolve loop + doctrine) and `docs/combat/WEAPONS-DESIGN.md` (the weapon triangle) first — carriers plug into both, they don't replace either. **§13 (2026-08-04) sharpens three things the developer called out when we went to model this in the resolver sim: where the "deployable" designation lives, that recover is per-type not universal, and that the launched wing is a typed sub-fleet carrying its host's id.**
...
## 13. Refinements (developer, 2026-08-04) — the squadron as a typed unit, and the one boundary the generalization respects

*Captured when we went to model carriers/fighters in the resolver sim (`RESOLVER-SIM.md`). A six-system Prime-Directive survey of the as-built engine + an adversarial pass produced three sharpenings. **All three are consistent with the §9 locked decisions** — they clarify HOW the wing is typed and WHERE the "carried" designation lives, and they draw the one line the "works for any chassis" generalization must not cross. Nothing here reopens a §9 decision; it makes them buildable without walking into a dead dial.*

### 13a. The "deployable" designation lives on the unit's CARRY-CLASS — never as a boolean flag on the carried unit

The developer's phrasing was "flag a unit as a fighter, but generalize it so it works for any chassis." The honest, engine-true form: **the flag is the chassis's carry-class**, and the cradle-to-grave home is the HOST's bay (§9 decision 4), not the carried unit.

- A fighter's chassis declares `BayClass.StrikeCraft`; a rifle platoon's declares `Personnel`; a tank's declares `Vehicle`. The **matching host bay** carries it. This already exists for ground — `GroundChassisAtb.CarryClass` (`GroundCombat/GroundChassisAtb.cs`) declares what class a unit *is*, and `GroundTransport` matches it to a bay's class. The C1 generalization (`GroundCarryClass → BayClass`, §12) extends that same field to `StrikeCraft`/`Passenger`/`Shuttle`. So a per-unit designation is real, it drives unit↔bay matching, and it is **not** a new mechanism.
- A bare `IsFighter`/`IsDeployable` **boolean on the carried design** would fail the `DESIGNER-NORTH-STAR.md` **intrinsic test**: deployability depends on whether a matching host bay exists (an *assembly*/emergent property), so a stored flag on the unit writes to no sim variable and reads from none. That is exactly the shape of the **dead scaffolding the 2026-08-04 survey found** and which must NOT be reproduced: the authored-but-unread **"Fighter Construction Points"** dial (`installations.json:424-430`, absent from the factory's `IndustryAtb` DataDict so nothing reads it), the never-populated `ColonyInfoDB.FighterStockpile` (`Colonies/ColonyInfoDB.cs:36`), the never-read `AbilityType.FighterConstruction` (`Enums.cs:225`, lone write at `FactionAbilitiesDB.cs:48`), the publisher-less `Fighter*` events (`EventTypes.cs:93/374/405`), and the display-only `ComponentMountType.Fighter` tag (`Enums.cs:138`). These are the L1 "dead code that looks live" corpses — a real strike-craft system fills or prunes them; it does not add a sixth.
- **"Fighter" is not a real engine concept today** — it is purely emergent from small-volume + high-thrust → high `Evasion` crossing `FleetRoleComposer`'s 0.5 Screen threshold. That's fine: a strike craft *is* just a small ship design (§4b), and its carry-class is what makes it hangar-borne.

### 13b. LAUNCH generalizes to every chassis; RECOVER does not — it is a per-type doctrine outcome

The developer: "fighter squads can't retreat — they launch and dock." True for a fighter. But the generalization to infantry has one hard limit, and the adversarial pass is right about it:

- A **fighter (StrikeCraft)** is **round-trip**: launch → strike → **recover & rearm** at the host (§6 steps 2/5). Its doctrine "break-off" outcome *is* recover-to-host, and it strands if the deck/carrier is gone (§6 step 6).
- A **marine (Personnel)** is **one-way**: it deploys onto the target surface (`GroundTransport.LandTroops` → a hex on `ColonyHexMapDB`) and **holds ground or dies** — it does not dock back to the dropship mid-battle. Different coordinate space (surface hex, not orbital `PositionDB`), different lifecycle (one-way, not round-trip).
- **Therefore "a squadron's retreat = recover-to-host" is a per-CLASS rule, not a universal law.** What generalizes across every chassis is the **ORDER VERB** — one queued *Launch/Deploy* command both the player and the AI issue (One Verb, Both Seats; route it through the order queue, never a direct engine call — the ground click-to-march bypass is the anti-pattern). What is per-class is the recover semantics: StrikeCraft/Shuttle recover round-trip; Personnel/Vehicle deploy one-way. **Expendable one-way craft** (assault drop pod, kamikaze drone) also opt out of the strand rule — losing their host after they deploy is fine.

### 13c. The launched wing is a TYPED sub-fleet carrying its host's id — and that host back-ref is load-bearing

§9 decision 2 already makes the wing a sub-fleet with its own doctrine. The developer asked that it also be a distinct *type*. It must be, and the reason is the strand rule:

- Type it with a **new sibling marker blob** (e.g. `DeployableSquadronDB { HostShipId }`), following the passive-marker pattern of `FleetRoleDB`/`FleetCompositionDB` (one `Clone()`, `[JsonProperty]`s, no processor). **Do NOT** add a value to the `FleetRole` enum — that enum is *derived* from ship stats by `FleetRoleComposer.ClassifyRole` and **re-bucketed every monthly pass** by `FormRoleSubFleets`, so a `Deployable` value would be clobbered. (`FormRoleSubFleets` must also learn to skip an already-typed sub-fleet.)
- The **`HostShipId` back-reference is the only state that makes "recover, else stranded" resolvable.** Today the moment a craft launches it leaves the carrier's `DockedShipsDB` (a flat forward list on the carrier), and **`ShipInfoDB` has no host field** (grep-confirmed: no `HostShip`/`CarriedBy`/`ParentCarrier`). So without the tag, a wing that's aloft has nothing to recover *to*. Build order: the tag + host back-ref + a **`CanRecover`** reachability gate (the launch-side `CanDock` mirror — same-system + a recovery window) land **before** the strand rule is advertised, or "stranded" is a claim with no state behind it.
- **Wire the grave rung that already has a primitive but no caller:** `DockTools.UndockAll(carrier)` exists and releases the wing alive, but **no destruction path calls it** — the ship-death path (`Damage/DamageComplex/DamageProcessor.cs:484-492`) just removes the entity. Carrier death (and station death) must call `UndockAll` (or the strand handler) so the §6-step-6 "carrier dies → wing stranded" is real, not dormant.

### 13d. Target priority + positioning are doctrine fields to WIRE (authored, currently dropped)

- **Target priority:** the `TargetPriority` enum + `CombatDoctrine.ParseTargetPriority` exist, but **the parser has zero callers**, `FleetDoctrineDB` has no field for it, and `FleetDoctrine.TrySetDoctrine` (`FleetDoctrine.cs:54-66`) copies six fields and **drops** `TargetPriority` (plus `RetreatCasualtyThreshold`/`BreakAwaySeconds`/`Pursues`/`DamageTakenMult`/`Domain`). Wiring it = add the field to `FleetDoctrineDB`, copy it in `TrySetDoctrine`, and read it in the resolver's target-selection. No new system.
- **Positioning:** `GroupPlane.RoleOffset` (bearing/standoff/spread as pure trig) already exists behind `EnableGroupPlane`, but **no per-squadron source feeds it a bearing/standoff** (the S3 role-geometry table is deferred). "Positioning is just doctrine" is right — it's a field-add (a per-squadron bearing/standoff) feeding an existing consumer, not a new system.

### 13e. Build-order impact (unchanged keystone)

C1 (bay generalization, §10) is untouched — still the byte-identical `GroundBayAtb → BayAtb` promotion done first. The 13a-13d refinements land in the carrier slices: **C3** (`CarrierDB`/`CarrierOps`) adds the `DeployableSquadronDB` type + `HostShipId` + `CanRecover`, and wires `UndockAll` into carrier death; **C4** (resolver hook + doctrine) adds the `TargetPriority` carry-through + the per-squadron positioning source, and enforces recover-is-per-class. The carry-class designation (13a) is already inside C1's `BayClass`/`CarryClass` work.

*The resolver sim (`RESOLVER-SIM.md`) models 13a-13d as the TARGET (blue MODEL flag), since every runtime piece above is MISSING/NEW in the engine — the sim shows the locked design so it can be felt before it's built, it does not imply the engine does any of it yet.*
