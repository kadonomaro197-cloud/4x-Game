...
> **⚠ Four DATA rows were stubs that looked live** — they issued and completed but did nothing: fleet `RefuelAction`/`ResupplyAction` (empty `Execute`), `ServeyAnomalyAction` (threw `NotImplementedException`), `ShipLogisticsOrders` (empty). **✅ DE-FANGED (A4, 2026-08-13) then RESOLVED against engine reality (B6, 2026-08-17):** the two real LANDMINES are gone — Refuel/Resupply no longer WEDGE the fleet order lane, and ServeyAnomaly/ShipLogistics no longer THROW in `Clone`. **Outcomes (see `GameEngine/Fleets/CLAUDE.md`):** `ServeyAnomalyAction` is **now BUILT** ("survey the nearest anomaly," commit `b33fe21`); `RefuelAction` stays a deliberate no-op (a ship's burnable fuel IS its fuel cargo, so self-refuel is a no-op — the real external refuel is the client's "Refuel at ⟨colony⟩" button); `ResupplyAction` stays a deliberate no-op ("resupply" = ordnance reload, already the client's "Rearm at a base" button — a true per-launcher magazine is the separate Magazine-Phase-B mechanic); `ShipLogisticsOrders` is an intentional display shim over the freight market (the membership toggle already lives in `SetLogisticsOrder`). Gauge `OrderStubSafetyTests`.
...
| **Survey Anomaly** | fleet | LIVE | **BUILT (B6, 2026-08-17):** "survey the nearest anomaly" — Execute finds the nearest un-surveyed grav anomaly (`JPSurveyableDB`) in the fleet's own system and dispatches the same warp + `JPSurveyOrder` the client's JP-survey menu issues (one verb, both seats); save-safe (lazy target). | `GameEngine/Fleets/ServeyAnomalyAction.cs:33 (real Execute)` |
