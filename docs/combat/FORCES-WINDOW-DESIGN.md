...
> **⚠ Four DATA rows are stubs that look live** — they issue and complete but do nothing: fleet `RefuelAction`/`ResupplyAction` (empty `Execute`), `ServeyAnomalyAction` (throws `NotImplementedException`), `ShipLogisticsOrders` (empty). Finish them before surfacing, or they mislead the player. **✅ DE-FANGED (A4, 2026-08-13, OPERATION BLUEPRINT-TO-STEEL):** the two real LANDMINES are gone — Refuel/Resupply no longer WEDGE the fleet order lane (they complete instead of jamming), and ServeyAnomaly/ShipLogistics no longer THROW in `Clone` (a sim-thread [FATAL]). Their real BEHAVIOR is parked pending developer design decisions (campaign log ADJUDICATION QUEUE); they complete as safe no-ops for now. Gauge `OrderStubSafetyTests`.
