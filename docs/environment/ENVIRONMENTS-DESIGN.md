...
Companion to `docs/ground/GROUND-SURFACE-MAP-DESIGN.md` (the ground map) and `GameEngine/Hazards/CLAUDE.md` (the built space-hazard engine this extends). Read those first.
...
|---|---|
| **`SystemBodyInfoDB.BodyType`** = GasGiant / IceGiant | **NO *SURFACE* environments** (no ground to stand on) — but **NOT inert**: it has its OWN **AERIAL environment class** hosted by floating platforms (see "Gas giants aren't inert" below). The gate skips only the *surface* generator. **Load-bearing gate — surface-only.** Terrestrial / Moon / DwarfPlanet → surface environments allowed. |
...
*"A gas giant may have different environments than a rocky world — on Neptune/Uranus the winds are high as hell with shards of diamond; that's an environment of itself."* Correct — and it sharpens the gate: **the surface gate skips only SURFACE hazards; a gas/ice giant has its OWN environment class**, hosted not by ground troops but by **floating platforms / aerostats** (the gas-giant station host, `docs/economy/OFF-WORLD-INFRASTRUCTURE-DESIGN.md`). Real examples the developer named + more:
...
- **Continent-scale storm systems / the Great-Red-Spot band** ⟶ `MovementDrag` + `SensorJam`.
- **Metallic-hydrogen depths, ammonia/methane cloud decks, lightning megastorms** ⟶ their own `EMDamage`/`Corrosive` flavours.

**How it slots in (no new engine):** it's the SAME `PlanetEnvironmentsDB` typed-effect layer, generated by a **gas-giant branch** of `PlanetEnvironmentFactory` (read composition + wind/pressure), hosted on **atmospheric-band "regions"** instead of surface regions — and the affected units are floating platforms, not ground troops. So it reuses the whole infrastructure; only the *host* (aerial band vs ground region) and the *physics branch* are new. **Deferred — "explore later"**; captured here so the gate is understood as *surface-only*, never "gas giants are boring."
