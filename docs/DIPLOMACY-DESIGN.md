...
**Seated in the leader pipeline (2026-07-07 — the overt twin of the Spymaster).** The FM runs the same six-rung people pipeline as every leader (`docs/GOVERNANCE-AND-DELEGATION-DESIGN.md`), and its org mirrors the espionage Director→station-chief→agents shape line-for-line:
...
2. **Detection-quality is degenerate** — ✅ **DISSOLVED, not fixed.** `SignalQuality` was **CUT** (`docs/DETECTION-DESIGN.md`, 2026-07-07); the hidden-information gradient moved to the **Information Ledger** (agents + decay), which is where it belonged. This keystone no longer exists — the prerequisite is "the Ledger carries the gradient," not "fix the sensor field."
