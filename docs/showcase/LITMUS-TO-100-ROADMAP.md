...
| S1 | **Elite-quality / caliber** — (a) ✅ **DONE (ship half, ~2026-08-02)** — the firepower-caliber enhancer already bites in *live* combat (`ShipCombatValueDB.cs:529`, called by `AutoResolve.cs:124` + `CombatEngagement.cs:1656`) **and** (b) still open: add a **ground** caliber dial (`GroundUnitCaliberAtb`, mirror of `UnitCaliberAtb`) | ⭐ | (a) done · (b) S–M | Elite marines (a veteran cadre that actually hits harder) **and** elite Republic gunnery on the Venator | audit **D-gate-2** (a, now LIVE) + squad ledger **GAP 8** (b) |
