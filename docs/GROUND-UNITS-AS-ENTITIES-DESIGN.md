...
| **P parametric locomotion** | ✅ 5a built — `GroundLocomotionAtb` is a designable, TWEAKABLE component (speed / rough-handling / amphibious dials), not a fixed menu, so a player builds any drive for any environment. Its stats fall out of the unit's component store; `GroundMobility.SpeedMultForUnit` prefers the designed drive over the chassis enum. Transparency rule recorded (`CONVENTIONS.md §16`). Gauge: `GroundLocomotionTests`. **Follow-ups: 5b wire rough-handling into the terrain move-penalty; 5c cost scaling + a base-mod locomotion template + the designer showing the stats; amphibious→pathfinding.** |
