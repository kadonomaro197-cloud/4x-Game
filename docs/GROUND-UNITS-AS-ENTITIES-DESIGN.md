...
| **3 radar-reveal (the payoff)** | ✅ built — `GroundSensorAtb` (a radar component, `Range_km`) + `GroundSensors.RevealFromUnits` reads it off the unit's backing store (`TryGetComponentsByAttribute`, like a ship), translates real km → hex reach → region bands (`Range_km / HexPitchKm`), and reveals via `PlanetRegionsDB.RevealRegion`. Hooked into `GroundForcesProcessor` step 0 (reveals every tick, idempotent). Keyed off the mounted component, not a unit type. Gauge: `GroundSensorsTests` (a designed radar unit reveals its region + neighbours; a no-radar unit reveals nothing). **Follow-up: a base-mod radar component design so it's player-buildable in a stock game.** |
| **4 speed** | ✅ built — `GroundMobility.SpeedMultForUnit` reads the unit's CHASSIS `Locomotion` off its backing store (falls out, no new stat) and divides march time by it at all three move paths (region/hex/global). Foot = ×1.0 baseline; monolithic units default Foot so nothing regresses. **⚠ FLAGGED new numbers (tunable dials): Tracked ×2.0, Walker ×1.5, Hover ×3.0** — move to per-chassis JSON once tuned. Gauge: `GroundMobilityTests`. |
| **P parametric locomotion** | ✅ 5a + 5b built — `GroundLocomotionAtb` is a designable, TWEAKABLE component (speed / rough-handling / amphibious dials), not a fixed menu, so a player builds any drive for any environment. Its stats fall out of the unit's component store; `GroundMobility.SpeedMultForUnit` prefers the designed drive over the chassis enum. **5b: the rough-handling dial is now mechanically real** — `GroundMobility.TerrainMult`/`StepSecondsFor` adjust the terrain move-penalty by the unit's rough handling (a tracked/walker drive EASES the rough penalty, wheels WORSEN it; 0.5 is neutral = old behaviour; open ground never penalised), applied at all four hex/global march-timing sites. So a wheeled scout is fast on the open map but crawls in the mountains, a tracked drive plods everywhere but shrugs off rough — the cost of the dial is felt. Transparency rule recorded (`CONVENTIONS.md §16`). Gauge: `GroundLocomotionTests` + `GroundMobilityTests.TerrainMult_*`. **5c: two base-mod SCOUT PARTS now buildable in a stock game** — `ground-locomotion` (a designable drive: SpeedFactor / RoughHandling / Amphibious sliders, six-point registered) whose **mass/cost SCALES with the dials** (`Mass = 100 × SpeedFactor × (1+RoughHandling) × (1+Amphibious)` → faster/all-terrain/amphibious costs more, the "benefits AND costs apparent" rule made real) and `ground-radar` (a `GroundSensorAtb` with a Range slider, mass scales with range). Both mount `GroundUnit`, both surface their dials as `GuiSelectionMaxMin` sliders in the existing component designer (the "designer showing the stats" payoff, for free). So the developer's core ask is now buildable end-to-end from designed parts: human frame + fast low-support drive + radar = a scout that reveals the map. Gauge: `GroundScoutPartsBaseModTests` (both bind their atb from JSON; a scout assembled from only base-mod parts has speed + radar fall out). **Follow-ups: amphibious→pathfinding (a wet-world route); the client designer showing the live cost delta as you drag (CI-blind).** |
| 5 crew · 6 compose+UI | ⚫ not started |
