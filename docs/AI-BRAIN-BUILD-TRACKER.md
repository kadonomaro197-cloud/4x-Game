...
| **F-C2** | **Internal popular-demands / bloc layer** — the Stellaris-parties demand engine on the morale/legitimacy substrate; an unmet demand pressures legitimacy | building per `docs/GOVERNMENT-AND-POLITICS-DESIGN.md`. **F-C2a built:** `Factions/PoliticalBlocs.cs` — the 5 fixed blocs (Labor/Merchants/Militarists/Liberty/Order) + derived `Loudness(gov, bloc)` (the government dials bias which blocs organise); pure → byte-identical; gauge `PoliticalBlocsTests`. **F-C2b built:** `Factions/DemandEngine.cs` — `SurfaceDemands(moraleFactors, gov, atWar)` emits a `PoliticalDemand` (bloc · ask · pressure) per morale factor bad past a threshold (tax→Merchants, unemployment→Labor, conditions/crowding→ImproveLiving), + a war demand that flips on militarism (hawks→Confront Rival, else→End the War); pressure = badness × bloc loudness; pure → byte-identical; gauge `DemandEngineTests`. **Next: F-C2c enact/refuse → legitimacy.** | `docs/GOVERNMENT-AND-POLITICS-DESIGN.md` (popular-demands) | role #3 Interior Minister; Authoritarianism (full) | 🟡 F-C2a + F-C2b WIRED (awaiting CI); F-C2c next |
