...
> **Progress (2026-07-11):** Phase 0 ✅ · Phase 1 (four first-trait wires) ✅ **CI-green** (`cb1c4b5` / `41abdcb` / `a2a0a33` / `107523e`, all verified `success`) · Phase 2 the **DECISION loop** ✅ committed byte-identically, **CI still verifying** (2.1 `9e5ebd8` → 2.2 `ced4309` → 2.3 `2b9241d` → 2.4a `2842532` → 2.4b `1095045`). **Next: 2.4c** — the first per-objective ORDER emission (the first behaviour-changing slice; hold it until the Phase-2 stack is CI-green), then 2.5 (Ambition/Aggression cadence), 2.6 (eyes→Risk), 2.7 (officer traits). Phases 3–5 + Exploration remain.
...
| # | Slice | Plug point | Gauge | Status |
|---|---|---|---|---|
| 2.1 | `StrategicObjectiveDB` goal slot | new blob on faction | write/read/clone | ✅ **M2-2.1** (committed `9e5ebd8`, CI verifying) — `Factions/StrategicObjectiveDB.cs`: the goal slot (`NeedTier` Survive→Stabilize→Thrive→Ambition + `StrategicObjective` None/Defend/Consolidate/GrowEconomy/AdvanceTech/Expand/Conquer + `TargetFactionId` + `CommittedUntil` for 2.3 hysteresis) with deep Clone. A new blob attached to nothing → byte-identical. Gauge `StrategicObjectiveTests`. |
| 2.2 | Needs-ladder read (Survive→Stabilize→Thrive→Ambition) from planet gauges + rollups | new logic over the built gauges | tier flips as morale/money/war-standing move | ✅ **M2-2.2** (committed `ced4309`, CI verifying) — `Factions/NeedsLadder.cs`: pure `AssessTier(atWar, own/enemy strength, morale, legitimacy, balance, inRebellion)` settles the lowest unmet tier (rebellion/collapse/losing-war → Survive; any war or internal trouble → Stabilize; dominant-secure-rich → Ambition; else Thrive), + an entity gatherer over `FactionRollup` + `DiplomacyDB` + `RebellionDB`. Read-only → byte-identical. Gauge `NeedsLadderTests` (tier boundaries + a fresh start faction reads Thrive). |
| 2.3 | Transition engine (commit + hysteresis) | new pure logic | no thrash under noise; re-plans on real change | ✅ **M2-2.3** (committed `2b9241d`, CI verifying) — `Factions/ObjectiveTransition.cs`: `ShouldReplan(currentTier, committedUntil, proposedTier, now)` (a more-urgent tier preempts; else re-plan only once the commitment expires) + `Advance(objective, …)` stamps the new plan + re-arms `CommittedUntil`. Anti-thrash: holds a committed objective under noise, drops it for an emergency. Pure logic; the only mutation is a `StrategicObjectiveDB` nothing attaches yet → byte-identical. Gauge `ObjectiveTransitionTests`. |
| 2.4 | **Fill `NPCDecisionProcessor.Tick`** — score objective → emit orders | `NPCDecisionProcessor.cs:79-83` | an NPC colonizes/builds/attacks per its objective | 🟡 **2.4a + 2.4b done; 2.4c (order emission) next.** **M2-2.4a** (committed `2842532`, CI verifying) — `Factions/ObjectiveSelector.cs`: pure `SelectObjective(tier, doctrine, personality)` (Survive→Defend, Stabilize→Consolidate, Thrive→strongest growth axis, Ambition→Conquer for a Military-led/Aggressive faction else press the axis). Gauge `ObjectiveSelectorTests`. **M2-2.4b** (committed `1095045`, CI verifying) — `NPCDecisionProcessor.UpdateStrategicObjective`: the Tick now assesses the tier (`NeedsLadder`) → selects (`ObjectiveSelector`) → commits through the hysteresis engine (`ObjectiveTransition`) → stores a `StrategicObjectiveDB` on the NPC faction (created on first run). The **decision loop is live** (the ~4-year-old Tick stub is filled); byte-identical because the plan is stored but not yet acted on (order emission is 2.4c). Uses GAME time (deterministic). Gauge `NPCObjectiveTickTests` (an NPC settles Thrive/GrowEconomy, a player faction settles nothing, the plan then holds). **2.4c (next):** per-objective order emission (GrowEconomy→queue industry, Expand→survey/colonize, Conquer→build+attack). |
