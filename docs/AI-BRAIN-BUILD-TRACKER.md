...
> **Progress (2026-07-11):** Phase 0 ✅ · Phase 1 (four first-trait wires) ✅ **CI-green** (`cb1c4b5` / `41abdcb` / `a2a0a33` / `107523e`, all verified `success`) · Phase 2 the **DECISION loop** ✅ **CI-green** end-to-end (2.1 `9e5ebd8` → 2.2 `ced4309` → 2.3 `2b9241d` → 2.4a `2842532` → 2.4b the filled Tick `1095045`, all verified `success`). The **first ACT** is in and green: **2.4c** (CI-green `b2fb85b`) — a GrowEconomy objective queues an industry job behind the default-OFF `EnableOrderEmission` gate (byte-identical off). **Decision fork (2026-07-11):** the 4-agent survey (row 2.8, `docs/AI-MEANS-ENDS-PLANNER-DESIGN.md`) showed the reactive **Conquer** emitter is premature (no target selection, no reachability, no fuel-readiness — it would fire orders that can't execute), so 2.4d should NOT blindly follow. Honest next options: the cheap `AutoAddSubJobs` fix (make the existing GrowEconomy emitter material-aware), 2.6 (eyes→Risk, self-contained), or start the planner (P-0). Developer's call. Phases 3–5 + Exploration remain.
...
| 2.6 | `ThreatEstimate` (the eyes) → Risk trait | new logic over `GetSensorContacts` (`EntityManager.cs:638`) | engages at parity (Risk 1) vs demands 2× (Risk 0) | ✅ **M2-2.6** (committed, CI verifying) — `Factions/CombatRisk.cs`: `RequiredStrengthRatio(risk)` scales the own-vs-enemy bar from 2× (Risk 0, overwhelming) to 1× (Risk 1, parity); `WouldEngage(own, enemy, risk)` decides, + an entity convenience that reads own strength (`FactionRollup.MilitaryStrength`) vs the fog-limited/intel-sharpened enemy estimate (`ThreatAssessment.DetectedStrengthOf` / `IntelAssessment`) and the faction's Risk trait. The eyes now decide willingness to fight. Pure/read-only, nothing calls it yet → byte-identical. Gauge `CombatRiskTests`. |
...
>
> **Planner build progress:** **P0-a ✅ `FactionState` snapshot** (`Factions/FactionState.cs` — the gather-once "what I have"; faction gauges off `FactionRollup` + a `ColonyState` slice per colony; no consumer yet → byte-identical; gauge `FactionStateTests`) committed, CI verifying. **Next: P0-b** — `PlannerAction` + `IObjectiveResolver` + registry + `FeasibilityOracle` skeleton + `GrowEconomyResolver` Rung C (queue-on-free-line through `AutoAddSubJobs`) + rewire `EmitOrders`.
