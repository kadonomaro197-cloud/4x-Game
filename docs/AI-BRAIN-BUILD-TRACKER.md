...
>
> **Planner build progress:** **P0-a ✅ CI-green `5d5581c`** — `FactionState` snapshot (the gather-once "what I have"; gauge `FactionStateTests`). **P0-b ✅ committed (CI verifying)** — `PlannerAction`/`IObjectiveResolver`+registry/`FeasibilityOracle` (capacity+tech) / `GrowEconomyResolver` Rung C; `EmitOrders` rewired to snapshot→registry→resolve→execute (REPLACE the GrowEconomy arm — `TryQueueEconomyJob` superseded, retained only for its unit gauge). **The build now routes through `AutoAddSubJobs` → the economy brain is material-aware for the first time** (the fix for 2.4c's blind-build shallowness). Byte-identical off; gauge `GrowEconomyResolverTests` (+ `NPCOrderEmissionTests` still green). **Next: P1-a** — `StalledJobs()`/`MineralShortfalls()` sensor, then the mineral-floor rungs (mine/logistics/survey).
