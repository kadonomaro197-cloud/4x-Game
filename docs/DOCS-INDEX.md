...
|-----|---------|--------|-------|-------|
| `docs/SPACE-STATIONS-DESIGN.md` | Station-as-universal-off-world-host (parallel to colony) | 🔒 | ✅ | v1 + grave rung (Slice B) + front door (Slice A) + cost/upkeep curve (Slice C: StationUpkeepProcessor, placeholder gradient) delivered; efficiency-catch claim corrected. Remaining: auto-resolve targeting, invasion, real tuning |
