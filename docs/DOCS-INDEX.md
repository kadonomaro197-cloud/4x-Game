...
| `docs/GROUND-COMBAT-MAP-DESIGN.md` | The planet map: nested region-graph (strategic ring) + hex (tactical); features + exploration | 🔒 | 🚧 | **Design-locked; slice 1 (region layer) building.** `PlanetRegionsDB` (4-slice ring, persistent) + generation (logical features, authored-known/procedural-unknown). Next: build-at-a-region, planet view, survey-reveal, ground units |
