...
| `docs/GROUND-COMBAT-MAP-DESIGN.md` | The planet map: nested region-graph (strategic ring) + hex (tactical); features + exploration | 🔒 | 🚧 | **Design-locked; slices 1–4 built.** `PlanetRegionsDB` (4-slice ring, persistent) + generation ✅; build-at-a-region (`PlaceInstallationInRegionOrder`) ✅; `PlanetViewWindow` (flat 3-region view) ✅ compile-checked (runtime G3); **survey-reveal (`GeoSurveyProcessor`→`RevealAll`, 2026-07-03)** ✅ CI-gauged. Next: ground units (slice 5) |
