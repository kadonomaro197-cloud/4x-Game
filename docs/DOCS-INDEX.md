...
| `docs/HEX-GROUND-AND-ORDERS-DESIGN.md` | **NEW 2026-07-04.** The next evolution: planet regions made of **HEXes** (Planet→Region→Hex, ~1800 on Earth, lazy-gen) so formations move hex-by-hex (London→Paris) with terrain/hazards on hexes; + a rich Aurora-style **order catalog** (~60) for fleets AND formations. Locked: Planet→Region→Hex, Operational density, coords-only, LAZY hex gen. Two-track roadmap H1–H5 (ground hex) + O1–O3 (orders). | 🔒 | 🚧 | **Design-locked; H1 (hex data layer) + H2 (hex movement/pathfinding) built** (`GroundHex` + `Region.Hexes` + `PlanetHexFactory` lazy gen; `HexPathfinder` A\* terrain-weighted **crossing region borders at edge gates** + `HexMovement` **UNIT-dependent cost** (`MovementDomain` Land/Water/Air — the developer's "cost depends on the unit" call: land bogs/blocked by rough+ocean, air flies flat, water on water) + `GroundUnit` hex position/`Domain`/`Path` + `OrderMove`/`OrderFormationMove` hex-target overloads → `GroundForcesProcessor` hex-walk (`AdvanceHexPath`) — gauges `PlanetRegionsTests.Hex*` + `GroundForcesTests.HexMovement_*`/`FindPath_*`/`OrderFormationMove_ToHex_*`). Next: H3 combat-on-hex, client hex render (H4); O1 order-catalog framework |
