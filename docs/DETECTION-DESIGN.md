...
**Slice 3 build list (the wires, grounded):** (1) ✅ **DONE** — `SensorProfileDB.ActivityMultiplier` read in the detection math (`SensorTools.AttenuatedForDistance` + `AttenuatedForDistanceList`), scaling EMITTED only; default 1.0 so no behavior change until a driver sets it. Gauged by `SensorEmconTests`. (2) a small `EMCONProcessor` / per-scan recompute that sets it from reactor-load + thrust-state + firing × the posture; (3) ✅ **DONE** — `FleetEmconDB` + `FleetEmcon.SetPosture` (**Full / Cruise / Silent**, mirrors doctrine; direct call, pushes the scale onto member ships). Gauged by `SensorEmconTests`. *(The processor that combines this with reactor/thrust/firing is item (2), next.)* (4) `PowerThrottlePercent` on `EnergyGenAbilityDB` so Silent caps output; (5) a runtime burn/firing state so thrust + the first shot spike you; (6) active-ping adds to your own `EmittedEMSpectra`. Each its own gauged slice.
...
3. **Heat/EMCON as COMPONENTS + a posture ORDER.** The *capability* (signature level, run-silent, active-ping) lives on **components** (researched/built/installed — §6 hands you that chain free); the *choice* is a fleet order (`FleetEmconDB`, like doctrine). Make the signature **dynamic**. CI-gauge: a cold ship is detected at shorter range than a hot one, AND a fleet with a better EMCON component runs quieter — capability comes from the components, *not a free flag*. *(Engine-side.)* **Built in sub-wires (3a–3f, list below); each its own gauged slice:**
   ...
   - **3c–3f — make the signature respond to ACTIVITY (next):** a small `EMCONProcessor` that sets `ActivityMultiplier = posture × reactor-load × thrust × firing` each tick (reactor `Load` is read-ready at `EnergyGenAbilityDB.cs:38`; weapons-fired flag at `GenericFiringWeaponsProcessor` `ShotsFiredThisTick[]`; **the one real gap** is thrust has no runtime "drive-lit" flag — add `IsBurning`); `PowerThrottlePercent` so Silent caps reactor output; active-ping adds to your own emitted spectra (you can't shoot/ping quietly). Each its own gauged slice. *Switch cooldown (reactor thermal inertia) = flagged candidate.*
