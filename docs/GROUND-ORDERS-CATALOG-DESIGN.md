...
- **Ground formations** — driven today by direct `GroundForces.*` calls; O1a already gave them a real `GroundFormation.Orders` queue. The remaining ground/hex movement, ROE, and surface-map work is tracked in `GROUND-SURFACE-MAP-DESIGN.md`.
