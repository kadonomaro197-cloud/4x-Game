...
- **H3 — combat by RANGE on hexes. ✅ RANGE BUILT 2026-07-04 (the developer's model).** Instead of a flat "same/adjacent" rule, each unit has a **strike `Range` in hexes** (a design stat — Infantry/Armor 1, Artillery 3 default, moddable): an attacker only damages enemies within its reach, so a longer-ranged unit hits a shorter-ranged one **as it closes, taking no fire back** until the enemy reaches its own range — the *"clone trooper vs zerg swarm has the advantage until they reach him"* standoff, the ground echo of the space first-strike (`GroundForcesProcessor.ResolveRegionCombat`, directed per-target fire). **Co-located units (same hex) are byte-identical to the pre-hex resolver**, so the whole existing combat suite stays green. Range lives in **hexes** (the developer's call — at operational scale a hex is continental and its real size differs by body); `GroundRangeTools.RealReachKm` surfaces the *"1 hex ≠ same distance on Earth vs Io"* variance as a **readout**. Gauge: `GroundForcesTests.RangeCombat_OutRangerHitsCloserUnitFirst_CloneVsZerg` + `RangeDefaults_*` + `RealReach_*`.
  - **Still open in H3 (follow-ons, flagged):** **(a) H3b — terrain/fortification read from the DEFENDER's own HEX** (today combat terrain is still the region's dominant feature; moving it to per-hex needs the affected terrain/fort tests updated to set hex terrain, so it's its own slice). **(b) per-hex HAZARDS** (move `PlanetEnvironmentsDB` from region-indexed to per-hex). **(c) commander ENGAGEMENT RULES / ROE** (the developer's *"detailed engagement rules like they do in space"*) — auto hold-at-range vs close-to-melee, so the range advantage is used without micro; a sizeable design in its own right (the space ROE is a whole doc), tracked as the next ground-combat design piece. H3 delivers the range MECHANIC; the player uses it today by MOVING units (H2) to hold or close.
