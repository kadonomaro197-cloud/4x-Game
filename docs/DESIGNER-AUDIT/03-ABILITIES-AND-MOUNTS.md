...
> Companion to `01-DESIGNER-UIS.md` (mount legality) and `02-DESIGNABLE-TYPES.md` (design classes). All file:line references are as of 2026-07-08. Related design docs already circling this problem: `docs/UNIVERSAL-ASSEMBLY-DESIGN.md`, `docs/WEAPON-UNIFICATION-DESIGN.md`, `docs/GROUND-UNITS-AS-ENTITIES-DESIGN.md`.
...
| **BeamFireControlAtbDB** | `Weapons/WeaponFireControl/BeamFireControlAtbDB.cs:9` | Fire-control / targeting | `Weapons/WeaponFireControl/FireControlAbilityDB.cs:41` **(commented out)** | Ship (latent) | — |
...
- **Status — partially unified already:** `GroundCombat/WeaponSupply.cs:41-95` classifies the **space** weapon atbs (laser→Energy, railgun→Both, flak→Ammo, etc.) for the ground power/ammo gate, so a ground unit can *carry* a space weapon and be supply-gated. So today a ground unit can mount **either** the old `GroundWeaponAtb` **or** a real space weapon — the classic duplicate-mid-refactor. `docs/WEAPON-UNIFICATION-DESIGN.md` tracks the planned P4 merge that retires `GroundWeaponAtb`.
...
| Fire control | ◐ (code commented) | ✗ | ✗ | ✗ | BeamFireControlAtbDB |
