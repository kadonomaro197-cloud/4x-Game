...
Pulsar **already has** people: `GameEngine/People/` (`CommanderFactory.CreateShipCaptain`, `CreateScientist`, ranks, commission dates — seen in `ColonyFactory`/`ShipFactory`), and a `CommanderWindow`. **The NAVAL side of the combat-bonus loop is now wired** (verified in code, 2026-07-13): a fleet's flagship commander scales the whole fleet's firepower and toughness in the auto-resolver — `CombatEngagement.FleetCommanderMult` (`Combat/CombatEngagement.cs:1575-1587`) walks flagship → `ShipInfoDB.CommanderID` → that commander's `BonusesDB` → `CommanderBonuses.CombatMultiplier`, and `CollectCombatShips` folds it into firepower/toughness (`:1545-1547`). Returns 1.0 (no effect) if any link is missing, so it's byte-identical until a commander actually carries a bonus. The **GROUND** side is the piece still open. See `docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md` for the Pulsar-side officer-as-delegate design this maps onto.
...
| Naval combat bonus (flagship → fleet) | **wired** (`CombatEngagement.FleetCommanderMult`) | the naval half of the loop is done — a flagship officer's competence scales fleet firepower/toughness |
| Ground combat bonuses (GCO/GCD/…) | **NOT consumed yet — the open build item** | `GroundForcesProcessor.ResolveRegionCombat` routes terrain / fortification / stance mults through the shared `CombatKernel` but reads no ground-commander skills. `CommanderTypes.Ground` exists (`Enums.cs`); the build work is having the ground resolver read the assigned commander's bonuses and fold them into the to-hit / supply / garrison math (`GROUND-COMBAT.md` §4, §7) |
| Academy produces officers | **research academy reachable; naval academy NOT** | The scientist-graduating `ResearchAcademyProcessor` + its `ResearchAcademyAtb` are wired and the `research-academy` template sits in Earth's start colony (`earth.json` StartingItems + ComponentDesigns), so it's buildable from turn one. The `NavalAcademyProcessor` + `NavalAcademyAtb` code exists too, BUT the `naval-academy` template is in **no** colony's StartingItems and its JSON `AttributeType` uses a legacy FQN (`Pulsar4X.Atb.NavalAcademyAtb`; the class is actually `Pulsar4X.People.NavalAcademyAtb`) that won't resolve — so the naval academy is unreachable in practice. Fixing that FQN + adding the template to a colony is the officer-supply build item |
| Command capacity scaling | verify | apply the "over capacity → reduced bonus" rule |

**Takeaway:** the naval combat-bonus loop is built and wired (code-level — runtime unverified, CI can't run the client); ground combat still must *consume* commander bonuses to reach Aurora-depth. Officers already exist in `People/`; the remaining work is reading their ground skills inside the ground-combat math, and making a naval-officer supply (the research academy already graduates scientists). `CONVENTIONS.md` §6, §9 (resolve the commander via ID).
