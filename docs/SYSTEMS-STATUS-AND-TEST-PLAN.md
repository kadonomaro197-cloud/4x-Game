...
|-------|-------|--------------|---------------|
| **Spawned ships invisible / "fleets not working"** | `DevToolsWindow.cs` · `Ships/ShipFactory.cs` | **Two layered causes, both found 2026-06-24.** (1) `ShipFactory.CreateShip` orbits a ship at **2× the planet's radius** → sub-pixel on the planet icon at system zoom (zoom in / Fleet window to see it); the "previous name stayed" wart was the post-spawn `HardRefresh()` resetting the dropdown. (2) **A bare `CreateShip` puts the ship in NO fleet**, so it never shows in the Fleet window or takes orders — and the *same gap one level up* is why the **New Game start builds no fleet at all** (next row). | ✅ **Engine proven** (`ShipSpawnTests`, CI-green). ✅ **`DevToolsWindow` fixed:** spawned ships now join a controllable **"Spawned Ships" fleet** (`FleetFactory.Create` + `AddChild`) so they appear in the Fleet window; new **"Dump State"** button logs ship/fleet counts; dropdown-reset removed; logging flushed (`DevLog`). ⏳ **Confirm live**: spawn → it's in the Fleet window. |
