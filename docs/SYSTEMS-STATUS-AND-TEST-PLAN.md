...
|-------|-------|--------------|---------------|
| **Spawned ships invisible / "fleets not working"** | `DevToolsWindow.cs` · `Ships/ShipFactory.cs` | **Two layered causes, both found 2026-06-24.** (1) `ShipFactory.CreateShip` orbits a ship at **2× the planet's radius** → sub-pixel on the planet icon at system zoom (zoom in / Fleet window to see it); the "previous name stayed" wart was the post-spawn `HardRefresh()` resetting the dropdown. (2) **A bare `CreateShip` puts the ship in NO fleet**, so it never shows in the Fleet window or takes orders — and the *same gap one level up* is why the **New Game start builds no fleet at all** (next row). | ✅ **Engine proven** (`ShipSpawnTests`, CI-green). ✅ **`DevToolsWindow`:** new **"Dump State"** button logs ship/fleet counts; dropdown-reset removed; logging flushed (`DevLog`). ⚠️ **Auto-joining the spawned ship to a fleet from the client was reverted — it broke the build:** `FleetDB.SetParent/AddChild/FlagShipID` are **engine-internal**, not callable from the client (CI can't catch client build breaks — only the dev's local build does). Client fleet changes must go through the **order system** (`FleetOrder.AssignShip` → `OrderHandler.HandleOrder`, as `FleetWindow` does), or be done engine-side. So **the real "controllable ships" fix is the next row** (build the fleet in the start path, where the API is accessible). |
| **New Game builds a colony but NO starting fleet** | `Menus/NewGameMenu.cs:CreateGameCore` → `Colonies/ColonyFactory.cs:CreateFromBlueprint` | **Root cause of "no ships around Earth / empty fleet view" (2026-06-24).** `CreateFromBlueprint` only builds ships from fleets **nested inside the colony blueprint** (`colonyBlueprint.Fleets`). But the start data defines its fleets at the **faction level** (`GameData/basemod/ScenarioFiles/uef.json` top-level `"fleets"`: gunship, freighter, Luna surveyor), which only the *scenario loader* (`DefaultStartFactory.LoadFromJson` — **not used by the wizard**) reads. So the intended starting fleet is **defined but never built**; `FactionFactory.CreateBasicFaction` makes none either. | 🔴 **Open — real bug.** Fix options: (a) nest the fleets under the colony blueprint the wizard loads, or (b) have `CreateGameCore` build the faction's fleets from the scenario data. **Gauge: the new DevTools "Dump State" reads 0 fleets on a fresh New Game.** Confirm, then fix. |
