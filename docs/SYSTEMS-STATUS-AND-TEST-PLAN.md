...
|-------|-------|--------------|---------------|
| **"Spawn Ship" looked like it did nothing** | `DevToolsWindow.cs` (Spawn Ship) | **Live test 2026-06-24: the spawn actually WORKS** — the status line said "Spawned <name>" (no error) and the engine path is CI-proven (`ShipSpawnTests`). The ship was invisible because `ShipFactory.CreateShip` puts it in orbit at **2× the planet's radius** (~12,000 km), which at the zoomed-out system view is sub-pixel **right on top of the planet icon**. Standard "ships orbit planets" behaviour — you must zoom into the body to see it. The "previous name stayed on the 2nd try" was a separate wart: the post-spawn `HardRefresh()` reset the Design dropdown to index 0, so repeat-clicks re-spawned design #1. | ✅ **Engine proven** (`ShipSpawnTests`, CI-green). ✅ **UX fixed** (`DevToolsWindow`): post-spawn status now reports the ship id + **system ship count** + "zoom into <body> / open Fleet window to see it"; the dropdown-reset is removed (no more stuck name); and DevTools logging now **flushes** (`DevLog`) so it lands in `console_output.txt` mid-run instead of only on game exit. ⏳ **Confirm live**: spawn, then zoom into the parent body / open the Fleet window — the ship is there. |
