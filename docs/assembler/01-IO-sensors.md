...
Checked the census against `sensorsderived.html` source.
- **Confirmed:** init `let job='listen'` (`:308`); `STOCK.listen={d1:0,d2:68,d3:50,d4:35}` (`:468`); formulas `size=1.25+d1*0.2875` (`:362`), `peak=Math.pow(10,0.5+d2*0.0335)` (`:363`), `bw=1+d3*4.99` (`:364`), `mass=90+0.01*size*size` (`:369`). Sliders d1=0/d2=68/d3=50/d4=35 (`:182-185`), single forced-choice chip row.
- **Gate bug confirmed exactly** (the interconnection-map crack C7, this door's headline): `gateAsWritten(rec,sig) = Math.max(rec.min,sig.min) < Math.max(sig.min,sig.max)` (`:316`) — the RHS `Math.max(sig.min,sig.max)` reduces to `sig.max`, so **the receiver's upper edge `rec.max` is never consulted**; the correct test `gateCorrect = … < Math.min(rec.max,sig.max)` (`:317`). A visible-light receiver admits an infrared emitter. Verbatim as recorded.
- **No corrections.** The census correctly distinguishes the four *sliders* (antenna/band/bandwidth/scan for Listen; per-job for the rest) from the *phantom* dials described in prose (Resolution, Size-vs-Range, Size-vs-TrackingSpeed) that are NOT controls in this HTML. For the Assembler, Sensors is the **detection-footprint** side (an emergent readout — what an assembled entity is detected at depends on the observer) and the fire-control track-range that gates weapons; `IntelDirectorateAtb` correctly belongs to Command, not here.
