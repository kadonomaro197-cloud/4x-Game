...
|---|---|---|
| **ammunition / ordnance charge** | Weapons (projectile/guided "eat ammo"), Logistical (magazine) | No material named `ammo` — **still a real gap** (the resolver's ammo pool is mass-based; no `ammo` material backs it). ~~`ordnance.json` charges three undefined minerals — `gallicite`, `duranium`, `mercassium`~~ **✅ FIXED 2026-08-02:** the ordnance recipes were redirected to defined materials (`electronics`+`aluminium` / `stainless-steel`+`aluminium`); a grep of GameData now finds **zero** references to those three, so nothing faults. (Was "the single most broken chain in the game" — no longer. See Part 5 NEW-1.) |
