...
| **Fleets** (`FleetOrderProcessor`) | Ships, movement, orders, combat. Client fleet changes go via `FleetOrder.*` + `OrderHandler`; `FleetDB` mutators are engine-internal. |
...
| **Factions & NPC AI** (`NPCDecisionProcessor`) | Economy (could auto-queue jobs), industry, diplomacy, combat doctrine, People/seats, sensors (enemy-strength read), exploration-content. |
| ↳ *AI build prereqs* | Seat durability (`People/AdminSpaceProcessor.cs:36-46`); two NO-SOCKETs to build first — fleet situational read; crisis seed. |
