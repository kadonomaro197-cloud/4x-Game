...
> **📌 Historical record — the plan below was EXECUTED (2026-07-13).** This doc names the *pre-reorg* doc set, so some file paths it lists (the merged-away sources, and pre-foldering `docs/*.md` paths) no longer resolve — that's expected; they document what was. The reorg landed: ~60 stale claims flipped, 12→6 AI docs + ground/combat/economy merges, `docs/` foldered into subject subfolders. The permanent method lives in `docs/DOCS-AUDIT-PROCESS.md`; live status is `docs/DOCS-INDEX.md`.
...
1. **`docs/aurora/DIPLOMACY-AND-INTEL.md` and `docs/aurora/GROUND-COMBAT.md` still tell a reader diplomacy, intel, and ground combat don't exist in Pulsar.** All three are built and wired. Highest rebuild risk. Add a pointer to `docs/society/DIPLOMACY-DESIGN.md`, `docs/society/ESPIONAGE-AND-INTELLIGENCE-DESIGN.md`, and `GroundCombat/CLAUDE.md`.
...
3. **`docs/society/ESPIONAGE-AND-INTELLIGENCE-DESIGN.md` and `DOCS-INDEX.md` mark espionage NOT built.** False — built cradle-to-grave (`IntelDirectorateDB`/`Atb` registered in `installations.json`, `componentDesigns.json`, `earth.json`; `IntelDirectorateProcessor` recruits agents; `Espionage.TaskAgent` + `EspionageProcessor` resolve ops; client `IntelligenceWindow`; tests exist). Inert only because default flags are off.
...
5. **`docs/combat/COMBAT-DESIGN.md`'s bottom "What Already Exists" table (stamped 2026-06-21)** lists Doctrine, Retreat, Auto-resolution, EMCON, Environmental hazards, and Ground-combat-interface as "Not started" — all six are built and wired.
...
- **`docs/archive/PLAN.md`** — self-declared superseded 2026-06-26; its state assessment badly understates combat/ground/infrastructure and would steer a reader back onto parked per-pixel-combat paths.
- **`docs/archive/AURORA-GAP-ANALYSIS.md`** — a 2026-06-21 snapshot whose four "Critical Gaps" and "ground-combat 100% gap" are all contradicted by shipped code. Keep as a baseline record with a HISTORICAL banner.
- **`docs/archive/HAZARD-DISCOVERY-RESISTANCE-RESEARCH-DESIGN.md`** — its "no code yet" premise is flatly contradicted; salvage the locked decisions into `Hazards/CLAUDE.md` first.
...
| VISION | `docs/archive/PLAN.md` | ARCHIVE | OK | Move to docs/archive/PLAN-original.md (or docs/superseded/) and reduce the in-repo footpri |
...
| SOCIETY | `docs/society/MORALE-AND-POPULATION-DESIGN.md` | STALE-FIX | OK | Add a top banner stamping M1-M5a as BUILT/wired (cite ColonyMoraleDB, ColonyManpowerDB, Co |
| DIPLO | `docs/society/DIPLOMACY-DESIGN.md` | KEEP-TRIM | VERBOSE | Keep as the canonical external-politics design. Reconcile the body with the 2026-07-07 ban |
| DIPLO | `docs/society/ESPIONAGE-AND-INTELLIGENCE-DESIGN.md` | STALE-FIX | VERBOSE | Keep in place as the espionage design of record, but add an 'AS-BUILT (2026-07)' banner at |
...
| DETECT | `docs/combat/INFORMATION-DELTA-DESIGN.md` | KEEP-TRIM | LEAN | Keep as the canonical A-vs-B readout principle doc. Trim or update the 'base laser two-zon |
| ENV | `docs/environment/ENVIRONMENTS-DESIGN.md` | KEEP-TRIM | VERBOSE | Keep as the environments design+status doc. Immediate fix: replace the top banner with an  |
| ENV | `docs/archive/HAZARD-DISCOVERY-RESISTANCE-RESEARCH-DESIGN.md` | ARCHIVE | VERBOSE | Move to docs/archive/ (or mark SUPERSEDED in DOCS-INDEX) with a one-line pointer to GameEn |
| ENV | `docs/environment/STELLAR-ENVIRONMENTS-CATALOG.md` | KEEP | OK | Keep as-is but add a one-line staleness note near conclusion #3 and the binary row that th |
| AURORA | `docs/archive/AURORA-GAP-ANALYSIS.md` | ARCHIVE | OK | Move to a docs/archive/ (or docs/history/) folder and add a one-line 'SUPERSEDED — see SYS |
...
| AURORA | `docs/aurora/SENSORS-AND-DETECTION.md` | KEEP | LEAN | Keep in place under docs/aurora/ as the reference-side twin of docs/combat/DETECTION-DESIGN.md. A |
