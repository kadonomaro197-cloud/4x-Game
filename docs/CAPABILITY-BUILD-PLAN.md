...
**As of 2026-07-12.** The single, ordered implementation plan that **consolidates `COMPONENT-DESIGNER-DIAL-LEDGER.md` (what's built) + `AI-CAPABILITY-CATALOG.md` (what the AI can choose)** into one step-by-step build, sliced as few ways as possible. Produced from a 7-agent deep code audit (build→wire→test, file:line verified). This is the doc we build from.
...
- **The capability catalog** = jobs (a)+(c): four whole needs (**INFLUENCE, DIPLOMACY-as-buildable, ESPIONAGE-as-buildable, GOVERNANCE-policy**) have **no buildable at all** (root: `Factions/` has zero component attributes), and nothing yet reads the catalog to know its options.
...
The dial ledger was stale in two rows (both now flipped in `COMPONENT-DESIGNER-DIAL-LEDGER.md`):
...
| 4.6 | **Spy network (espionage buildable)** | (a) Attach `InformationLedgerDB` to factions in `FactionFactory` (all paths — its own sub-slice, BI). (b) `IntelNetworkAtb` (network strength → `CovertRisk` agentSkill + agent slots) + six-point JSON + `CovertOperationsProcessor` (spends slots to run `CovertActionCatalog` through `CovertRisk`, writes the ledger) behind `EnableCovertOps=false`. `CovertOperationsTests`. | BI (flag off + neutral blob) |
