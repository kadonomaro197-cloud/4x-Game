...
- **Reconciles the progression ladder** (`docs/OFF-WORLD-INFRASTRUCTURE-DESIGN.md` "Outpost = the only automatable tier"): automation is **always** available via a governor; what scales with size is *how much potential a poor/no governor wastes*. Outpost barely needs one; a Capitol with a bad governor underperforms badly. Same system, scaling stakes.
