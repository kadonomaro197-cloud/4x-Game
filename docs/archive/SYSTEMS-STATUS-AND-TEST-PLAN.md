...
| **Morale** (the population-tank valve) | `PopulationProcessor` (M1) reads `ColonyMoraleDB` | ✅ DONE (M1) | `MoraleTests` — pure morale math + migration sign/bounds + the real start born neutral | population (migration), Galaxy (conditions = colony cost), capacity (overcrowding); **roadmap** jobs/tax/power/food/**governor** (`docs/society/MORALE-AND-POPULATION-DESIGN.md`) |
...
| ↳ *AI design entry point* | see `docs/ai/AI-BRAIN-BUILD-TRACKER.md` (hub; §4 socket verification) → `docs/ai/AI-PERSONALITY-IMPLEMENTATION-SPEC.md` (code-ready) + 6 more `AI-*.md` | 📝 DESIGN COMPLETE, 0% BUILT | n/a (design) | **Prereqs before any build:** ⚠ fix seat durability (`People/AdminSpaceProcessor.cs:36-46`, "paramount"); 2 NO-SOCKETs (fleet situational read; crisis seed) |
...
| **Diplomacy** | — | ⚫ ABSENT | design only (`docs/society/DIPLOMACY-DESIGN.md`) | factions, IFF, trade, logistics |
