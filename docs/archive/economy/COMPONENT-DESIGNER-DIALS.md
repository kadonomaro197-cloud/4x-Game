> # 🗄 ARCHIVED 2026-08-03 — SUPERSEDED DESIGNER DIAL SPEC (wiring dossiers retained as reference)
>
> **The DESIGN model here — the old "37 doors, dial-by-dial" spec (every door marked 🔒) — is SUPERSEDED** by the canonical set: the **12 door HTMLs** (`docs/Actual HTMLs Of designers/`) + **`docs/economy/DESIGNER-NORTH-STAR.md`**, which collapses the 37 doors / 41 dial-groups to a few choices + sliders per category. Do NOT infer the current designer from the 37-door model here.
>
> **Retained as a HISTORICAL engine-wiring reference:** this doc's `⚙ 1`–`⚙ 11` WIRING DOSSIERS and its `§0d`/`§0d′` modellability & free-dial cost audits document how each dial wired to engine code (file:line) — that mapping is factual and the engine `CLAUDE.md` pointers resolve here. Use it to trace a wire; use the canonical set to design. Locked developer decisions live in `docs/COMBAT-DESIGNER-GROUND-TRUTH-2026-07-28.md`.

# The Component Designer — Dial Specifications (living doc)

**As of:** 2026-07-09 · the lowest-level spec: for each of the ~37 doors (`docs/archive/economy/COMPONENT-DESIGNER-CATEGORIES.md`), the actual **dials** a player turns, the **derived stats** they produce, and the **dependencies** they require. Locked door-by-door; this doc is updated every time a decision is locked in.
...
> **⚠ 🔒 LOCKED = DESIGN decided, NOT code built.** This is a **design** spec: every door below reads 🔒, meaning the decision is made — it does **not** mean the dial is wired. At the *code* level only a fraction of these dials actually move a number a resolver reads (a 7-agent audit, 2026-07-12, found ~25 of 37 doors have *any* wired dial, and the "built" ones are thin — Survey 3/10, Transfer 3/10, Hull 1/4). **For what is actually WIRED, and the exact wire+test to build each unwired dial, read the companion `docs/archive/economy/COMPONENT-DESIGNER-DIAL-LEDGER.md`** — the honest code-state tracker. Do not infer build-state from 🔒 here.

> **⚙ WIRING-READY (2026-07-09).** Every category now carries a self-contained **⚙ Wiring Dossier** at the end of its section (`⚙ 1`…`⚙ 11`). **To wire a category, that dossier is the only reference you need** — it consolidates, verified against the live engine (file:line): the complete dial → derived-stat → engine-wire table (with the essence-extension dials folded in as first-class rows), the resolver/system insertion points (from `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1`, adapted), the prerequisite fixes & dead stubs, the design essence captured inline (so the five merged strategic docs aren't needed), the §0g Reachable/Mirrored/Observable stamp, the cross-category shared state, and the holes the category owns. The cross-cutting appendices (📎 Main-Merge Delta, 🧭 Essence Extensions) and `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1` remain the *origin/rationale*; their per-category content is now **distributed into the dossiers**, which are the wiring authority.
...
Tiers overlap by ~an order of magnitude; **tech multiplies the achievable max** (multiplicative growth, `docs/economy/UNIVERSAL-ASSEMBLY-DESIGN.md`; `TechData` formulas like beam-range `10000 × 2^level`).
...
**The resolver's exact input surface + where every dial inserts is audited in `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1`** — the salvo engine taken apart bit by bit: the `WeaponProfile` 7-field footprint the salvo math reads, which dials write an existing field (✅), which need one of six named new fields/terms (➕ — Penetration first), and which need a deferred mechanic (⚙). That doc is the resolver-wiring backlog behind these dials.
...
2. **Mirrored** — name how the **NPC runs this same gear/dial against the player through the identical order path** (delegation = NPC AI; there is no player-only code path — `docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md`). Miss it → *solitaire* (the inert-AI trap). *Nuance: a pure-infrastructure door the NPC uses FOR itself, not AT you — but it must still USE it.*
3. **Observable** — name the **gauge that shows both sides** the gear working (when the player uses it, and when it's used on them). Miss it → *pretty / an invisible bug* — usually cheap Failure-A (the number exists, just unwired; `docs/combat/INFORMATION-DELTA-DESIGN.md`), so build the readout. A door that passes only #1 is a display piece.
...
5. **Heat → sustained-rate term.** Throttle effective dps under sustained fire. ✅ **FOUNDATION LANDED (Weapons pilot W5a, 2026-07-10):** the heat twin of the ammo magazine — `Combat.RadiatorAtb` (the heat-sink component), `ShipCombatValueDB.HeatCapacity_kJ` (sum of installed radiators, 0 if none), and `FleetCombatStateDB.HeatPool_kJ` (starts cold). **Byte-identical** — a ship with no radiator reads 0 capacity, AND every current weapon has `WeaponProfile.HeatPerSecond` 0, so the heat step is skipped → energy fire untouched (gauge `ShipHeatTests`). **W5b WIRED (2026-07-10):** `WeaponProfile.HeatPerSecond` (the per-weapon waste-heat rate, default 0) + a HEAT pass in `StepEngagementGroup` before the damage phase — a fleet's energy fire adds `Σ HeatPerSecond × dt` to `HeatPool_kJ`, radiators shed `HeatCapacity × HeatDissipationFraction`, and over capacity the energy weapons throttle by `capacity/heat` (floored at `HeatThrottleFloor` 0.1). **Self-gating**: HeatPerSecond 0 → HeatPool stays 0 → skipped → byte-identical (a "cool" weapon needs no radiator; a HOT weapon needs enough radiators or it throttles). Gauge: a hot-beam attacker with a big radiator sustains and grinds the defender down, the same attacker with a small radiator overheats and the defender survives (radiator is the only variable). *(W5b also fixed a real bug the gauge caught: `BuildFireMix` dropped `HeatPerSecond` in aggregation, so the throttle never fired — now carried through, summed.)* **W5c CRADLE-TO-GRAVE DONE (2026-07-10):** two base-mod components (`heat-radiator` → `RadiatorAtb`; `pulse-laser` → `GenericBeamWeaponAtb` with a **Combat Heat** dial via a new 8-arg ctor, the exact-arity binder pattern) mounted on a NEW ship, the **Ember Pulse Cruiser** (hot pulse-lasers + radiators). A player-built Ember carries both the heat SOURCE and SINK the resolver balances (gauge `ShipHeatTests.TheEmberPulseCruiser_...`). **Heat item COMPLETE** (all three slices green). **Unlocks: Energy cooling, burst-vs-sustained** — the energy-weapon analog of ammo depletion. **Unlocks: Energy cooling, burst-vs-sustained** — the energy-weapon analog of ammo depletion (the magazine limits kinetic sustained fire; the radiator limits energy sustained fire).
6. **Missiles as resolvable targets + PD-capable flag.** Missiles were a firepower stub, not projectiles the salvo loop could shoot down. ✅ **FOUNDATION LANDED (Weapons pilot W6a, 2026-07-10):** the dedicated missile-killer — `Combat.PointDefenseAtb` (the PD-mount component, mirror of radiator/magazine), `ShipCombatValueDB.PointDefense_Jps` (sum of installed mounts, health-scaled, 0 if none), and the pure intercept math in `CombatEngagement` (`IsInterceptable` = `WeaponDelivery.Guided` — which survives `BuildFireMix` aggregation; `MissileFireDamage`; `FleetPointDefense`; `PointDefenseInterceptFraction` = `pd/(pd+missile)` saturating, capped at `PointDefenseMaxIntercept` 0.95). **Byte-identical** — a fleet with no PD reads 0 → intercept skipped (gauge `ShipPointDefenseTests`). **W6b WIRED (2026-07-10):** a per-defender PD pass in `StepEngagementGroup`'s damage phase — `InterceptMissiles(incoming, pdRating)` scales down the GUIDED fraction of a fleet's incoming fire by the intercept fraction, gated on `FleetPointDefense>0` (a PD-less fleet is byte-identical; non-guided fire never intercepted). Gauge: a defender under pure missile fire with a big PD screen holds all its ships where the same defender with no PD is ground down (PD the only variable). **W6c CRADLE-TO-GRAVE DONE (2026-07-10):** the buildable base-mod `point-defense-mount` (Intercept-Rating dial → `PointDefenseAtb`, six-point registration) on a NEW **Barricade PD Escort**, AND the previously-unregistered `missile-launcher` unlocked for the start faction on a NEW **Javelin Missile Cruiser** — so both the missile SOURCE and the PD SINK are player-buildable (new ships, so no battle fixture is perturbed; gauges: the Barricade reads PointDefense_Jps>0, the Javelin projects Guided fire). **PD item COMPLETE — the whole ⚙1 resolver backlog is now built (all six items green).** **Unlocks: real point-defense, the salvo-vs-PD duel.**
...
- **Δv (delta-v)** → the Tsiolkovsky reach (`Ve × ln(wet/dry)`) → the closing model's `DeltaVFloor` / `ManeuverBudget` (a fleet with more Δv can dictate range; a burned-out kiter loses control — `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.4` P2).
...
| **Exotic** | Reactionless thrust | Infinite Δv, no propellant | ✅ **LANDED (P2, 2026-07-10)** | `Movement.ReactionlessThrustAtb` sets `ThrustInNewtons` DIRECTLY (not Ve×burnRate — the naive "FuelBurnRate=0" would zero the thrust) + marks `NewtonThrustAbilityDB.Reactionless`, which pins `DeltaV = ReactionlessDeltaV` (1e12) and is guarded through the single fuel recompute funnel (`SetFuel` / `CargoTransferProcessor.UpdateMassFuelAndDeltaV`). Byte-identical (flag default false). Base-mod `reactionless-drive` on the NEW **Nomad** (no fuel-burning engine). Gauge `ShipReactionlessDriveTests`. *Follow-up: the in-space burn model (`NewtonianMovementProcessor` burning without consuming fuel).* | ✅ no-fuel flag |
| Exotic | **Inertialess maneuver** | Evasion decoupled from mass | ✅ **LANDED (P1, 2026-07-10)** | `Combat.InertialessDriveAtb` (evasion-override component) → `CalculateEvasion` reads the installed drives' `EvasionOverride` (health-scaled) as a FLOOR (max of mass-bound evasion and the override, capped at `EvasionCap`) `ShipCombatValueDB.cs`. Byte-identical (no drive → floor 0). Cradle-to-grave base-mod `inertialess-drive` on the NEW **Phantom** capital. Gauge `ShipInertialessDriveTests` | ✅ new Evasion term |
...
1. **Evasion-override term** (Exotic ▸ inertialess) — ✅ **LANDED (P1, 2026-07-10):** `Combat.InertialessDriveAtb` (a component) → `ShipCombatValueDB.CalculateEvasion` reads `InertialessEvasionFloor(ship)` (the greatest installed `EvasionOverride`, health-scaled) as a FLOOR (max of the mass-bound evasion and the floor, capped at `EvasionCap`). Byte-identical (no drive → floor 0 → ordinary mass-bound evasion). Cradle-to-grave: the base-mod `inertialess-drive` (Evasion-Override dial → `InertialessDriveAtb`, six-point registration in `engines.json`) on a NEW heavy capital, the **Phantom**, which dodges like a fighter where the equally-heavy Leviathan stays a sitting target. Gauge `ShipInertialessDriveTests`. *Was the ONLY genuinely-new resolver field the whole category needed — now built.* Unlocks: a capital that dodges like a fighter.
2. **Reactionless no-fuel flag** (Exotic) — ✅ **LANDED (P2, 2026-07-10):** NOT "nearly free" as first thought — the naive `FuelBurnRate=0` would zero the thrust (`ThrustInNewtons = Ve × FuelBurnRate`). The real change: a `NewtonThrustAbilityDB.Reactionless` flag (default false → byte-identical) + `Movement.ReactionlessThrustAtb` which sets `ThrustInNewtons` DIRECTLY and pins `DeltaV = ReactionlessDeltaV` (1e12, "unlimited"). The flag is guarded at the SINGLE fuel recompute funnel — `NewtonThrustAbilityDB.SetFuel` (which `CargoTransferProcessor.UpdateMassFuelAndDeltaV:182` routes through) — so a fuel/mass update can't reset it to a Tsiolkovsky value. Combat payoff: `FleetCombat.DeltaVFloor` → `ManeuverBudget` never depletes → kites forever. Cradle-to-grave base-mod `reactionless-drive` on a NEW cruiser, the **Nomad** (no fuel-burning engine at all). Gauge `ShipReactionlessDriveTests`. *v1 delivers the combat/closing payoff + the strategic Δv readout; the deeper in-space burn model (`NewtonianMovementProcessor` executing a burn without consuming fuel) is a flagged follow-up.*
3. **Terrain-combat term** (Traction) — ✅ **LANDED (P3, 2026-07-10):** terrain-in-combat was already wired by unit TYPE (`GroundTerrain.TerrainAttackMult`); P3 CONNECTS the *designed* drive. `GroundTerrain.LocomotionTerrainMult(roughHandling, terrain)` reads the unit's `GroundLocomotionAtb.RoughHandling` (via `GroundMobility.RoughHandlingForUnit`) so an all-terrain drive gets a combat edge on Rough/Cover ground and a road-bound one a penalty (open is neutral — mobility doesn't gate the open fight). Wired into `ResolveRegionCombat`'s attacker output. Centred on the neutral handling 0.5 → ×1.0 → byte-identical (every current unit). Reuses the existing base-mod `ground-locomotion` part — no new component. Gauge `GroundLocomotionCombatTests` (pure term + a designed all-terrain unit out-fights a road-bound one on rough ground). **⚙2 Propulsion complete — all three tractable ➕ terms (P1 inertialess · P2 reactionless · P3 terrain-combat) landed; the deferred ⚙ items (air/altitude/depth layer, H8 gate-network, H1 teleport) each need a whole subsystem and are left as noted.**
...
| **Decoy / spoof** (false contacts) | ◐ **wire** (injector) + ⏳ **defer** (the reading game) | inject a false `SensorContact` into the enemy track table (the table + `SensorContact` exist); but the "decide what's real" gameplay wants the **intelligence/information-ledger layer** (`docs/society/ESPIONAGE-AND-INTELLIGENCE-DESIGN.md`) for its full payoff |
...
| **Anomaly / mystery-box** | research + event chain | ⏳ **defer — NET-NEW, now DESIGNED** | one catalog entry on the field-site loop (`docs/explore/EXPLORATION-CONTENT-DESIGN.md`); no anomaly system in engine yet | research / late-game-crisis seed |
...
| Hole (`docs/archive/economy/COMPONENT-DESIGNER-CATEGORIES.md §5`) | Status | Home |
|----------------------------------------------|--------|------|
...
| Anomaly mystery-box + Xenoarchaeology (Survey §3.2) | **OPEN — designed, unbuilt** | Collapsed to the one field-site loop (`docs/explore/EXPLORATION-CONTENT-DESIGN.md`); first build step = fix `SystemBodyFactory.GenerateRuins` (`SystemBodyFactory.cs:~1418`). |
...
- **Weapons ▸ Beam** — reads/deducts `EnergyStored` before every shot (`GenericFiringWeaponsProcessor.cs:65-72`). No power → no shot. (Auto-resolve firepower folds weapon efficiency into **Mass**, not the salvo — `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1:101` "build-side, correct"; the resolver never reads power directly.)
- **Propulsion ▸ Warp** — the bubble is paid from **stored electricity**, not fuel (`WarpMoveProcessor.cs:233,244-246,265-267`). A flat battery is the literal "spawned ship won't move" bug; `ChargeReactors` is the fix. `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1:199`: warp power is **movement-side, not the salvo**.
...
2. **Armour nature-resistance — ✅ WIRED (2026-07-11), both ground and ship.** *(This was "the single richest wire in the category"; it's now built.)* Armour now carries a nature dimension the way shields do: a plating tuned to a damage nature (ablative shrugs off ENERGY, thins vs a KINETIC slug) soaks more or less accordingly. **Ground:** `GroundArmorAtb` gains four soak-effectiveness dials (`VsKinetic/VsEnergy/VsExplosive/VsExotic`) → the assembler Defense-weight-averages them onto `GroundUnit.ArmourVs*` → `GroundDamageMatrix.ArmourSoak(..., natureFactor)` (base-mod ablative/reactive plating; gauge `CombatKernelTests.ArmourSoak_NatureFactor` + `GroundUnitAssemblyTests`). **Ship:** `ArmourHardeningAtb` (per-nature soak) → `ShipCombatValueDB.ArmourSoakVs*` → `CombatEngagement.FleetArmourSoakFraction` applies a toughness-weighted fleet-average soak *after the shield* (base-mod Ironclad; gauge `ShipArmourHardeningTests`). **Byte-identical** for plain plate (natureFactor 1.0 / no hardening → soak 0). The armour-vs-shield choice is now a real rock-paper-scissors. See §5.1 rows + `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1`. *(Still open — headline #1: the ship-side flat-per-source SWARM-BOUNCE, distinct from this nature soak, remains unbuilt.)*
...
| **Material / nature-resistance — GROUND** | ✅ **WIRED (2026-07-11)** | `GroundArmorAtb.VsKinetic/VsEnergy/VsExplosive/VsExotic` (default 1.0) → Defense-weighted-averaged by the assembler → `GroundUnit.ArmourVs*` → the resolver passes `ArmourResistFor(weapon.Nature)` as the `natureFactor` on `CombatKernel.ArmourSoak` (scales the flat soak; penetration still decides the breach). Byte-identical at 1.0. **Three base-mod plating types now form a real matchup:** Composite (balanced) / Ablative (anti-energy) / Reactive (anti-kinetic/explosive), equal mass+Defense so the choice is purely "armour vs which threat." Gauges `CombatKernelTests.ArmourSoak_NatureFactor` + `GroundUnitAssemblyTests.ArmourNature_*`. |
| **Material / nature-resistance — SPACE** | ✅ **WIRED (2026-07-11)** | the SHIP mirror of the ground armour-nature: a `Combat/ArmourHardeningAtb.cs` component (four soak fractions vs kinetic/energy/explosive/exotic) → `ShipCombatValueDB.ArmourSoakVs*` → the fleet resolve soaks that fraction of the matching-nature salvo AFTER the shield (`CombatEngagement.FleetArmourSoakFraction`, toughness-weighted fleet average). 0 when absent → byte-identical. Base-mod `armour-hardening` (Ablative Hull Plating, anti-energy) on the Ironclad Ablative Cruiser. Gauge `ShipArmourHardeningTests`. *(v1 is the nature-SOAK decision — the fuller "give ships the ground flat-per-source swarm-bounce armour" model is a separate, balance-changing follow-up.)* |
...
**Reading:** Armour's **toughness core is Modelled** on both sides, the **flat-per-source soak is live on the ground** through the shared kernel, and — as of 2026-07-11 — **armour's NATURE dimension is now live on the ground too**: composite/ablative/reactive plating make "armour against WHAT" a real rock-paper-scissors decision that stacks against the threat (was: armour a flat scalar while shields carried the whole matchup). The remaining Defense wire is the **SPACE** side of both: **populate ship `Combatant.Armour`** so space armour bounces swarms like ground does (the resolver-merge already built the flat-soak math — it's just unfed on the ship side), then feed a ship-armour nature table through the same shared `CombatKernel.ArmourSoak natureFactor` the ground path now uses.
...
| Regen — GROUND | ✅ **WIRED (2026-07-11)** | `GroundAugmentAtb.ShieldRegenFraction` (per-design, default 0.34 = the old constant) → Shield-weighted-averaged onto `GroundUnit.ShieldRegenFraction` → the resolver's regen step reads it. Base-mod **Ward Projector** (fast recharge, small pool) vs the big slow Shield Generator — the capacity-vs-recharge decision. Byte-identical default. Gauge `GroundUnitAssemblyTests.ShieldRecharge_*` |
...
| Redundancy / damage-control | ⏳ **defer** | ties to the **degraded-condition** model (`docs/combat/WEAPONS-DESIGN.md`) — a genuinely net-new hook |
...
| 5.2 | **Regen (ground)** | `GroundAugmentAtb.ShieldRegenFraction` | ✅ **WIRED (2026-07-11)** | per-design recharge dial (default 0.34 = the old global constant), Shield-weighted onto `GroundUnit.ShieldRegenFraction`, read by `GroundForcesProcessor`'s regen step. Ward Projector (fast/small) vs Shield Generator (slow/big) | live in the ground regen step; byte-identical default |
...
| 5.3 | Redundancy / damage-control (crit-resist) | *(none)* | ⏳ defer | ties to the parked degraded-condition model (`docs/combat/WEAPONS-DESIGN.md`) — net-new hook. **This is hole H2's own resolution point** (see below). | none yet |
...
### Resolver insertion points (from RESOLVER-DESIGN Part A §1–§4)
...
- **Ground shield regen — ✅ NOW A DIAL (2026-07-11):** `GroundAugmentAtb.ShieldRegenFraction` (default 0.34) → `GroundUnit.ShieldRegenFraction` → the resolver's regen step; base-mod Ward Projector. *(The `GroundCombatant.cs:103` bridge still seeds 0 because that kernel-bridge path is additive/unwired; the LIVE regen step uses the per-unit dial.)*
...
| Dial | Verdict | How the sim models it |
|------|---------|------------------------|
| **Quality tier (design-time)** | ✅ **WIRED (2026-07-11)** | built as `UnitCaliberAtb` → `ShipCombatValueDB.UnitCaliberFirepowerMult` / `UnitCaliberToughnessMult` (health-scaled, best-of; default 1.0 → byte-identical), the resolver multiplies firepower/toughness by it. The literal field is the caliber mult rather than a field named `Quality`, but the capability — a static caliber tier baked at build, making a veteran-cadre hull ≠ a stock one — is live. Base-mod **Praetorian**; gauge `ShipCaliberTests`. Ground twin rides the same pattern. |
| Skill focus | ⏳ **defer** | which stat the tier weights (attack vs evasion) — rides the caliber field |
| Combat-earned | ⏳ **defer** | `CommanderDB.Experience` ticks up but is **never read**; a per-unit XP accrual that raises caliber is the harder second path |
| Academy (production) | ✅ (production) / ✅ (combat effect wired) | `NavalAcademyAtb` graduates officers — real; the officer→combat effect now lands via `CommanderBonuses` + a combat `BonusesDB` category (`CommanderCombatBonusTests`) — no longer net-new |
| Scarcity cost | ✅ **WIRED (2026-07-11)** | elites draw a **scarce talent pool** — `ManpowerTools.HasTalentToBuild` / `CommitTalent` / `ReleaseTalent` gate and draw talent through the colony host, so a caliber hull ties up scarce talent and a lost one frees it. Gauge `ManpowerTests` (talent commit/release/gate through the host). You *can't* mass-produce Space Marines. |
...
| **Automation** (non-combat) | run with **less crew** / faster industry — the §0f efficiency | ✅ **WIRED (2026-07-11)** — `CrewAutomationAtb` cuts a hull's bulk crew requirement (can't automate the officers/talent floor); base-mod Vanguard, gauge `ShipAutomationTests` |
...
| Automation (non-combat) | ✅ **WIRED (2026-07-11)** | `CrewAutomationAtb.CrewReduction` cuts a hull's bulk crew requirement in `ShipDesign.Recalculate` (floored at the talent/officer requirement — can't automate away the command crew). Base-mod Vanguard; gauge `ShipAutomationTests`. |
...
| **6.2 Unit-Caliber** | **Quality tier** (conscript/regular/veteran/elite) — the CORE design-time dial | a caliber **multiplier** stamped on the unit design | ✅ **WIRED (2026-07-11)** | built as `Combat/UnitCaliberAtb.cs` (firepower + toughness mult dials, NCalc ctor) → `ShipCombatValueDB.UnitCaliberFirepowerMult`/`UnitCaliberToughnessMult` (health-scaled best-of, default 1.0 → byte-identical). The capability is the caliber mult, not a field literally named `Quality`, but a veteran-cadre hull now hits harder / takes more than a stock hull on the same chassis. Base-mod Praetorian; gauge `ShipCaliberTests`. Ground twin rides the same mult slot | **Insertion point (used):** the mult folds into per-ship firepower/toughness alongside the doctrine mult (`CombatEngagement` `CombatShip` struct), so distinct-caliber units bucket naturally — O(buckets) aggregation preserved |
...
| **6.2 Unit-Caliber** | **Scarcity cost** (elites draw a scarce talent pool) | talent drawn per elite | ✅ **WIRED (2026-07-11)** | a ship's `ShipDesign.TalentReq` (Σ caliber-module crew) is drawn from `ColonyManpowerDB.TalentPool` (`AvailableTalent`/`CanCommitTalent`/`CommitTalent`/`ReleaseTalent`, was ZERO-consumer) at all 3 crew sites via `ManpowerTools.HasTalentToBuild`/`CommitTalent`/`ReleaseTalent` (gate `IndustryTools.ConstructStuff`, commit `ShipDesign.OnConstructionComplete`, release `ShipFactory.DestroyShip`). TalentReq 0 for non-caliber ships → byte-identical | the anti-dominance handle, LIVE: an elite hull ties up scarce veterans → can't mass-produce Space Marines. Gauge `ShipCaliberTests` + `ManpowerTests` |
...
| **6.3 Systems** | Automation (non-combat) | run with less crew | ✅ **WIRED (2026-07-11)** | `Combat/CrewAutomationAtb.cs` (`CrewReduction`, double-arg NCalc ctor) → `ShipDesign.Recalculate` subtracts it from BULK crew (never the veteran cadre) → the ship commits less scarce workforce. Base-mod `crew-automation` on the Vanguard (20 fewer crew than the stock hull). 0 = no module → byte-identical | the cheapest live win in this door, LIVE — mass + high tech traded for crew. Gauge `ShipAutomationTests`. *(faster-industry half still open)* |
...
- **The caliber field — ✅ BUILT (2026-07-11).** Was flagged as a net-new field to add; it's now `Combat/UnitCaliberAtb.cs` → `ShipCombatValueDB.UnitCaliberFirepowerMult`/`UnitCaliberToughnessMult`, folded into the per-ship firepower/toughness mult slot (health-scaled, default 1.0 → byte-identical). Gauge `ShipCaliberTests`. The ground twin rides the same pattern.
...
### §0g stamp
- **Reachable (cradle-to-grave, player-side):** ✅ for Bio-augmentation on the ground — research → build the augment → mount on a unit → the assembler reads its four fields (`GroundUnitAssembly.cs:74,108-110,129`) → the augmented unit fights → the unit dies and the augment is lost. ✅ (2026-07-11) for §6.2 **caliber** and its **automation** sibling — both are now real cradle-to-grave components (`UnitCaliberAtb`/`CrewAutomationAtb`, six-point registered on the Praetorian/Vanguard, drawn from the scarce talent/workforce pool, lost with the ship). ✅ for the §6.3 **fire-control RANGE** foothold (`FireControlRangeFactor`, ⚙4). ⏳ still for the rest of §6.3 Systems (the in-`HitFraction` targeting-computer term). The mineral→material→component→research→unit→decision→loss chain is now complete for augments AND caliber/automation.
...
| 8.2 Transfer | **TOW / GRAPPLE mode** (E1b) | reuse transfer throughput to move a whole *entity* | ◐ **Wire** | `CargoTransferAtb` reused as tow throughput; towed mass drags the tug's `NewtonThrustAbilityDB` accel (the honest cost) — DIALS §E1b `docs/archive/economy/COMPONENT-DESIGNER-DIALS.md:2513` | recover a drifting DERELICT (not just cargo). Re-parent the towed entity's `PositionDB` to the tug; blocked-on the exploration `SpawnWreck`/derelict content to have something to tow |
| 8.2 Transfer | **COVERT-INSERTION / infiltration bay** (E1d) | a bay that inserts an operative | ◐ **Wire** | mirrors `GroundTransport.TryLoadUnit/TryLandUnit` `GroundCombat/GroundTransport.cs:82/104` but gated by the **INVERSE** of `HasOrbitalControl` `:120` (insert where you DON'T hold orbit) + "attach as infiltrator" (hidden state) instead of a visible region roster — DIALS §E1d `docs/archive/economy/COMPONENT-DESIGNER-DIALS.md:2535` | get an operative into a hostile system undetected (stealth/trade/diplomatic cover); interceptable by the live sensor loop; the effect lands in the espionage Information Ledger, never the fleet resolver |
...
Engine home: `Pulsar4X/GameEngine/People/` (seats + officers) and `Pulsar4X/GameEngine/Factions/NPCDecisionProcessor.cs` (the auto-runner). Category maps the old templates `admin-complex` + `ship-command` (`docs/archive/economy/COMPONENT-DESIGNER-CATEGORIES.md` §2, row 10) onto **one door** with a scale dial.
...
| **§10.1 Command** | **Contracts** (Main-Merge Delta, NEW dial) — fixed-term assignment (5-yr default + early-break cost) | a term + break-cost on the seat | ⏳ **NET-NEW (designed-only)** | No engine field; designed in `docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md`. Composes with the existing doctrine-switch **cooldown** (a leader on contract runs their stance for the term — no per-tick flip-flop, good for the AI-cost bill). | The commitment texture on a delegation: you can't cheaply re-shuffle a cabinet mid-crisis. |
...
**§0i says: count resolvers, not doors.** Command has exactly ONE resolver — **the delegation loop** — and all 19 leader roles (Governor / Admiral / Foreign Minister / Spymaster / Chief Scientist / …) are *configuration* on that one pipeline, not 19 separate systems (`docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md`: "don't build 19 leaders — build one pipeline, prove it on one role, then every other role is a `BonusCategory` + a `Refresh` method"). The loop is the count-resolver §0i names.
...
The essence names these in build order (`docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md`; DIALS §E1a `:2503`, E3 `:2546`):
...
Across all five docs the same instruction repeats in five costumes: **build ONE parametric shape, let the specifics be data/configuration/emergence, make the NPC run it too, and make both sides see it.** The pillar skeleton (`docs/society/INFLUENCE-PILLAR-DESIGN.md`) = one shape, four media. Delegation = NPC AI (`docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md`) = one machinery, run both ways. The field-site loop (`docs/explore/EXPLORATION-CONTENT-DESIGN.md`) = one loop, every type is data. The Living Galaxy (`LIVING-GALAXY-DESIGN.md`) = don't build the emergence, build deep+connected+observable parts and refuse a director.
...
### E1a — Leadership essence → Command & Control gear (`docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md`)
...
### E1b — Exploration essence → expedition & network gear (`docs/explore/EXPLORATION-CONTENT-DESIGN.md`)
...
### E1c — Influence essence → soft-power projection gear (`docs/society/INFLUENCE-PILLAR-DESIGN.md`)
...
### E1d — Espionage essence → intelligence & covert gear (`docs/society/ESPIONAGE-AND-INTELLIGENCE-DESIGN.md`)
