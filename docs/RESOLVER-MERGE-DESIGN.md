...
3. **Planetary side adopts the kernel — split into 3a/3b (the ledger, /scratchpad slice3-ledger, said only ONE piece is truly byte-identical today):**
   ...
   - **3b — the DELIBERATE rebalance.** `GroundUnit` presents a `Combatant` (a synthesized `WeaponProfile` from Attack/DamageType/Range; Position from hexes×HexPitch), and the per-source dodge + shield route through the kernel; the Armor▸Infantry▸Artillery **triangle DISSOLVES** into weapon-nature × armour/evasion; the innate-% shield becomes the depleting **pool**. **This moves ground numbers** — `GroundForcesTests` is re-baselined with the reason written down, and the developer reviews the new numbers (it's a balance change, not a refactor).
