...
| 21 | What does capturing a planet transfer? | **RESOLVED 2026-08-17 (developer, via OPERATION BLUEPRINT-TO-STEEL C7 = Option A, answered 2026-08-16): capture flips the colony + installations + surviving population + stockpiles to the conqueror, WITH a population/unrest hit** (a morale/legitimacy penalty on the taken colony). Build = C7 in the implementation campaign; recon at `GroundForcesProcessor.cs:1073` (owner-flip) + `FactionInfoDB.Colonies` registry add/remove + `ColonyMoraleDB`/`LegitimacyDB` capture hit. |
