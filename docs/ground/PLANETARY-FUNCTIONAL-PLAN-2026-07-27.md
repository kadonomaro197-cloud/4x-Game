...
| **DS-2** | **Retire `groundStances.json`.** Point ground formations at the 25-entry unified catalog; delete the 3-entry legacy file; keep the `AttackMult` → `FirepowerMult` mapping. | **D1-2** | The existing `UnifiedDoctrineTests` ±25% byte-identity guard, plus a new assert that the three ids resolve to the unified entries. | **Behaviour-affecting today** — the two files collide on three ids and **load order decides which stance a formation gets**. This is not tidying. |
