...
| B-orders-inherit (C1) | Toggle Inherit Orders (sub-fleet) | `forceswindow.html` §10 Fleet-ops | 🔨 | client `FleetWindow.cs` — un-commented the dead Standing-Orders wire, repaired the stale `FleetDB.Parent.Guid` check (now the `TreeHierarchyDB.Parent` sub-fleet test) → `FleetOrder.ToggleInheritOrders` |
