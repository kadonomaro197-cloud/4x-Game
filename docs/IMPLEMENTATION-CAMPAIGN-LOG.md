...
> - **A1 (guided warhead) = A** — read a representative/default warhead at build (approximate, cached). → BUILD C-guided.
...
> - **D9 (Console Space on ship bridge) = rec** — hide/relabel it on ship mounts. → BUILD (dead-knob hide).
> - **D10 (chassis structural-efficiency slider) = DROP.** → BUILD (remove the dead dial).
> - **D11 (Fighter Construction Points) = DROP.** → BUILD (remove the dead dial).
