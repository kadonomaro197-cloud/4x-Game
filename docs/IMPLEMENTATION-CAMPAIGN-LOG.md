...
| C-guided | Guided-weapon real warhead (item #3) | `entityassembler.html` / TIER 3 #3 | ✅ answered ⬜ build | **= THE PICKED WARHEAD** (2026-08-17) — firepower reads the specifically-picked ordnance, not A1's representative-heaviest proxy; ready to build |
...
| C-mobility | `GroundMobility.SpeedMultForUnit` scale-not-replace (item #5) | `propulsionderived.html` / TIER 3 #5 | ✅ answered ⬜ build | **= MULTIPLY** (2026-08-17) — `speed = frameMode × SpeedFactor` (one-line; re-baseline mobility gauges); ready to build |
