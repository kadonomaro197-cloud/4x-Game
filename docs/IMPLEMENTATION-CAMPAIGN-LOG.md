...
> **This batch (one CI run):** shipDesigns.json + earth.json (ordnance) + CarrierSortieTests.cs + DockTools.cs/NewGameMenu.cs comments + weapons.json/energy.json (5 dead knobs) + AuraAtb.cs (enum doc) + WEAPONS-DESIGN.md (won't-build) + docs. **NEXT: #2 Magazine Phase B** (the ready-locker wire into the live missile path — recon ledger banked: flag `EnableOrdnanceMagazine`, reload loop `GenericFiringWeaponsProcessor.cs:99-102`, fire-gate `:50`, deplete `:80`, hold-deduction `MissleProcessor.cs:134` + the count-vs-1 salvo bug; CI-compile-only, developer verifies live), then the backlog (planet-view per-hex, HTML re-sync, Entity Assembler window).
>
...
> 2. **Magazine Phase B** — the charge-counter conversion (developer-approved); CI-blind ON path, developer verifies live. [recon banked]
