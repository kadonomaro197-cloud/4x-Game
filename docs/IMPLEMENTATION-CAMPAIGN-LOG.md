...
> **🧭 FRESH NEXT ACTION (2026-08-16, end of session) — Phase C run-cost + both morale/legitimacy civic dials are DONE + CI-green (through `cf6638b`; Medical `daae8f2` ⏳CI). Pick the next FILE-DISJOINT buildable slice:**
> 1. **C-civic Commerce dial** — the one remaining civic dial (trade → income). First re-read `docs/assembler/01-IO-civic.md` §A (9 civic jobs) + `civicderived.html` to enumerate the EXACT remaining dial set (Recreation/comfort is already live via `HousingComfort`; Jobs=A1, PublicOrder=Security, Medical=done — so Commerce + verify Amenity). Same flag-gated-OFF + buildable-not-installed + gauge pattern as Security/Medical.
> 2. **Phase D — planetary view (biggest unstarted chunk):** D-units (select/read/move one ground unit through the ONE queued verb both seats use — kill the direct-call bypass `PlanetViewWindow.cs:581`), then D-stockpile (per-hex stockpile + hex-to-hex haul), then D-planfn.
> 3. **Phase E — E-env:** `CombatConditions` into the shared combat kernel (space combat stops being environment-blind).
> 4. **C-sensors** — the band-match fix (⚠ backlog file:line STALE + behaviour-changing → own focused slice).
> **PARKED for the developer (ADJUDICATION QUEUE — do NOT guess):** A4-ORDERS (4 order-stub behaviors) · C-GUIDED (missile ordnance-at-build) · C-MOBILITY (drive×frame combine) · B-ORDERS-MOVEMENT (Intercept/Ram) · C-deadknobs TIER 4 #6–8 + the capture-transfer ruling. **Full slice board below reflects this session.** Historical NEXT ACTION detail preserved below.
>
...
| C-civic | New civic dials per IO census (`01-IO-civic.md`) | `civicderived.html` / 02-IO-MATRIX | 🔨 | **Both morale/legitimacy dials DONE (2026-08-16):** **Security → legitimacy** (`SecurityAtbDB` → `GetTotalSecurity` → flag-gated `LegitimacyInputs.SecurityStrength`, `security-precinct` buildable, gauge `SecurityLegitimacyTests`, `a7b9981`, CI-green) + **Medical → health → morale** (`MedicalAtbDB` → `GetTotalMedical` → flag-gated `MoraleInputs.HealthStrength`, `medical-hospital` buildable, gauge `MedicalMoraleTests`, `daae8f2` ⏳CI). Both flag-gated OFF + `NewGameMenu`-on + buildable-not-installed (out of the shared-fixture baseline). **Jobs** = A1 employment (✅). **REMAINING:** a **Commerce** dial (trade→income) + verify the full civic-designer census (`01-IO-civic.md` §A: the 9 civic jobs — Recreation/comfort is already live via `HousingComfort`; confirm which others still need a dial) before flipping ✅ |
