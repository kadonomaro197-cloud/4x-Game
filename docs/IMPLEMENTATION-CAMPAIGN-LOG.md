...
> 1. **Aura base-mod TEMPLATE** — buildable projector (ship) + command building (colony) carrying `Combat.AuraAtb`, six-point gotcha-10 registration (mirror the `command-berth` SE-2a precedent). **Unblocks BOTH E14 folds (3a space + 3b ground) in-game** — they're built but have nothing to mount/build without this. [recon in flight]
...
> 3. **E-env slice 3** — thread `CombatConditions.AmbientDoT_Jps` (the live piece) + Firepower/ShieldRegen/Cover (identity today → byte-identical) into the resolver. [recon in flight]
> 4. **E12 slice 2** (carrier rearm/refuel-on-recovery — needs a parasite-with-holds design) · **E13 later** (organic feed + no-electrical-power bypass).
