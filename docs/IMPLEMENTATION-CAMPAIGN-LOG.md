...
> **🧭 FRESH NEXT ACTION (2026-08-22f) — SLICE-2b: the core/advanced split is now DATA-DRIVEN (the developer questioned "10 more times?" — answer: no). The client per-door map is GONE; a template's JSON marks its own fine dials.**
...
> - **NEXT:** wait for CI green (build-client + the shards; the JSON field is exercised by every colony-build test). Then the developer runtime-verifies the Weapons designer locally (Component Designer → Weapons → laser shows Range/Power Input/Charge Period + an "Advanced settings" expander; railgun/flak/etc. stay flat). Tagging any other door's fine dials is then a pure data follow-up on request.
>
