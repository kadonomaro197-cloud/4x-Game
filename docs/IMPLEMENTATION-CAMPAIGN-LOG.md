...
> - **A2 (drive × frame speed) = realistic MUTUAL EXCLUSIVITY** (dev: "remove one option if another is selected"). The frame's `Locomotion` mode is the unit's DEFAULT drive; a mounted `GroundLocomotionAtb` drive REPLACES it (engine already does this), and the **Entity Assembler HIDES/greys the frame's Locomotion dial when a drive is mounted** so you never set two conflicting mobility systems and the frame mode isn't "dead weight" — it only shows when it's the thing in play. → BUILD C-mobility (revised; assembler-UI change, engine unchanged).
