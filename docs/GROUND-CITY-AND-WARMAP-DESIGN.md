...
- **C1b — AREA-SCALED mini-hex count (developer refinement, 2026-07-04).** The mini hex is a **fixed real footprint** (a building's size — e.g. a factory takes exactly 1 mini hex, everywhere). The number of mini hexes inside a developed operational hex is therefore **its real area ÷ the mini-hex footprint** — NOT the fixed `CityPatchRadius=6` C1 hardcodes. So a bigger operational hex (more real km²) holds more mini hexes; a factory is the same physical size on Earth and Mars. The engine already carries the input: `Region.Area_km2` (÷ the hex count for per-hex area) is the "Africa is huge" instrument. **Nuance:** whether an Earth operational hex holds *more* mini hexes than a Mars one depends on whether Earth's operational hexes cover more real km² — today the grid gives a bigger planet *more* hexes (≈constant per-hex area), so to get the developer's "Earth hex = 7 mini, Mars hex = 3" feel we make big planets have **fewer, bigger** operational hexes (a scaling knob), so each covers more ground. **Open number (confirm before hardcoding, per the developer's rule):** the real footprint of one mini hex in km² (equivalently, a factory's footprint). Locked intent: infrastructure builds on BOTH scales (strategic building = a whole operational hex, W-track; fine building = 1 mini hex, C-track); battles stay at operational-hex scale.
