...
#### C6 — Diplomacy behaves (IFF + first-contact + commerce wiring, #32/#33) — 🔵 IFF + FIRST-CONTACT DONE (CI); commerce ⚫ NOT-YET
...
- **Still NOT-YET:** commerce/logistics/military transit gated on the treaty flags (`TradeAgreement`/`LogisticsAccess`/`MilitaryAccess`). **Method:** engine integration test (two factions, set a flag → assert commerce/supply responds); live drive from a diplomacy panel.
