...
> **🚧 QUARANTINED test — ROOT CAUSE FOUND 2026-07-10:** `EconomyReadoutTests.Economy_BaselineReadout_OverOneYear` — the refinery produces **no Space-Crete over a game-year**. The cause is **not** "inputs not mined" as first thought — it's a **STORAGE VOLUME CAP that jams the whole colony economy**. The `earth.json` Cargo block stocks 50M of ~17 general-storage goods, but 10 warehouses only hold ~6.5 of them: the CI readout (run #786) shows the first 6 minerals at 50M, then Lithium partial, **Titanium at -3**, and Silicon/Regolith/Water/Space-Crete/etc. **clamped to 0**. Storage being FULL means mined output has nowhere to go (deposits deplete only ~11 units/yr) and the refinery starves (3 of Space-Crete's 5 inputs at 0 → job crawls to 3/25 pts/yr, never completes). **A first fix that just ADDED regolith+water to the Cargo list FAILED (run #786 red)** — they clamp to 0 like the rest. **The real fix is a storage/stockpile rebalance** (more warehouse volume, or smaller per-good amounts) so the economy isn't storage-locked — verify the `CargoMath` volume mechanism first, then re-enable. Full detail: `GameEngine/Industry/CLAUDE.md` → ALPHA stockpiles. This is the one deliberately-red engine gap right now.
