...
| **🎛 COMPONENT-DESIGNER RE-DERIVATION** (`FireControlMassLeakTests`, `PowerFuelGateTests`, `PowerJustificationTests`, `CargoCompartmentTests`) | the fixes falling out of `docs/economy/DESIGNER-NORTH-STAR.md`, each pinned byte-identical where it touches shipped designs: **Sensors S2** — the beam director's two phantom dials (`Size vs Range` / `Size vs TrackingSpeed`) wrote no sim variable and only multiplied mass, so both at 0.25 was a **free 16× mass saving**; deleted structurally, and the shipped director still weighs exactly 150 kg. **Power P0** — `Lifetime` reaches the attribute in SECONDS (the reactor's real endurance was 2.43 hours, the RTG's 5 seconds), `Output vs Economy` makes 2× power cost 4× fuel, the reactor's specific fuel rate is anchored on the turbine's as a **ratio** so a re-tune fails loudly, and `EnableFuelExhaustion` (default off) makes a dry generator produce nothing without a save-format change. **Power §39.8** — the developer's rule *every option must win one axis outright*, executable: the reactor wins per-kg (and its crew is no longer a kilogram count — it demanded **1500 crew for 1500 kg**), the turbine wins per-m³, solar is the ONLY silent source and can now be built on a colony, the RTG's zero-crew axis is pinned on the template, and **`NoPowerType_IsDominatedOnEveryAxis` is the rule itself** — the next dead recipe fails CI instead of shipping. **Logistical L1 (§46a)** — **three of six declared cargo classes had no component that provided them**, so `TeamObject` (every `Scientist`), which has always asked for `passenger-storage`, could never be loaded onto anything — and `CargoMath.GetFreeVolume` reported that as a **silent 0**, making "cannot carry that" indistinguishable from "no room right now". Four compartment templates landed (`passenger-cabin` · `cryo-bay` · `refrigerated-hold` · `containment-hold`) on the existing 2-arg `CargoStorageAtb`, plus `perishable-storage` and `contained-storage`; and **the first test is the structural rule that cannot rot** — every declared class must be providable by something, allow-list entries carrying a stated reason. Person-volume corrected from **65 litres a head (body volume)** to a 10 m³ berth / 2 m³ pod, byte-identical because nothing had ever provided the store to read it | 🔵 |
