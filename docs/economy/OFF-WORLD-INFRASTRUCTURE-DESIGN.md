...
**Consolidated 2026-07-13 from:** `docs/economy/OFF-WORLD-INFRASTRUCTURE-DESIGN.md` (the universal off-world host + its full cradle-to-grave build state), `docs/economy/OFF-WORLD-INFRASTRUCTURE-DESIGN.md` (the growth ladder — the shared progression axis).
...
Companions: the colony **morale/population** survey (`docs/society/MORALE-AND-POPULATION-DESIGN.md`), `docs/NORTH-STAR-VISION.md` (the BSG nomadic pillar), and `docs/aurora/PLANETARY-INFRASTRUCTURE.md`.
...
- **Morale / population / manpower (M1–M3, built)** — `ColonyMoraleDB`, `ColonyManpowerDB`, `EmploymentAtbDB`, `HousingAtbDB` were built **host-agnostic** (no `ColonyInfoDB` dependency). A manned station attaches the *same* blobs; the one load-bearing change is teaching `PopulationProcessor` to also process `StationInfoDB` (or a thin parallel processor). So the morale/people loop serves stations nearly for free — this is the answer to the station "manning" open question. See `docs/society/MORALE-AND-POPULATION-DESIGN.md` Prime-Directive accounting finding #7.
