...
|---|---------|--------|----------|
| **B1** | **3 templates aren't unlocked in the `earth.json` colony** | `food-production`, `naval-academy`, `logistics-office` have complete templates *and* default designs, but their ids are **absent from the `earth.json` embedded 8.2 B colony's `StartingItems`/`ComponentDesigns`** (gotcha #10, the two-unlock-ends rule). **⚠ Scope correction 2026-08-09: this was over-generalized to "un-buildable out of the box." The PLAYABLE start is `uef-devtest.json`, NOT this `earth.json` colony — and the DevTest/UMF/Kithrin starts DO unlock `food-production` (uef-devtest even begins with 2 pre-built agri-complexes; `Colonies/CLAUDE.md` `FoodProductionAtbDB` row). So food is buildable in the game you actually play; the gap is specific to the unused `earth.json` colony.** | **LOW** (scoped to `earth.json`) — the playable DevTest start already unlocks food |
