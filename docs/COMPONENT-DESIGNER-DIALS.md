...
| **Material / nature-resistance — SPACE** | ◐ **wire** | the SHIP twin is still open: `armorType` material EXISTS but is hazard-only, dead to the auto-resolver (ship armour folds into `Toughness`, and `Combatant.Armour` is unfed). The ground kernel path (`natureFactor`) is built and shared, so the space wire is: populate `Combatant.Armour` (the flat-soak wire above) THEN pass a ship-armour nature table through the same `CombatKernel.ArmourSoak natureFactor`. |
