...
5. **Heat → sustained-rate term.** Throttle effective dps under sustained fire. ✅ **FOUNDATION LANDED (Weapons pilot W5a, 2026-07-10):** the heat twin of the ammo magazine — `Combat.RadiatorAtb` (the heat-sink component), `ShipCombatValueDB.HeatCapacity_kJ` (sum of installed radiators, 0 if none), and `FleetCombatStateDB.HeatPool_kJ` (starts cold). **Byte-identical** — a ship with no radiator reads 0 capacity → the heat step is skipped → energy fire untouched (gauge `ShipHeatTests`). **Remaining: W5b** wires the per-salvo heat accumulation + throttle into `StepEngagementGroup` (energy fire adds heat, radiators shed it; over capacity → energy dps throttles by capacity/heat), gated on capacity>0 so radiator-less fleets stay byte-identical; **W5c** the buildable base-mod radiator on a beam-heavy ship (which overheats under sustained beam fire without enough radiators). **Unlocks: Energy cooling, burst-vs-sustained** — the energy-weapon analog of ammo depletion (the magazine limits kinetic sustained fire; the radiator limits energy sustained fire).
