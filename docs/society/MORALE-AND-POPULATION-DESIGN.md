...
*M1–M5a are built and wired (see the build-status banner at the top). The remaining live work is the M2-data calibration, the M3-2 enforcement wiring, M5b (energy+food attach), and Governance v1 (the morale-nudge consumer) — each needs the developer's local build to confirm runtime feel, since CI can't run the client. Live build-state is tracked in `docs/archive/SYSTEMS-STATUS-AND-TEST-PLAN.md`. Lock each remaining slice's open questions as we reach it.*
