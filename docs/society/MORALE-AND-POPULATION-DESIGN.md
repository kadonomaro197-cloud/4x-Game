...
- M2: housing-tier numbers; worker-slots per installation type; the labor-shortage output penalty curve. **Open finding (surfaced building M2-A) — ✅ RESOLVED 2026-08-13:** employment must be measured against something that scales with population, NOT raw total pop — a billions-pop homeworld can't be "employed" by a few installations, so a literal jobs/total-pop (or even jobs/workforce) ratio reads as total unemployment and would tank the colony. **The fix chosen (developer-authorized): a per-capita job DEMAND** — `jobs ÷ (population × ColonyMoraleDB.JobsPerCapita)`, the SAME shape `SustenanceProcessor` uses for food/power (NOT the M3 workforce×0.5, which is still ~5 orders of magnitude too large vs installation-crew jobs). `JobsPerCapita = 7.0e-6` (mutable/tunable) lands the ~52k-job start homeworld just under full employment (near-neutral); building industry earns the +15 bonus. Wired at all three morale sites (`PopulationProcessor` ×2 + `StationPopulationProcessor`), flag-gated OFF pending a local runtime read. Gauge `EmploymentMoraleTests`.
...
*M1–M5a are built and wired (see the build-status banner at the top). The remaining live work is the M2-data calibration, the M3-2 enforcement wiring, M5b (energy+food attach), and Governance v1 (the morale-nudge consumer) — each needs the developer's local build to confirm runtime feel, since CI can't run the client. Live build-state is tracked in `docs/TESTING-TRACKER.md` (the old `SYSTEMS-STATUS-AND-TEST-PLAN` was RETIRED 2026-07-27). Lock each remaining slice's open questions as we reach it.*
