...
> **Consolidated 2026-07-13 from:** `docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md` (the delegation mechanism + span of control) and `docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md` (the 21-role leadership roster, the two-axis seat grid, the parity/automation audit, and the six-rung leader cradle-to-grave pipeline). This is the single home for **how agency is delegated, who the leaders are, and how one delegation layer doubles as the NPC AI.** The dated conversational framing and the AI performance-cost walkthrough from the self-play doc were compressed to their durable decisions; nothing load-bearing was dropped.
...
| **Internal politics** | **Interior Minister** | favour stability / military / low taxes / balance the blocs | (designed — `docs/society/GOVERNMENT-AND-POLITICS-DESIGN.md`) |
| **External politics** | **Foreign Minister** | seek allies / isolationist / expand by tribute / keep the peace | (designed — `docs/society/DIPLOMACY-DESIGN.md`) |
...
- **Turned / defected** — the espionage grave rung (already named in `docs/society/ESPIONAGE-AND-INTELLIGENCE-DESIGN.md`). Clean defection (they leave) or a **mole** (stays seated, leaks/sabotages until counter-intel catches them). → **espionage + the Information Ledger.**
...
- **Colony development** — a new single `ColonyDevelopment` accessor aggregating `HexTile.InfrastructureLevel`, forward-compatible with `docs/economy/OFF-WORLD-INFRASTRUCTURE-DESIGN.md`'s tier ladder. Read by the academy competence roll; a bombarded world demotes it AND starves the pipeline (shared grave rung).
...
- **Ground combat** (`docs/ground/GROUND-SURFACE-MAP-DESIGN.md`, `docs/ground/GROUND-SURFACE-MAP-DESIGN.md`) — the General chain sits over the built formation/doctrine layer; the person-commander is the net-new wrapper.
...
- **Foreign Minister pillar** — worked out 2026-07-07 (in `docs/society/DIPLOMACY-DESIGN.md`). The overt twin of the Spymaster: empire FM → per-faction FM → Envoy/Ambassador hands, on the leader pipeline. **Key finding: the three "keystone" prerequisites are cleared/dissolved** (GlobalManager fixed, `SignalQuality` role cut → gradient in the Ledger *(the engine field itself is still live — corrected 2026-07-28)*, hostility-from-diplomacy done) and the substrate is built — so the FM is the *decider* that drives built-but-DARK machinery (reactive overture engine, treaty proposal, exchange-catalog executor, casus-belli gate). FM MVP = per-faction FM + the "Are we good?" loop + the NPC counterparty + an observation feeder.
...
- **Spymaster / espionage pillar** — worked out 2026-07-07 (`docs/society/ESPIONAGE-AND-INTELLIGENCE-DESIGN.md`): agents are full leader-entities; org = empire **Director of Intelligence** → per-faction **Spymaster** → agents; **counter-intel = same agent, target = self**; `SignalQuality` **CUT**; Spymaster MVP = Ledger + passive + per-faction seat + `gather` + the NPC counterparty, as the **second** leader vertical slice after the Governor. **Agent delivery = PHYSICAL** (troop-transport reuse, EMCON=stealth, "attach as infiltrator" the one new mechanic).
