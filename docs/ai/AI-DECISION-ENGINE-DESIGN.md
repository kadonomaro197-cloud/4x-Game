...
> **What this is.** The two halves of the faction AI's decision-making, folded into one doc. **PART A — the DESTINATION:** how a faction figures out *what it wants* (the needs-ladder objective engine — instinct tiers that emerge from the faction's state, filtered by identity and mood). **PART B — the MIDDLE (the means-ends planner):** how a *settled* objective becomes a *reachable* chain of orders — walking a goal backward through its prerequisites until it hits something the faction can actually do this cycle. Part A picks the destination; Part B plots the course to it. Together with `docs/ai/AI-COMMAND-AND-COMMUNICATION-DESIGN.md` (the "how" — delegates, mandates, traits, mood) they are the whole brain: identity → destination → plan → orders.
>
> **Consolidated 2026-07-13 from:** `docs/ai/AI-DECISION-ENGINE-DESIGN.md` (the destination half, v0.2 discussion draft, 2026-07-10) + `docs/ai/AI-DECISION-ENGINE-DESIGN.md` (the means-ends planner scope/build map, 2026-07-11 four-agent survey).
>
...
*Companions: `docs/ai/AI-COMMAND-AND-COMMUNICATION-DESIGN.md` (the "how" — mandate/report, identity, mood, the three-mode dial), `docs/society/GOVERNANCE-AND-DELEGATION-DESIGN.md` (the roster + parity), `docs/ai/AI-PERSONALITY-IMPLEMENTATION-SPEC.md` (the trait code), `docs/ai/AI-BRAIN-BUILD-TRACKER.md` (design→code bridge + socket verification), `docs/ai/AI-BRAIN-BUILD-TRACKER.md` (row 2.8 is the Part-B planner build authority). This doc is the "what/where + how-to-reach-it" — the destination the objective engine picks, and the plan the means-ends layer computes to reach it.*
