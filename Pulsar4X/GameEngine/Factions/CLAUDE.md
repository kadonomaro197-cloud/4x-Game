...
| `MilitaryTarget.cs` | **NEW (P-3 military REACH, slice 1, 2026-07-12)** The "which enemy world do I aim at?" perception — the first of the three helpers `ConquerResolver` deferred (MilitaryTarget / MilitaryComposition / MilitaryReach). `NearestEnemyColonyBody(faction)` names the `ColonyInfoDB.PlanetEntity` of a colony owned by a faction this one holds an `AtWar` latch toward (the latch a 3.4b coalition sets) — so the chain composes: **3.4b declares the war → this names the target world → the reach slice sails the fleet there**. Pure read, no warp/order surface (the movement-order emission that USES this is the next slice; it touches the landmine-dense warp path so it's kept separate). v1 = the first at-war rival's first colony; nearest-by-jump-route is the MilitaryReach slice. No war / no rival colony → `InvalidEntity`. Gauge `MilitaryTargetTests`. |
