...
3. **NPCDecisionProcessor now FIRES (GlobalManager wired, 2026-06-30) but its decision body is still a stub.** The loop runs (see note above); the `Tick` that turns doctrine weights into actual orders is the TODO. So it's safe and live but does nothing yet — implement the decision body when building NPC turns. **⭐ Before writing that decision body, read THE AI DESIGN SUITE (8 `docs/AI-*.md` docs, done 2026-07-10/11).** This processor is the **socket the whole designed AI plugs into** — the full "quark→supercluster" design already exists. **Enter through `docs/AI-IMPLEMENTATION-AND-WIRING-MAP.md`** (the design→code bridge; its §4 SOCKET VERIFICATION maps every element to a code home), then the code-ready `docs/AI-PERSONALITY-IMPLEMENTATION-SPEC.md`. **Two hard prerequisites the suite flags before any AI logic:** (a) fix **seat durability** (`People/AdminSpaceProcessor.cs:36-46` rebuilds `CommanderSeats = new List<>()` every pass — the developer's "paramount, nothing can be built until fixed," 2026-07-11); (b) the fleet-commander *situational read* and the crisis ascension-seed are the two load-bearing NO-SOCKETs to build on paper first. The doctrine-weight approach here becomes the richer 12-trait `PersonalityDB` + shared `DecisionScorer` from the spec.
