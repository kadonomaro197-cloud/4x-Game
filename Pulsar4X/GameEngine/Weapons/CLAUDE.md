...
| `WeaponMissile/MissleProcessor.cs` | Static class. `LaunchMissile()` creates missile entity with NewtonMoveDB + ordnance design components. Uses `directAttack = true` → `ThrustToTargetCmd`. **Fires from the BULK `ordnance-storage` hold**: gates on `cargo.TypeStores[...].CurrentStoreInUnits[...] ≥ 1` (`:31`) and removes 1 on launch (`:134`) — **BOTH now flag-gated behind `OrdnanceMagazineTools.EnableOrdnanceMagazine` (Phase B, 2026-08-19): flag OFF = this exact behaviour (byte-identical); flag ON = the gate + deduction move to the ready-magazine reload** (see `OrdnanceMagazineTools.cs`). ⚠ The flag-OFF launch still removes exactly `1` regardless of the `count` salvo size (`:64` spawns `count`) — a pre-existing count mismatch left unchanged for byte-identity (flag ON debits the hold whole-round at reload, so the mismatch doesn't apply there). |
| `WeaponMissile/OrdnanceMagazineTools.cs` | **NEW — PHYSICAL ORDNANCE MAGAZINE, Phase A (physical-supply, 2026-08-18).** The pure two-tier "ready-service locker" math: a launcher fires from a small READY magazine that depletes on fire and refills from the bulk `ordnance-storage` hold over time — the naval ready-locker vs deep-magazine split the engine lacks (today a launcher fires straight from the bulk hold, gated only by an abstract rate counter that reloads from NOTHING). Pure/unit-testable (`CanFire`/`Fire`/`Reload(readyRounds, readyCapacity, holdAvailable, reloadRate, dt) → (newReady, pulledFromHold)`, conservation-preserving). **Phase A = this math + gauge ONLY, byte-identical (nothing calls it) + flag `EnableOrdnanceMagazine` default OFF.** ⚠ NOT to be confused with `Combat/ShipMagazineAtb` (that's the auto-resolver's ABSTRACT kg ammo-pool — a deliberately separate model, Combat gotcha #1). **Phase B BUILT (2026-08-19, flag-gated OFF — developer verifies live).** The developer's ruling was NOT the rounds-based `Reload` (its `reloadRate × dt` FLOORS to 0 for a base-mod launcher that reloads 1 charge-point/sec = 1 round per 120 s → silenced forever) but **REUSE the charge counter with a conversion**: new pure `ReloadCharge(currentCharge, reloadPerTick, magSize, amountPerShot, holdAvailable) → (newCharge, roundsPulledFromHold)` treats the launcher's existing `InternalMagQty` (abstract charge) as the ready-locker, keeps the SAME charge accumulation for timing, but a round only COMPLETES (crosses an `amountPerShot` boundary) when a physical round is pulled from the bulk hold; an empty hold HOLDS the charge just below the boundary (grave rung, no floor-trap). Wired flag-gated at THREE points: (1) `GenericFiringWeaponsProcessor` reload loop — for a missile launcher with a loaded `AssignedOrdnance`, call `ReloadCharge` + `RemoveCargoItems(entity, ord, pulled)`; (2) `MissleProcessor` fire-gate (`:31`) — skip the bulk-hold gate (the ready-locker/charge gate upstream is authoritative); (3) `MissleProcessor` launch-deduction (`:134`) — skip (the hold was debited whole-round at reload). **Flag OFF → all three `else`/`!flag` branches are the verbatim current code → byte-identical** (the fire-gate `shots = InternalMagQty/AmountPerShot` and deplete `:80` are UNCHANGED — they already work on the charge/ready-locker). The pure `ReloadCharge` conversion is CI-gauged (`OrdnanceMagazineTests.ReloadCharge_*`), but the wiring's LIVE behaviour is CI-blind (the missile-firing path can't run headless) → the developer flips `EnableOrdnanceMagazine = true` + verifies. ⚠ NOT to be confused with `Combat/ShipMagazineAtb` (the auto-resolver's ABSTRACT kg ammo-pool — Combat gotcha #1). Gauge: `OrdnanceMagazineTests`. |
...
**S14 completes it for the SIEGE railgun** — the same `+ Max(0, MuzzleVelocity-50000)/1000` term on the `siege-railgun` template, with the base-mod `default-design-high-velocity-siege` (Muzzle Velocity 250,000, +200 mass). Anchored at the 50,000 baseline → the stock siege design and the Bombard that mounts it are byte-identical. Gauged by `ShipHighVelocitySiegeTests`. **This closes "velocity is earned" for ALL ballistic mass-drivers (railgun + siege).** The full free-dial audit of the whole designer (what's costed / what's truly-free-and-why-left) is the ledger in `docs/archive/economy/COMPONENT-DESIGNER-DIALS.md §0d′` — the transparency-law sweep is substantially complete after S8–S14; flak Muzzle Velocity is truly free but left (saturation-dominated, marginal).

> **DEAD-KNOB deletion (2026-08-19, developer "delete the dead knobs").** Distinct from the free-dial audit above (that's *costed-vs-free*; this is *read-by-nothing*). Deleted `laser-weapon.ReloadRate` (the "cooldown-cut" — a settable `GuiSelectionMaxMin` dial that misled the designer; the reload rate is driven by `Charge Period`, `ReloadRate` fed no atb-arg and no formula) plus the debug readouts `laser-weapon.Beam waist` and `missile-launcher.Power Efficency` — none read by any `AtbConstrArgs`/Mass/derived formula, none overridden by any design (so no `ComponentDesignFromJson` unguarded-index crash). Also removed `rtg.Watt per kg` + `solarArray."Area "` (a trailing-space duplicate of the live `Area`) in energy.json. **KEPT `fuel-cargo-hold.Dry Weight`** — it looked dead but FEEDS that template's `Mass` formula (`"Mass": "PropertyValue('Dry Weight')"`), so deleting it would crash New Game.
