...
- **The genuinely-free remainder shares the "settable mass-dial" pattern — needs a DESIGN DECISION, not a data slice**: **ground-weapon `Attack`** (component Mass = the settable `CarryMass` dial — see `GroundCombat/CLAUDE.md` "Flagged design question") AND the exotic drives (**reactionless `Thrust`**, **inertialess `Evasion Override`** — component Mass = the settable `Drive Mass` dial). In all three, the impactful dial is free because Mass is an INDEPENDENT settable dial a designer could override low, re-opening the free win. Coupling the impactful dial to the mass (a floor, or removing the independent mass dial) is an engine change + a deliberate cost-curve rebalance — ONE decision covers all three. Deferred to the developer; see the `GroundCombat/CLAUDE.md` note for the three options.
