...
| `EarthTerrainMap.cs` | **NEW (Sol playtest, 2026-07-06)** The **real Earth surface** — a baked, hand-authored equirectangular biome map (72×36) of our actual planet (Americas + Andes, Sahara, Congo, Himalaya, Siberia, India, SE Asia, Australia, both ice caps). `Sample(lon,lat)` returns the terrain at a fractional position (**row 0 = North pole**, **col 0 = 180°W** so the wrap seam falls on the mid-Pacific — matching the client's top-down draw + the engine's lat=0→north / lon-wraps convention). It's DATA, not physics (authored by `scratchpad/gen_earth.py`). `WorldTerrain.ForBody` routes a body whose `NameDB.DefaultName == "Earth"` (gated to a non-gas terrestrial) to this map instead of the noise field, at the **same** sample points — so the cylinder grid, region disks, hex terrain and client map keep their shape, only the terrain changes. Fixed the "Earth is a joke as far as how the map looks" report. Gauge: `Pulsar4X.Tests/EarthTerrainMapTests.cs` (well-formed shape + poles-ice / Pacific-ocean / continents-land / Earth-like mix). Edit the strings to tune (keep each row exactly `Cols` chars — `IsWellFormed` is the test's guard). |
