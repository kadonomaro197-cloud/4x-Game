...
| `PlanetHexFactory.cs` | **NEW (Hex ground H1, 2026-07-04)** Generates a body's hex patches **LAZILY** — `EnsureHexesForBody(body)` builds a hex DISK per region (radius `HexPatchRadiusFor(planetRadius)` — Earth→12≈469 hexes/region, scales linearly, clamped [2,24]), terrain per hex drawn from the region's `Features` mix. Idempotent, deterministic (system RNG), defensive. **Only theatres carry hexes** (hooked at `ColonyFactory` colony creation) so a galaxy isn't bloated with millions. Pure `HexPatchRadiusFor`/`HexDiskCount` unit-tested. Gauge: `PlanetRegionsTests.Hex*`. |
