...
| SE-2 | the Command Berth as a real component (Role/Grade/Support/Survivability/Span) + the posting-danger incident roll | 🔨 building (SE-2a done — the berth gear) |
...
| `CommandBerthDB.cs` | **SE-2a (2026-07-13)** The berth GEAR — one host's roster of installed Command Berths (`List<CommandBerth>`; a host can carry several, like an Enterprise-D). Each `CommandBerth` carries the §5 dials — `Role` (which `SiteRole` it works), `Grade` (work-rate tier), `Support` (leader competence boost), `Survivability` (incident-risk reduction), `Span` (force size) — plus its `ComponentName` identity. `BestBerthFor(role)` = the highest-Grade matching berth (the read SE-2b uses). Occupancy (who is seated) is deliberately NOT here — that reuses the existing admin-seat plumbing (SE-2b), not a parallel one. `[JsonProperty]` + deep-copy `Clone()`. Nothing reads it yet → byte-identical. Gauge: `CommandBerthBaseModTests`. |
