...
| SE-4 | the incident shape (Europa) — bleeds-you pressure + hold-while-you-work + a menace faction + the spawn event | 🔨 building (SE-4a incident state + SE-4b menace factory done; SE-4c pressure next). SE-4a: `FieldSiteDB` incident dials + `SiteMachine.IsIncidentLive`/`CurrentPressure` (`IncidentMachineTests`). SE-4b: `MenaceFactory.RaiseMenaceAt` (`CreateBasicFaction`+`RaiseUnit`) stands up a hostile menace faction on the site's region → trips the SE-3d guardian gate (`MenaceFactoryTests`). Plan: SE-4c pressure via a site-owned `RegionEnvironment` (reuse `GroundForcesProcessor` attrition), SE-4d spawn/spread `IInstanceProcessor` (`AddEntityInterupt`) + `EventType`, SE-4e discovery wire (flag-gated). |
