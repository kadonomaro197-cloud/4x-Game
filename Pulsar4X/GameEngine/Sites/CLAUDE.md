...
| SE-4 | the incident shape (Europa) — bleeds-you pressure + hold-while-you-work + a menace faction + the spawn event | 🔨 building (SE-4a incident state + machine reads done; SE-4b menace factory next). SE-4a: `FieldSiteDB` incident dials (`MenaceFactionId`/`PressurePerDay`/`SpawnIntervalDays`) + `SiteMachine.IsIncidentLive`/`CurrentPressure`; gauge `IncidentMachineTests`. Plan: SE-4b menace factory (`CreateBasicFaction`+`RaiseUnit`), SE-4c pressure via a site-owned `RegionEnvironment` (reuse `GroundForcesProcessor` attrition), SE-4d spawn/spread `IInstanceProcessor` (`AddEntityInterupt`) + `EventType`, SE-4e discovery wire (flag-gated). |
