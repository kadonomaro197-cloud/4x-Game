...
| SE-4 | the incident shape (Europa) — bleeds-you pressure + hold-while-you-work + a menace faction + the spawn event | 🔨 building (SE-4a incident state + SE-4b menace factory + SE-4c pressure done; SE-4d spawn/spread next). SE-4a: `FieldSiteDB` incident dials + `SiteMachine.IsIncidentLive`/`CurrentPressure` (`IncidentMachineTests`). SE-4b: `MenaceFactory.RaiseMenaceAt` (`CreateBasicFaction`+`RaiseUnit`) stands up a hostile menace faction on the site's region → trips the SE-3d guardian gate (`MenaceFactoryTests`). SE-4c: `SiteWorkProcessor.ApplyIncidentPressure` (in `ProcessEntity`, gated on `SiteMachine.IsIncidentLive`) bleeds `CurrentPressure × days` health off every NON-menace unit in the incident's region each tick (the menace is spared — it's the source); gauge `IncidentPressureTests`. Plan: SE-4d spawn/spread `IInstanceProcessor` (`AddEntityInterupt`) + `EventType`, SE-4e discovery wire (flag-gated). |
