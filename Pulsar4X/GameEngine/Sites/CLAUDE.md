...
|-------|----------------|--------|
| **SE-1** | the engine on a space anomaly — `FieldSiteDB` + state machine + presence detection + accrue + resolve + a research yield | 🔨 building (SE-1a + SE-1b done) |
...
| `SiteWorkProcessor.cs` | **SE-1b (2026-07-12)** The DRIVER: a daily `IHotloopProcessor` keyed to `FieldSiteDB`. For each site, if a worker is present (`TryFindWorker` — SE-1b v1 = the nearest non-neutral **ship** within `PresenceRadius_m` = 1000 km; SE-2 tightens it to a Command Berth of the site's Role), it feeds one day's `WorkPerDay`/`UnderstandingPerDay` into `SiteMachine.Accrue` and records `WorkedByFactionId` (yield routes there, SE-1c). No worker → no advance (pressure, not a timer). Sleeps on an empty system → byte-identical live. Gauge: `SiteWorkTests`. |
