...
| `AuraProjectorDB.cs` + `AuraBuffDB.cs` + `AuraSweepProcessor.cs` | **E14 auras, SLICE 2 — THE SWEEP (2026-08-18).** The per-tick pass that takes a projector's field to the ships around it. **`AuraProjectorDB`** is the marker (a roster of `AuraProjectorField` snapshots — radius/magnitude/effect/target — copied from each installed `AuraAtb`, the exact `CommandBerthDB` shape); `AuraAtb.OnComponentInstallation` seeds it, `OnComponentUninstallation` drops it when the last projector leaves (the grave rung). It exists to be the `IHotloopProcessor` KEY (a fresh blob so it collides with none — gotcha L9, the `StarFlareSourceDB`-off-`StarInfoDB` precedent) and to gate empty-system sleep (L5). **`AuraSweepProcessor`** (5 s, keyed to `AuraProjectorDB`, mirrors `SpaceHazardProcessor`'s region scan) walks every `ShipInfoDB` in the system, takes the STRONGEST in-range field of each kind (never the sum — `AuraTools.BestOf`), IFF-filtered by `AuraTarget` and distance-tapered by `AuraTools.MagnitudeAt`, and records it on the ship's **`AuraBuffDB`** (Firepower/Toughness), re-derived from scratch each pass (removes the record from a ship that has left every field — the `ColonyMoraleDB` discipline). **Byte-identical:** nothing reads `AuraBuffDB` yet, no base-mod projector template, AND `AuraSweepProcessor.EnableAuraSweep` defaults OFF. **v1 = space ships only** (ground units are data objects with no `PositionDB` — a hex-distance ground sweep is deferred); only Command→Firepower and Ward→Toughness are wired (the two effects with a live combat sink). **NEXT (slice 3) = the combat-read:** fold each ship's `AuraBuffDB` into its firepower/toughness multiplier in the resolver — where the per-ship-vs-flagship design fork gets settled (the resolver reads `People.BonusesDB` today ONLY from a fleet's flagship commander, applied fleet-wide, so there is no per-ship channel yet). Gauge `AuraSweepTests`. | ✅ slice 2 (sweep + record, byte-identical) |
