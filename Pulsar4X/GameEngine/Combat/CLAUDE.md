...
| File | Purpose | Status |
|------|---------|--------|
| `ShipCombatValueDB.cs` | DataBlob: a ship's **Firepower** (joules/sec from beams + **railguns** + a missile-launcher stub), **Toughness** (live components + armour), **RoleWeight**, **Evasion** (size + agility), and a **`Weapons`** list of per-weapon flavor profiles. Computed once at build. `Calculate(Entity)` + `CalculateEvasion(Entity)`. | ✅ built (spine step 2; +Evasion/Weapons in the depth pass; +railgun P3) |
