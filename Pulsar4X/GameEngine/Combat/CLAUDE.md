...
| `FleetDoctrine.cs` | Helpers: `FirepowerMult`/`ToughnessMult`/`IsRetreat(fleet)` reads; `PostureOf`/`TargetingOf(fleet)` (ROE reads, default WeaponsFree/Balanced); `TrySetDoctrine(fleet, blueprint, now)` sets a posture from the catalog (copying `Targeting` from `CombatDoctrine.ParseTargetPriority`), honouring the cooldown. | ✅ built (spine step 5; +Targeting P5) |
