...
The **one** combat engine for this fork: a math loop that resolves fleet battles off each ship's "spec sheet," with **doctrine** as the player's only lever. Lives in `GameEngine/Combat/`. This is the v1 spine described in `docs/combat/COMBAT-DESIGN.md` -> "What we're building (v1)".

> ⭐ **START AT `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md` — the single consolidated record of this engine** (it replaced eight design docs on 2026-07-29). It holds the as-built anatomy, every locked ruling, the build ledger for all six tracks, the **14-row scenario matrix** ("does it handle X?"), the **ship-vs-ground divergence table**, the open developer decisions, and a **stale-claim register**. This file is the *file map*; that doc is the *engine's ground truth*. Where they disagree, verify in source and fix both.
...
> **Status: extensively built and wired — runtime unverified.** The spine is all here in code and CI-tested: the shared `CombatKernel`, the live multi-party stepped resolver, doctrine, retreat, the engagement lock, eight combat component `*Atb`s (shield / magazine / radiator / point-defense / unit-caliber / crew-automation / armour-hardening / inertialess-drive) each with full six-point base-mod registration, and shield / ammo / heat / point-defense / armour-nature all folded into the resolve. Built piece-by-piece, each under a test, in the order in `docs/combat/COMBAT-DESIGN.md` -> "Build order"; this file still grows as each new piece lands. **"Built and wired" here means the code exists and CI's engine tests pass — it does NOT mean the live client has been runtime-verified (CI can't run the client). What genuinely remains unbuilt is called out per-row below, and several player-facing behaviours ship gated behind a default-off flag (the client turns them on).**

> **DECIDED 2026-07-06 — this resolver becomes THE resolver (ship AND planetary).** The developer's call: *"there is no ground tactics, just tactics; there is one weapon triangle; why a separate resolver?"* Today `GroundForcesProcessor.ResolveRegionCombat` DUPLICATES this salvo/triangle/dodge/shield/armour math over data-object `GroundUnit`s (only because they aren't entities). The next combat branch **extracts the shared damage math onto a neutral COMBATANT view** both a ship entity and a planetary unit present, routes both through it, and deletes the ground duplicate. Planetary combat then contributes terrain + **absolute metric range** (a 100 km weapon is 100 km on a surface too; hexes are just the board — `GroundRangeTools.RealReachKm`) + the **air/altitude layer**; the Armor▸Infantry▸Artillery type-triangle DISSOLVES into weapon×armour matchups. It's a rewrite of the only green combat code — do it as additive CI-gated slices (kernel first, no behaviour change), with the developer's go. Full plan: `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1` (+ `docs/combat/WEAPONS-DESIGN.md` §0 pt 6).
>
> **Slices 1–2 ✅ LANDED 2026-07-08 — `CombatKernel.cs` (shared kernel; ship side wired).** The shared home for the pure salvo math now exists AND the ship resolver runs on it. Slice 1 added the neutral `CombatKernel.Combatant` view (value fields + `WeaponProfile`s + a 1-D `Position_m`, no hex/`Entity`) plus the pure functions `HitFraction` / `LandedFraction` / `SoakFractionOf` / `ResolveShield` / `ShieldSoakFraction` / `ArmourSoak`. Slice 2 **routed the ship side through it**: `CombatEngagement`'s `HitFraction` / `LandedFraction` / `SoakFractionOf` / `ResolveShield` / `ShieldSoakFraction` are now thin `=> CombatKernel.X(...)` delegators, its dodge/shield tuning constants forward to the kernel's, and `RangeBaseMiss` is a forwarding property — so there is ONE definition of the ship dodge/shield arithmetic and no drift is possible. Byte-identical by construction (delegation to identical code); the ship combat fixtures (CombatPerformance / Dodge / Shield / Triangle / Stress / BattleSims) are the tripwire. **Slice 3a ✅ LANDED — the shared armour math.** `GroundDamageMatrix`'s `ArmourSoak` + its `ArmourSoakPerPoint` / `ArmourMinPassFraction` constants now delegate to `CombatKernel` (`GroundDamageMatrix.cs:45,49,93-117`), and `ResolveRegionCombat` already routes its per-unit fire through the kernel's `HitFraction` / `ShieldSoakFraction` / `ArmourSoak` / `BurstShotCount` (`GroundForcesProcessor.cs:340-363`, via `GroundCombatant.ToWeaponProfile`) — so the ship and ground sides share ONE definition of the salvo/soak arithmetic, and `ArmourSoak`'s duplication has collapsed. **Still to come:** giving a `GroundUnit` a full `Combatant` view and dissolving the remaining Armor▸Infantry▸Artillery type-triangle into pure weapon×armour matchups — that ground behaviour change is the deliberate one, to be re-baselined with a written reason.
...
| File | Purpose | Status |
|------|---------|--------|
| `CombatKernel.cs` | **NEW (resolver merge, slices 1–2, 2026-07-08)** The shared, domain-neutral salvo KERNEL both a ship battle and a planetary battle run through. `CombatKernel.Combatant` = the neutral view (FactionId · Health/MaxHealth · Evasion · Armour · Shield pool/capacity/regen · `List<WeaponProfile>` Weapons · 1-D `Position_m`) — no hex, no `Entity`. Pure static math: `HitFraction`, `LandedFraction`, `SoakFractionOf`, `ResolveShield`, `ShieldSoakFraction` + the ground `ArmourSoak`. **SHIP side WIRED as of slice 2** — `CombatEngagement`'s same-named helpers are now thin delegators to this class and its tuning constants forward here, so the kernel is the single source of truth for the ship dodge/shield math (byte-identical; the ship fixtures are the tripwire). `ArmourSoak` is the shared flat-armour math for ship + ground (slice 3a). **+ PENETRATION (Weapons pilot W1a, 2026-07-10):** a 3-arg overload `ArmourSoak(armour, sourceDamage, penetration)` — penetration cancels armour point-for-point BEFORE the flat soak (an AP round with penetration ≥ armour lands in full; a normal round, penetration 0, is byte-for-byte the old 2-arg soak, which now forwards to it). Clamped so penetration can't make armour stronger. Byte-identical this slice (nothing passes penetration > 0 yet; the ground resolver wires it in W1b). **+ PER-SHOT-ENERGY BURST SOAK (W2a, 2026-07-10):** `BurstShotCount(w)` (= dps÷`WeaponProfile.PerShotEnergy`, clamped [1, `BurstSoakMaxShots` 1000]) + `ArmourSoakBurst(armour, dmg, shotCount, penetration)` split a source into N equal shots and soak each flat — the alpha-vs-chip identity (one big shot punches, a swarm bounces). shotCount ≤ 1 forwards to the flat soak (byte-identical); W2b routes the ground resolver through it. Gauge `CombatKernelTests`. Design: `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.1` + `docs/archive/economy/COMPONENT-DESIGNER-DIALS.md` ⚙1. **+ SHARED RANGE GATE (UNIFIED Slice 1, 2026-07-22):** `WithinReach(reach, gap)` = the convention-free core `gap <= reach` (same units — metres in space, hexes on the ground) + `WeaponReaches(WeaponProfile w, double separation_m)` = the space per-weapon gate layering the `Range_m <= 0 => unbounded` beam convention on top. **The reach-0 conventions genuinely DIFFER (space beam = unbounded, ground melee = contact-only), so the shared piece is the bounded `WithinReach`, not one unbounded predicate.** Space's `BuildFireMix` gate (`CombatEngagement.cs:1347`) and ground's `GroundForcesProcessor.WeaponReaches` (both branches) now route through it — "can this weapon reach that target" decided in ONE place for space AND ground, byte-identical. Gauge `CombatKernelTests.RangeGate_SharedReachPredicate_PinsBothConventions`. | ✅ slices 1–2 (ship wired) + Penetration W1a + PerShotEnergy W2a + shared range gate |
| `CombatConditions.cs` | **NEW (OPERATION BLUEPRINT-TO-STEEL Phase E-env, slice 1, 2026-08-16)** The "environment a battle is fought in" — a small value struct of ×multipliers (+ ambient DoT J/s) so a fight is shaped by WHERE it happens (a nebula cuts detection/accuracy/closing and corrodes; clean space is identity). The space-side mirror of what GROUND combat already reads off terrain + `PlanetEnvironmentsDB`. `CombatConditions.FromHazard(HazardModifiers)` is the pure translation of the live `Hazards.SpaceHazardTools` query (SensorJam→Detection+Accuracy, MovementDrag→Closing, hazard damage→AmbientDoT; blind→0); `ReadAt(system, position)` is the whole "wire" the design's Part 2 named as the gap. **Slice 1 = the value object + reader ONLY — NO combat caller, byte-identical.** A later slice stores it on `FleetCombatStateDB` and reads the accuracy coefficient into the shared `CombatKernel.HitFraction` (the keystone: one edit moves BOTH resolvers) + the space firepower/shield hooks. Plain struct (derived per battle, not saved → no `[JsonProperty]`/`Clone`). Gauge `CombatConditionsTests`. Design: `docs/combat/ENVIRONMENT-CONDITIONS-DESIGN.md`. **+ Slice 2a (2026-08-16, byte-identical foundation):** the KEYSTONE hook landed — `CombatKernel.HitFraction`/`LandedFraction` gained an optional `accuracy` coefficient (default 1.0 → `hit *= 1.0` exact → byte-identical; ONE edit both resolvers call), and `FleetCombatStateDB.Conditions` (a `CombatConditions` field) is SEEDED TO `Clean` via a field initializer (the struct-default trap guard — covers every ctor + copy-ctor + legacy saves). Nothing seeds/reads it yet. **+ Slice 2b DONE (2026-08-17, the LIVE wire):** `CombatEngagement.EnableCombatConditions` (static bool, default OFF, `PulsarMainWindow`-ON). `StartEngagement`/`EnsureInCombat` seed each fleet's `Conditions = CombatConditions.ReadAt(system, fleetPos)` (via the null-safe `TryGetFleetPosition`), and `ApplyCasualties` reads `state.Conditions.Accuracy` into `LandedFraction`→`CombatKernel.HitFraction` (`hit *= accuracy`) — so a fight INSIDE a gas cloud/nebula lands fewer shots (poor visibility). **Flag-off → `accuracy = 1.0` exact → byte-identical** (every combat fixture fights in clean space). Gauge `CombatConditionsWiringTests` (seed reads the hazard in a real gas cloud; a cut-accuracy defender takes LESS fire than in clean space; flag-off byte-identical). **+ Slice 3 DONE (2026-08-19, the remaining coefficients):** `StepEngagementGroup` now threads all four — (a) **AMBIENT DoT** (`Conditions.AmbientDoT_Jps × dt × shipCount`) grinds every ship each step, **undodgeable** (added to `DamageTakenPool`, NOT scaled by `SalvoDamageScale`), and it runs even on a fleet with **no attacker** on it this step (the old bare `continue` skipped that — a one-sided aggressor / a fleet alone in the murk now still takes environmental losses, via an empty-fire `ApplyCasualties` → `LandedFraction` 1.0); (b) the **ATTACKER's `Firepower`** scales its outgoing fire in the `AddScaledFire` split (a hot corona chokes beams); (c) the **DEFENDER's `ShieldRegen`** scales the regen passed to `ResolveShield` in `ApplyShield` (an ion storm won't let shields recharge); (d) the **DEFENDER's `Cover`** adds to each ship's evasion in `ApplyCasualties` (debris/limb), capped at `ShipCombatValueDB.EvasionCap`. **Firepower/ShieldRegen/Cover are all identity (1.0/1.0/0) from the raw hazard query today → byte-identical until an authored-environment fills them; ambient DoT is the one live-from-a-hazard behavior** (a gas cloud's `DamagePerSecond`). All flag-off → exact identities → byte-identical. Gauge `CombatConditionsWiringTests` (5 slice-3 cases: ambient kills faster / the no-attacker murk path / choked firepower kills slower / cover dodges more / suppressed shield-regen kills faster — each flag-off byte-identical). | ✅ slice 1 + 2a + 2b + 3 (all coefficients wired, flag-gated byte-identical) |
| `ShipCombatValueDB.cs` | DataBlob: a ship's **Firepower** (joules/sec from beams + **railguns** + **flak** + a missile-launcher stub), **Toughness** (live components + armour), **RoleWeight**, **Evasion** (size + agility), and a **`Weapons`** list of per-weapon flavor profiles. Computed once at build. `Calculate(Entity)` + `CalculateEvasion(Entity)`. **+ `AmmoCapacity_kg` (W3 — sum of `ShipMagazineAtb`, seeds the fleet ammo pool). + RECOIL→tracking (W4 — a kinetic weapon's `Recoil` reduces its built `WeaponProfile.Tracking` by `RecoilTrackingFactor(recoil, chassisMass)` = `mass/(mass+recoil)`; a heavy gun on a light hull tracks worse. Recoil 0 on every base-mod weapon → byte-identical; gauge `ShipRecoilTests`).** **+ INERTIALESS evasion (Propulsion ⚙2 · P1 — `CalculateEvasion` reads `InertialessEvasionFloor` = the best installed `Combat.InertialessDriveAtb.EvasionOverride`, health-scaled, as a floor: a heavy capital dodges like a fighter; 0 if none → byte-identical; gauge `ShipInertialessDriveTests`). + FIRE-CONTROL→beam-tracking (Sensors ⚙3 · S1 — `Calculate` raises each beam `WeaponProfile.Tracking` toward 1.0 by the best installed `Weapons.BeamFireControlAtbDB.TrackingSpeed`, `FireControlTrackingFactor(ts)=ts/(ts+5000)`, health-scaled = grave rung; gated behind the default-off `EnableFireControlTracking` flag (client-on) since the director already lives on the base-mod warships → byte-identical off; gauge `ShipFireControlTests`).** | ✅ built (spine step 2; +Evasion/Weapons; +railgun P3, +flak P4; +AmmoCapacity W3; +Recoil W4; +Inertialess P1; +FireControl-tracking S1) |
| `WeaponProfile.cs` | The per-weapon flavor: `WeaponClass` enum (Beam/Railgun/Missile/Flak) + `WeaponProfile` (damage/velocity/tracking/saturation/**`Range_m`**). **+ TWO-AXIS foundation (2026-07-06, `docs/combat/WEAPONS-DESIGN.md`):** `WeaponNature` (Kinetic/Energy/Explosive/Exotic — meets the defence) × `WeaponDelivery` (Beam/Bolt/Slug/Cloud/Guided/Blast — meets the dodge) as separate `WeaponProfile.Nature`/`.Delivery` fields; the fused `WeaponClass` is transitional and will become a COMPUTED readout of the specs (so a build can sit between corners; a blaster = Energy+dodgeable-Bolt becomes expressible). Additive — `WeaponClass` + all combat is byte-identical this slice; base-mod beam/railgun/flak/missile tagged Energy·Beam / Kinetic·Slug / Kinetic·Cloud / Explosive·Guided. Gauge `WeaponAxesTests`. The breakdown the dodge model + weapon triangle read. **+`Range_m` (Root A, 2026-06-27)** — how far the weapon reaches (0 = unbounded, the beam `IsInRange` convention). **Authentic-closing ranges (2026-06-27):** beams carry their design `MaxRange` (~knife range); **flak = short** (`FlakRange_m` 50 km, point-defense hard cutoff); **missile = long** (`MissileRange_m` 1000 km, the standoff opener); **railgun = MID** (`RailgunRange_m` 500 km, between flak and missile — **finite as of 2026-06-28**, was 0/unbounded). That gives the closing fight its range LAYERING: missile (1000 km) → railgun (500 km) → flak (50 km) → beam (knife). **Why railgun went finite:** rangeless (0) meant `MaxReach` returned `PositiveInfinity` → a railgun fleet was "IN RANGE" at ANY gap, so it fired across the whole 1 Gm engagement bubble — the live "ships firing at each other outside their detection range" report (2026-06-28). A finite range makes the closing model hold its fire until the gap is within 500 km (FAR inside any sensor reach), so a slug only lands on a target the ship has closed with. Accuracy still falls off with distance within that range (the `HitFraction` range term). Flak/missile/railgun ranges are v1 class-defaults in `ShipCombatValueDB`; per-design (paid-for) range fields are the follow-up. **Tests stay green** because the range gate only bites when `EnableClosingRange` is on (live client) — with it off (every headless fixture) `SeparationOf` returns 0, so the gate is a no-op (`RailgunWeaponTests` asserts the finite range; the resolve fixtures are unchanged). The data the closing-fight model reads (`docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.4`). **+`Penetration` (Weapons pilot W1a, 2026-07-10)** — the armour-crack dial (how much of the target's flat armour the shot IGNORES); 0 = a normal round, high = an AP/sabot/lance cracker. Read by the kernel's 3-arg `ArmourSoak`. **+`PerShotEnergy` (Weapons pilot W2a, 2026-07-10)** — the alpha-vs-chip dial (joules in one shot); the kernel's `BurstShotCount` (= dps÷PerShotEnergy) + `ArmourSoakBurst` split a source into that many flat-soaked shots, so a swarm of chips bounces off plate while one alpha of equal total punches through. Both serialized + carried through both ctors + the copy-ctor; **additive & byte-identical** (default 0 → single lump, and nothing calls the burst soak from live code yet — W2b wires the ground resolver). `⚙1` resolver backlog #1/#2 (`docs/archive/economy/COMPONENT-DESIGNER-DIALS.md`). | ✅ built (depth pass P2; +Range_m Root A; +authentic ranges; +Penetration W1a; +PerShotEnergy W2a; +HeatPerSecond W5b — per-weapon waste-heat, throttles energy fire over the fleet's radiator capacity, default 0 = byte-identical) |
| `ParametricWeaponAtb.cs` (`Weapons/`) + `parametric-weapon` template | **NEW (OPERATION BLUEPRINT-TO-STEEL Phase C / Path B, B1a+B1b, 2026-08-23) — the GENERIC parametric weapon (the HTML weapons-designer collapse).** ONE buildable weapon component carrying Delivery × Nature + the numbers (dps/saturation/range/velocity/tracking/penetration/perShotEnergy/heat); `ShipCombatValueDB.Calculate` reads it via `ParametricWeaponAtb.BuildProfile(healthPercent)` → runs the fidelity-proven `WeaponsDesignModel` → the SAME `WeaponProfile` the per-type guns produce, so ONE form reproduces every gun (laser=Beam×Energy, railgun=Slug×Kinetic, flak=Cloud×Kinetic, plasma=Bolt×Energy, disruptor=Beam×Exotic, missile=Guided×Explosive). Save-safe (new atb, mirrors `RailgunWeaponAtb`'s parameterless+10-double+copy ctors; enum args as doubles cast back, the `AuraAtb` pattern). **B1b:** base-mod `parametric-weapon` template (10 dials incl. Delivery/Nature `GuiEnumSelectionList`) + `default-design-parametric-laser` ComponentDesign (defaults = a Beam/Energy laser) six-point registered at Earth (StartingItems + ComponentDesigns). **Additive / byte-identical:** no base-mod ship mounts one, so combat is unchanged; this is the new DESIGN path. Gauge `ParametricWeaponTests` (JSON→atb gotcha-10 sensor + the per-delivery regime collapse). NEXT: B2 = the client Delivery×Nature designer screen; B4 (retire the 11 per-type templates) is developer-decision-gated. | ✅ B1a + B1b (engine + buildable, byte-identical) |
...
| `GroupPlane.cs` | **NEW (2D group-plane resolver, slice S0, Operation Earthfall T1.1)** Pure-static "invisible battle graph-paper" math (`docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §13` §13). `SeedFrame` lays a battle-local 2D `BattleFrame` (Origin + orthonormal XAxis/YAxis) down ONCE from the fighters' real 3D positions — deterministic basis (seeds sorted by id; XAxis = lowest-id→centre; YAxis = widest ⟂ spread, id tie-break; degenerate fallbacks, never throws). `Project` flattens a 3D position onto the FROZEN frame (a joiner uses the stored axes, so gaps don't jump as ships die). `EnemyDirection` gives the nearest-enemy facing with a lowest-id tie-break. `RoleOffset(enemyDir, bearingDeg, alongStandoff, perpSpread)` = the doctrine nudge as pure trig (0°=at enemy / ±90°=flank / 180°=rear; −standoff kites; perpSpread fans). `PairDistance` = the single scalar the plane hands the unchanged 1-D `CombatKernel`. **S0 has NO caller** (byte-identical); S1 seeds anchors in `FleetCombatStateDB`, S2 the 2D range gate. `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §13.7/§13.8` pins the fire-allocation + combined-theater joints that gate the later S5/S6. Gauge `GroupPlaneTests`. | ✅ S0 (pure math) + S1/S2 wired (flag-off) |
| `FleetCombatStateDB.cs` | DataBlob marking a fleet as **engaged** (a *representative* opponent id for readout, accumulated damage pool, steps fought, starting ship count for the retreat threshold). On **every** fleet in a battle (state is per-fleet); removed from a fleet when it leaves. Its presence is the "in combat" flag the engagement lock (step 11) keys on. **+ 2D group-plane (Operation Earthfall T2.1/T3.1, slices S1+S2, default-off `EnableGroupPlane`):** `HasFrame` + the frozen `BattleFrame` (`FrameOrigin`/`FrameXAxis`/`FrameYAxis`, three loose Vector3s — the `FleetRetreatDB.RetreatVector` save pattern) + `Anchor` (this fleet's 2D point) + `GroupPositions` (`List<Vector2>`, S1 = one whole-fleet group). Seeded at engagement start / copied to joiners; slid by `AdvanceClosing`; **READ by the S2 range gate** (`SeparationOf`/`WithinWeaponRange` → `GroupPlane.PairDistance` of two anchors). All `[JsonProperty]` + deep-copied; inert unless the flag is on → byte-identical scalar path. Gauges `EfGroupPlaneAnchorTests`/`EfGroupPlaneRangeGateTests`. | ✅ built (spine steps 4, 7; multi-party; +2D anchors S1/S2) |
| `CombatEngagement.cs` | The trigger's engine logic: `Tick(manager, dt)` engages/JOINS hostile fleets in range, then steps the **multi-party** engagement (every in-combat fleet in the system, sides = factions) incrementally over game-time. `StepEngagementGroup(members, dt)` is the resolver; `StepEngagement(a,b,dt)` is its n=2 special case; `EnsureInCombat` is the join primitive. `GetCombatShips` tags ships with their **component's** doctrine mults (step 6); `GetFleetShips` is the flat list (counts/detection). Hostility/range/sides are v1 stubs. Testable directly. **B-2c1: `IsSubFleet(fleet)` (internal) + TOP-LEVEL-only enrolment** — `Tick`/`NewEngagementImminent`/`OrderAttackNearestHostile` now drop sub-fleets (a fleet whose parent is another FLEET, not the faction root that carries `FactionInfoDB`) so a role sub-fleet is NOT enrolled separately — its ships fight via the parent's recursive `GetCombatShips`, which already applies each sub-fleet's own doctrine (enrolling it separately would DOUBLE-COUNT). No sub-fleets exist in a default game → byte-identical. **FROZEN-CLOCK WEDGE FIX (2026-07-16, from the developer's SIM-STALL log):** `GetFleetShips`/`GetCombatShips` (and `FleetTools.AllShipsRecursive`) now carry a **seen-set** — each fleet node is walked at most ONCE. A malformed fleet tree (a diamond: a sub-fleet reachable by two paths, or a cycle) previously re-explored the same subtree combinatorially; the depth-64 cap bounded DEPTH but not PATHS, so a few diamonds exploded into millions of calls and WEDGED the combat hotloop (game-time froze, `in-combat=0`, the watchdog's "TRUE WEDGE"). The seen-set makes the walk O(nodes), immune to any cycle/diamond. Gauge: `CombatFleetTreeSafetyTests` (diamond counts a shared ship once; cycle terminates — both `[Timeout]`-guarded). **WEDGE FIX COMPLETED — cradle-to-grave (2026-07-16 round 2, the developer re-hit the freeze):** the seen-set was applied to `GetFleetShips`/`GetCombatShips`/`FleetTools.AllShipsRecursive` but **MISSED `FleetCombat.Ships` (`FleetCombat.Collect`)** — the walker the WHOLE closing model (WarpSpeedFloor/FirepowerAtRange/SensorReach/ShieldCapacity/…) AND `FleetAssembly.ArmedShipCount` ride, run INSIDE this Tick → it was still the "StarInfoDB TRUE WEDGE" path. Now guarded (seen-set + depth cap), plus `TreeHierarchyDB.TryGetChild` (used by `ShipFactory.DestroyShip`). **The malformation SOURCE is fixed too:** it was NOT the Disband smell (that only makes a transient SHIP diamond — a leaf, harmless double-count, self-heals) — it was **`FleetOrder.ChangeParent` with no descendant check** (the client fleet drag-drop could drop a parent fleet onto its own sub-fleet → a 2-node parent cycle); `ChangeParent` now refuses a reparent-under-descendant (`WouldFormCycle`, bounded), and Disband got a `RemoveChild`-before-`AddChild` for correctness. **The captured freeze itself was the PERF thrash, not the wedge:** `EnsureInCombat` seeded the resolver gap to the 1000 km default while the engage gate had approved the pair on their REAL distance → every finite-range weapon read out-of-range → empty fire → the fight was released "frozen" the same tick → re-engage → re-freeze → `NewEngagementImminent` stayed true → the fine-stepper re-armed every game-hour (the SensorScan crawl). `EnsureInCombat` now seeds the **real** `FleetSeparation` to the representative opponent (inside `if (EnableClosingRange)` → inert in CI → byte-identical), so the fight resolves and the pair stops reading imminent. New gauges in `CombatFleetTreeSafetyTests` (cycle/stacked-diamond terminate the FleetCombat.* readouts; ChangeParent refuses the cycle; the fine-step is bounded across advances). **+ 2D GROUP-PLANE (Operation Earthfall T2/T3, slices S1+S2, behind `EnableGroupPlane` default-off):** `StartEngagement`/`EnsureInCombat` seed the frozen `GroupPlane` frame + per-fleet `Anchor` on `FleetCombatStateDB` (joiners COPY the frozen board); `AdvanceClosing` slides the controller's anchor along `GroupPlane.EnemyDirection` by the scalar-gap delta ("the faster side closes"); **S2:** `SeparationOf` + the entity-level `WithinWeaponRange` now measure the 2D anchor pair-distance (`GroupPlane.PairDistance`) instead of the shared scalar when the flag is on AND both fleets are framed — so "long-range fires, short-range can't answer" falls out of the existing per-weapon range gate per-fleet-pair. `SeparationOf` is STILL gated on `EnableClosingRange` first (0 → no gating); the new `RangeBetween` falls back to the real 3D `FleetSeparation` when a fleet isn't framed, so the live engage-trigger (pre-seed) is unchanged and every flag-off fixture is byte-identical. The damage kernel stays 1-D (it receives the scalar `d`). S3+ (role geometry, ground-onto-plane, combined theater, multi-party) are a LATER campaign. Gauges `EfGroupPlaneAnchorTests`/`EfGroupPlaneRangeGateTests`. | ✅ built (spine steps 4, 6; multi-party; B-2c1; wedge + perf-thrash fix; +2D group-plane S1/S2 flag-off; +E-env slice-3 folds — ambient-DoT/firepower/shield-regen/cover, flag-off byte-identical) |
...
| `FleetDoctrineDB.cs` | DataBlob: a fleet's **active** combat posture (doctrine id, firepower/toughness/speed multipliers, retreat flag, weapons-release `Posture`, switch-cooldown clock). Read by the engagement as a strength/toughness modifier. **+ `Targeting` (Phase 5 ROE — TARGET PRIORITY, wired 2026-08-04):** a `TargetPriority` deciding WHO the fleet shoots first, copied from the catalog by `TrySetDoctrine`, read by `ApplyCasualties`. Default `Balanced` = the legacy spread-by-health order → **byte-identical** for every un-doctrined fight. New `[JsonProperty]` + copy-ctor line (additive, save-safe). | ✅ built (spine step 5; +Posture P3; +Targeting P5) |
| `FleetDoctrine.cs` | Helpers: `FirepowerMult`/`ToughnessMult`/`IsRetreat(fleet)` reads; `PostureOf`/`TargetingOf(fleet)` (ROE reads, default WeaponsFree/Balanced); `TrySetDoctrine(fleet, blueprint, now)` sets a posture from the catalog (copying `Targeting` from `CombatDoctrine.ParseTargetPriority`), honouring the cooldown. **Direct setters (no cooldown, work mid-battle, mutate the blob in place or create a neutral one): `SetEngagementPosture(fleet, posture)` and `SetTargeting(fleet, priority)` — the latter added by OPERATION BLUEPRINT-TO-STEEL B-orders (route the "Set Target Priority" order), the sibling of the posture setter; byte-identical until called. Gauge: `FleetDoctrineTests.SetTargeting_SetsCreatesAndPreservesPosture`.** | ✅ built (spine step 5; +Targeting P5; +direct SetTargeting) |
...
| `ShieldAtb.cs` | **NEW (space SHIELD layer, 2026-07-06)** The shield-generator **component** (`IComponentDesignAttribute`; `Capacity_J` pool + `RegenRate_Jps`, double-arg NCalc ctor L7) — the space "shield" on the defence axis (`docs/economy/UNIVERSAL-ASSEMBLY-DESIGN.md` §2b), a depleting/regen POOL (option B). `ShipCombatValueDB.Calculate` sums installed generators (health-scaled) into `ShieldCapacity_J`/`ShieldRegen_Jps` — **0 if none → additive, combat byte-identical**. Cradle-to-grave (a shot-off generator drops the shield). **Phase A = the component + the combat-value hook. Phase B (below) = the resolver wiring.** Gauge `ShieldTests`. | ✅ A + B |
...
| `AuraTools.cs` + `AuraAtb.cs` | **NEW (OPERATION BLUEPRINT-TO-STEEL E14 — AURAS, Phase A, 2026-08-18)** An **aura** is a field a component projects onto nearby units — a commander's rally, a synapse/psionic ward, a jamming bubble. **`AuraAtb`** is the buildable PROJECTOR **component** (`IComponentDesignAttribute`, CONVENTIONS §6 — designed/researched/built/mounted/lost like any part); it mirrors `ShipMagazineAtb`'s save-safe shape EXACTLY (parameterless ctor + one 4-double NCalc ctor L7 + `[JsonProperty]` dials `Radius_m`/`Magnitude`/`Effect`/`Target` + a real `Clone` + inert install/uninstall). Two enums: `AuraEffect { Rally, Dread, Command, Jamming, Ward }` (WHAT it does) × `AuraTarget { Friends, Foes, Everyone }` (WHO it lands on). **`AuraTools`** is the PURE math the sweep will call — `InRange(centre, target, radius_m)` (edge-inclusive; non-positive radius reaches nothing), `MagnitudeAt(base, dist_m, radius_m)` (FULL at centre → linear taper → 0 at/beyond the edge), `BestOf(magnitudes)` (the **take-the-BEST-not-SUM** guard-rail — two rally beacons don't stack; null/empty → 0). **Phase A = the component + the pure math ONLY, byte-identical and NOT swept** (the GroupPlane-S0 / CombatConditions-slice-1 pure-foundation pattern): nothing reads it, no base-mod template (deferred to dodge gotcha-10 JSON drift). ⚠ Rally/Dread need a unit-morale field that doesn't exist yet — Command/Ward feed the read-time buff channel (Firepower/Toughness), Jamming the detection channel; Phase A wires no effect, so that blocker doesn't bite here. Gauge `AuraToolsTests`. | ✅ Phase A (component + pure math, byte-identical) |
| `AuraProjectorDB.cs` + `CombatEngagement.FleetAuraMult` | **E14 auras, SLICE 3a — THE FLEET-WIDE COMMAND BUFF (Fork B, 2026-08-18).** The developer's call: an aura is a **flagship/fleet-wide command buff** (and, next slice, battalion-wide on the ground), NOT a per-ship radius field. **`AuraProjectorDB`** is the per-ship roster (a list of `AuraProjectorField` snapshots — magnitude/effect/target — copied from each installed `AuraAtb`, the exact `CommandBerthDB` shape); `AuraAtb.OnComponentInstallation` seeds it, `OnComponentUninstallation` drops it when the last projector leaves. **`CombatEngagement.FleetAuraMult(fleet, category)`** scans a fleet's ships (`GetFleetShips`) for that roster and folds the STRONGEST projector's magnitude (never the sum — `AuraTools.BestOf`) into the fleet-wide multiplier, right beside the flagship-commander multiplier: `cmdrFire = FleetCommanderMult × FleetAuraMult` in `GetCombatShips` (Command→Firepower, Ward→Toughness; a `Foes`-targeted field doesn't buff its own fleet). **Byte-identical:** `EnableAuraCommandBuff` defaults OFF → `FleetAuraMult` returns 1.0 (and doesn't even walk the fleet); no base-mod ship mounts an aura. **Grave rung for free:** a destroyed/uninstalled projector simply isn't found on the next combat-collect. ⚠ Rally/Dread need a unit-morale field that doesn't exist, Jamming a detection channel — so only Command/Ward are wired. **NOTE — the earlier per-ship radius sweep (`AuraSweepProcessor` + `AuraBuffDB`) was REMOVED here:** it was the Fork-A per-ship model the developer's fleet-wide call superseded (`AuraTools.InRange`/`MagnitudeAt` + `AuraAtb.Radius_m` are now LATENT — kept, pure, tested, for a future per-proximity refinement). Gauge `AuraCommandBuffTests`. **The GROUND twin is BUILT (slice 3b): `GroundCombat.GroundCommandAura` folds a battalion-wide command buff into the ground resolver — see GroundCombat/CLAUDE.md.** **+ BUILDABLE base-mod projector DONE (2026-08-19):** `aura-projector` — a ship component in `weapons.json` (`ComponentType: Augment` / `component-construction`, the SAME Magnitude/Effect/Target/Radius dials + `AtbConstrArgs(Radius,Magnitude,Effect,Target)` as the ground command-post) → `default-design-aura-projector` → mounted on the NEW **Herald Command Cruiser** (`default-ship-design-test-aura`, an Aegis-like warship + one projector), six-point registered at Earth. `EnableAuraCommandBuff` is now flipped ON in `NewGameMenu.CreateGameCore` (engine default stays OFF → CI byte-identical). So the SPACE aura is cradle-to-grave playable: design → build a projector → its whole fleet gets the buff (the ground command-post `8cc5c49` did the planet side). Gauge `AuraTemplateBaseModTests.AuraProjector_LoadsFromJson_BindsItsAtb_WithDials` (JSON→atb with the enum dials) + `AuraCommandBuffTests.RealBaseModProjector_OnAHerald_BuffsTheFleet_FlagGated` (a built Herald's install-hook-seeded roster feeds `FleetAuraMult` → +50% fleet firepower, flag-off byte-identical). | ✅ slice 3a (fleet-wide fold) + buildable projector (cradle-to-grave, client-on) |
| `RadiatorAtb.cs` | **NEW (Weapons pilot W5a, 2026-07-10)** The HEAT RADIATOR **component** (`IComponentDesignAttribute`; `Capacity_kJ`, double-arg NCalc ctor L7) — the heat twin of the ammo magazine (magazine limits KINETIC sustained fire; radiator limits ENERGY sustained fire). `ShipCombatValueDB.Calculate` sums installed radiators (health-scaled) into **`HeatCapacity_kJ`** — **0 if none → additive, byte-identical**. Sets the fleet's heat ceiling + cooling; `FleetCombatStateDB.HeatPool_kJ` (starts cold) accumulates energy-fire heat and throttles the energy guns over capacity. **W5a = component + combat-value hook + pool field (byte-identical). W5b ✅ = the heat accumulate/throttle in `StepEngagementGroup`** (`CombatEngagement.cs` ~:641-659 — energy-fire heat accumulates into `HeatPool_kJ`, radiators shed `HeatCapacity × HeatDissipationFraction`, and an overheating fleet's energy guns throttle down to `HeatThrottleFloor`; self-gating on capacity>0 so a fleet with no hot weapon is byte-identical). **W5c ✅ = the buildable base-mod radiator** — the `heat-radiator` template (Heat-Capacity dial → `RadiatorAtb`, six-point registration in `weapons.json`/`componentDesigns.json`/`earth.json`/`shipDesigns.json`) PLUS the hot `pulse-laser` (a Combat-Heat dial → `WeaponProfile.HeatPerSecond` via `GenericBeamWeaponAtb.CombatHeat_kJps` → `ShipCombatValueDB.cs:342`), both mounted on the NEW **Ember Pulse Cruiser** — so a player-built hot-beam ship carries both the heat SOURCE and the radiator SINK the W5b resolver balances (designed → built → installed → must be cooled). Gauge `ShipHeatTests` (incl. `TheEmberPulseCruiser_HasHotBeamsAndRadiators_FromJson`, the JSON→atb sensor). | ✅ W5a + W5b + W5c |
| `PointDefenseAtb.cs` | **NEW (Weapons pilot W6a, 2026-07-10)** The POINT-DEFENSE mount **component** (`IComponentDesignAttribute`; `InterceptRating_Jps`, double-arg NCalc ctor L7) — the dedicated missile-killer. A missile is a GUIDED projectile you can SHOOT DOWN on the way in (unlike a beam/slug); a fleet's total PD intercepts a **saturating** fraction of incoming `WeaponDelivery.Guided` fire (capped at `PointDefenseMaxIntercept` 0.95 — a big enough swarm always leaks). `ShipCombatValueDB.Calculate` sums installed mounts (health-scaled) into **`PointDefense_Jps`** — **0 if none → additive, byte-identical**. Pure math in `CombatEngagement`: `IsInterceptable`(=Guided) / `MissileFireDamage` / `FleetPointDefense` / `PointDefenseInterceptFraction`(pd/(pd+missile), capped). **W6a = component + combat-value hook + pure helpers (byte-identical, nothing wired). W6b ✅ = the intercept in `StepEngagementGroup`** — a per-defender PD pass in the damage phase: `InterceptMissiles(incoming, pdRating)` reduces the GUIDED (missile) fraction of a fleet's incoming fire by `PointDefenseInterceptFraction`, gated on `FleetPointDefense(ships)>0` (a PD-less fleet is byte-identical; non-guided fire never intercepted). **W6c ✅ = the buildable base-mod PD mount + a missile ship + a PD ship** — the `point-defense-mount` template (Intercept-Rating dial → `PointDefenseAtb`, six-point registration) on a NEW **Barricade PD Escort** (PD mounts + lasers), AND the previously-unregistered `missile-launcher` unlocked for the start faction on a NEW **Javelin Missile Cruiser** — so both the missile SOURCE and the PD SINK are player-buildable (new example ships, so no battle fixture is perturbed). Gauge `ShipPointDefenseTests` (the W6b end-to-end: a screened defender under pure missile fire survives where a PD-less one is ground down; + the W6c JSON→atb sensors: the Barricade reads PointDefense_Jps>0, the Javelin projects Guided fire). | ✅ W6a + W6b + W6c |
| `UnitCaliberAtb.cs` | **NEW (Enhancers ⚙6.2, 2026-07-11) — the FIRST wired dial in the previously all-dark Enhancers category.** A per-hull ELITE stamp (`IComponentDesignAttribute`; `FirepowerMult`/`ToughnessMult`, double-arg NCalc ctor L7) — a "Veteran Cadre" that multiplies the hull's Firepower AND Toughness at build. `ShipCombatValueDB.Calculate` reads the best installed module (health-scaled `1+(mult-1)×HealthPercent` → a shot-off cadre reverts to green-crew baseline, the grave rung) via `UnitCaliberFirepowerMult`/`UnitCaliberToughnessMult`. **Genuinely NEW axis, not a duplicate:** doctrine (`FleetDoctrineDB`) is a switchable *fleet* posture; commander (`FleetCommanderMult`) scales the *whole fleet* off the flagship officer; caliber is baked PER-HULL AT BUILD — the only thing that makes two identical chassis under the same admiral fight differently (the dossier's acceptance test). STACKS on both. **1.0 when no module → byte-identical** (every current ship). Cradle-to-grave: base-mod `unit-caliber` (`electronics.json`, `ComponentType: Augment`, six-point registration) on the NEW **Praetorian Veteran Cruiser** (an exact Aegis + a cadre). Gauge `ShipCaliberTests` (Praetorian out-guns an identical Aegis ×1.30; un-calibered ship reads 1.0). **SCARCITY DRAW WIRED (2026-07-11) [see CrewAutomationAtb row for the crew-reduction twin]** — the anti-dominance handle (economy×combat):** a caliber module's crew is a `ShipDesign.TalentReq` slice drawn from the colony's SCARCE `ColonyManpowerDB.TalentPool` (officers, ~0.5% of pop), not bulk workforce — so an elite hull ties up scarce veterans and can't be spammed. Split at all three crew sites (`ManpowerTools.HasTalentToBuild`/`CommitTalent`/`ReleaseTalent`): gate in `IndustryTools.ConstructStuff`, commit in `ShipDesign.OnConstructionComplete`, release in `ShipFactory.DestroyShip`. TalentReq is 0 for every non-caliber ship → bulk-crew commit stays the full CrewReq → byte-identical. Gauge `ShipCaliberTests.ThePraetorian_DrawsItsCadreFromTheScarceTalentPool` + `ManpowerTests` (talent commit/release + gate). | ✅ ⚙6.2 first wire + scarcity draw |
| `CrewAutomationAtb.cs` | **NEW (Enhancers ⚙6.3 Systems ▸ Automation, 2026-07-11) — the second wired Enhancers dial.** An AI / automation suite **component** (`IComponentDesignAttribute`; `CrewReduction`, double-arg NCalc ctor L7) — the "run with fewer people" dial, the dossier's "cheapest live win" in the otherwise net-new Systems door (the combat-computer half has no auto-resolver hook; this half rides the manpower system). `ShipDesign.Recalculate` subtracts the summed `CrewReduction` from the hull's BULK crew (clamped so the total never drops below the veteran-cadre `TalentReq` — you can't automate away the officers). So an automated hull commits less scarce workforce from `ColonyManpowerDB` — an economy×engineering trade (mass + high tech for crew). **0 when no module → CrewReq unchanged → the whole manpower path is byte-identical** (every current ship). Cradle-to-grave: base-mod `crew-automation` (`electronics.json`, `ComponentType: Augment`, six-point registration) on the NEW **Vanguard Automated Cruiser** (an exact Aegis + one Automation Suite — needs fewer crew than the stock hull). Gauge `ShipAutomationTests` (the suite binds CrewReduction 30 from JSON; the Vanguard needs fewer crew than an identical Aegis). Follow-up (deferred): the combat-computer half (a targeting term in `HitFraction`, shares the Sensors ▸ Fire-Control wire). | ✅ ⚙6.3 automation |
| `ArmourHardeningAtb.cs` | **NEW (Defense ⚙3, 2026-07-11) — the SHIP mirror of the ground armour-nature** (developer's "mirror ground armor to space" call). Plain ship armour folds into one flat toughness pool, blind to what's hitting it; this **component** (`IComponentDesignAttribute`; four soak fractions `SoakVsKinetic/Energy/Explosive/Exotic` 0..0.9, four-double NCalc ctor L7) makes a ship's plating TUNED to a damage NATURE — it soaks that fraction of the matching incoming fire AFTER the shield (an ablative-clad cruiser shrugs off beams, a composite one walls kinetic). `ShipCombatValueDB.Calculate` reads the best installed plate per nature (health-scaled → `ArmourSoakVs*`); the fleet salvo resolve applies `CombatEngagement.FleetArmourSoakFraction` (the toughness-weighted fleet average, weighted across the salvo's damage-by-nature mix) between the shield step and the hull pool. **All 0 when absent → the fleet soak is skipped → combat byte-identical** (every current ship). Cradle-to-grave: base-mod `armour-hardening` (Ablative Hull Plating, `weapons.json`, six-point registration) on the NEW **Ironclad Ablative Cruiser** (Aegis + one plate). Gauge `ShipArmourHardeningTests` (binds the nature soak from JSON; the ironclad fleet soaks ~40% of an energy salvo but ~10% of a kinetic one; a plain fleet soaks 0). v1 is the nature-SOAK decision; the fuller ground-style flat-per-source swarm-bounce armour on ships is a separate balance-changing follow-up. | ✅ ⚙3 ship armour-nature |
| `InertialessDriveAtb.cs` | **NEW (Propulsion ⚙2 · P1, 2026-07-10)** The Exotic-propulsion INERTIALESS drive **component** (`IComponentDesignAttribute`; `EvasionOverride` 0..1, double-arg NCalc ctor L7) — breaks the mass↔evasion coupling so a heavy capital dodges like a fighter. Lives in Combat/ beside its sole reader: `ShipCombatValueDB.CalculateEvasion` reads `InertialessEvasionFloor(ship)` (the greatest installed `EvasionOverride`, health-scaled) as a **floor** — final evasion = max(mass-bound evasion, floor), capped at `EvasionCap`. **0 = no drive → floor 0 → the ordinary mass-bound evasion → byte-identical** (every current ship). Cradle-to-grave: base-mod `inertialess-drive` (`engines.json`, six-point registration) on a NEW heavy capital, the **Phantom Inertialess Cruiser**. A shot-off drive drops you back to mass-bound evasion (the grave rung). Gauge `ShipInertialessDriveTests`. | ✅ P1 |
...
- **`Firepower`** — hurt-per-second. Each beam weapon contributes `Energy ÷ ChargePeriod` (joules/sec), scaled by that component's `HealthPercent`. Each missile launcher adds a flat `MissileLauncherFirepowerStub` (v1 stub) — **UNLESS the C-guided flag `EnableGuidedWarheadFirepower` is on** (default off → the stub → byte-identical; the client turns it on), in which case a launcher reads its **actually-loaded** ordnance's warhead energy (`PickedWarheadFirepower` → `WarheadEnergyJoules(AssignedOrdnance) ÷ GuidedWarheadDivisor`), NOT the faction's heaviest (the developer's "the picked warhead" ruling, 2026-08-18). Because this value is cached once at build (when nothing is loaded), `Weapons.SetOrdinanceToWpnOrder.Execute` **recomputes** the ship's `ShipCombatValueDB` on ordnance assignment (also flag-gated) so a newly-loaded warhead actually changes the rating. A fresh/never-loaded launcher still rates the flat stub. Gauge `GuidedWarheadFirepowerTests`. *(Pre-existing fire-control quirks, documented not introduced: `AssignedOrdnance` is per-launcher-DESIGN and not itself serialized — but the recomputed combat value IS saved, so firepower survives save/load.)*
...
**v1 stubs (flagged):**
- **Hostility** = different non-neutral faction **as the FALLBACK only**. ~~There is **no diplomacy/relations system** in the engine yet~~ — **REFUTED, corrected 2026-07-27: there is one, and this very file reads it.** `AreHostile` (`CombatEngagement.cs:1847-1874`) now consults `Factions.DiplomacyDB` in two directions: a **declared war** (`RelationshipState.AtWar`) via `IsAtWar` (`:1876-1896`) makes two factions hostile *"full stop — no matter what stance or signed pact sits underneath it"*, and a **mutual** Friendly/Allied stance via `AtPeace` (`:1898-1915`) *suppresses* the default hostility. Diplomacy can only suppress, never create, hostility — and an unmet stranger has no stored relationship, so it falls through to the old different-faction rule, which is why every existing combat fixture is unchanged. Design: `docs/society/DIPLOMACY-DESIGN.md`. And the engine's own `EntityFilter.Hostile` additionally requires a *sensor contact* — which the v1 plan stubs as "everyone sees everyone." So the trigger ignores sensors and treats any two different-faction fleets as enemies. Real IFF/relations is a v2 layer.
...
**E12 slice 2 DONE (2026-08-19, FUEL refuel-on-recovery):** the hook lives in `DockTools` (not `CombatEngagement`) —
`DockTools.EnableCarrierRearm` (default OFF, its reader is `TryDock` itself) + `DockTools.RefuelFromCarrier(carrier, ship)`
(internal), called inside `TryDock` right after the position re-parent. It mirrors `ShipFactory.FillFuelTanks` but SOURCES
the fuel from the carrier's own `CargoStorageDB` instead of an infinite sentinel: take-what's-available — add up to the
craft's free tank space, capped by what the carrier holds (`GetFreeUnitSpace`/`GetUnitsStored`), then drain the carrier by
exactly what moved (conservation). A no-op (never throws) if the craft has no thruster/fuel-hold, the carrier has no store,
...
### Target priority — WHO a force shoots first (Phase 5 ROE, wired 2026-08-04)

**What it is.** A doctrine's `TargetPriority` (`TargetPriority.cs` — one enum shared by space + ground) now actually steers the casualty step. Before, both resolvers spread fire by CURRENT health, so damage preferentially landed on the *healthiest* target and a cripple was never finished (the opposite of how anyone fights — the enum's own docstring complaint). The dial + JSON field + parser existed; the resolver just ignored it. Now `ApplyCasualties` reads the **attacker doctrine's** priority and orders the defender's casualty buckets by it, after the always-first "combatants before utility" rule.

**Which modes are live (the honest scope).** The whole-or-dead aggregate model (a ship is full value or gone; damage is a fleet pool; buckets key on `(toughMult, evasion, toughness, role)`) can express exactly two of the six modes today:
- **`Heaviest`** → sort buckets by toughness desc (focus the toughest — the armour-breaker; base doctrine `armour-breaker`).
- **`BiggestThreat`** → sort by firepower desc (focus the highest-firepower enemy; base doctrine `focus-fire`).

`FinishWounded` needs per-ship health (the parked recalc-on-damage / degraded-condition model — gotcha #2), and `Closest` / `Backfield` need per-target position the aggregate pool doesn't carry; all three **fall back to `Balanced`** — flagged, not faked. `Balanced` reduces to the exact legacy sort (`role desc, then landed desc`), so every un-doctrined / Balanced fight is **byte-identical** (the whole existing suite + the default game — no base doctrine authors `Heaviest`/`BiggestThreat`).

**Whose priority governs a shared pool (v1 stub).** Fire is combined across all attackers before casualties, so v1 uses the **representative attacker's** priority (`attackersOf[i][0]` — the same fleet already used for `OpponentFleetId`/`attackerLabel`); per-attacker weighting is the follow-up. Ground formations still use the ground stance path; wiring `TargetPriority` into the ground resolver is the parallel follow-up.

**Connections (Prime Directive):** **Feeds IN** — `FleetDoctrineDB.Targeting` (from the catalog via `TrySetDoctrine`); the per-bucket `Firepower`/`Toughness` (added to `CasualtyBucket`, NOT the bucket key → bucketing unchanged). **Feeds OUT** — the casualty ORDER (who dies first). **Triggers** — nothing new (same ship-destruction side effect). **Cradle-to-grave:** the priority is a doctrine choice (moddable catalog → player picks a posture → resolver reads it); base doctrines `focus-fire` (BiggestThreat) + `armour-breaker` (Heaviest) make both live modes selectable in-game. **One-verb-both-seats:** the AI sets doctrine the same way the player does, so both drive targeting through the identical primitive.

**Test:** `Pulsar4X.Tests/CombatTargetPriorityTests.cs` — on the same two-ship defender (a thin high-firepower ship + a tanky low-firepower one), `BiggestThreat` finishes the high-firepower ship first while `Heaviest` finishes the tanky one first (opposite targets); `Balanced` (default) still resolves a clean win.

---

## Fleet components — per-component doctrine

**What it is.** A fleet can be split into named **components** — Front Line, Flank, Rear Guard, Artillery — so different parts of one fleet fight with different postures in the *same* engagement (docs/combat/COMBAT-DESIGN.md System 4, detailed design). A component is just a **sub-fleet**: `FleetDB` already nests via `TreeHierarchyDB`, so ship assignment, movement, and detach/reattach all already work — the only new part is that each sub-fleet can carry its own `FleetDoctrineDB`.
...
**What it is.** A fleet can **break off** an engagement instead of fighting to extinction (docs/combat/COMBAT-DESIGN.md System 5). v1 is a **math outcome**: breaking off attaches a `FleetRetreatDB` (the flag + a withdraw vector + who it fled from) and ends the engagement. It does **not** issue a movement order — ships don't physically run yet; that's a v2 movement-system layer. `FleetRetreatDB` is the hook that layer will read.
...
**The math (all in `CombatEngagement`, see `docs/combat/WEAPONS-DESIGN.md`):**
...
hook. Design + connected systems: `docs/combat/WEAPONS-DESIGN.md` → "Future depth — aggregate force
...
>
> **+ P4.2 (Operation Earthfall, findings A4 cause 3): the INDUSTRY build path now charges+fuels too — the FOURTH spawn path.** `ShipFactory.ChargeReactors`/`FillFuelTanks` were called only by the scenario loader + `CombatSandbox` — never on production-built ships — so a home-built transport booted with a dead reactor and empty tanks and couldn't warp, stranding the NPC sealift. `ShipDesign.ProvisionBuiltShip(ship, ownerFaction)` is the ONE policy gate both industry build paths (`ShipDesign.OnConstructionComplete` direct-build branch + `LaunchComplexProcessor.TryLaunchShip`) now call. Owner-split policy (both FLAGGED): `ChargeBuiltNpcShips` (default ON — the AI sealift boots ready to fly) / `ChargeBuiltPlayerShips` (was default OFF = the "player earns its charge over game-time" rule; **flipped ON 2026-07-21 per EARTHFALL-DECISIONS #3 — anything you build is ready to fly; one-line revert**). Byte-identical in the engine suite (no fixture completes an NPC ship build). Gauge: `NpcFleetReadyToSailTests.BuiltShips_ChargedForNpc_NotForPlayer_PerProvisioningPolicy`.
...
limits are exactly where the parked **aggregate force-condition** model plugs in (`docs/combat/WEAPONS-DESIGN.md`
...
deterministic (the locked rule — *fast-forward must equal watch*; see `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.4`). Today's
...
|----------|-------|---------|-------|
| `MissileLauncherFirepowerStub` | 100,000 | flat firepower (J/s) per missile launcher — the DEFAULT. Superseded by the loaded warhead's energy when `EnableGuidedWarheadFirepower` is on (C-guided, 2026-08-18); still the fallback for an unloaded launcher | `ShipCombatValueDB.cs` |
| `EnableGuidedWarheadFirepower` | false (client: true) | C-guided flag: a missile launcher reads its LOADED warhead (`PickedWarheadFirepower`) instead of the flat stub; also gates the recompute-on-assign in `SetOrdinanceToWpnOrder.Execute`. Off → byte-identical | `ShipCombatValueDB.cs` |
| `GuidedWarheadDivisor` | 20.0 (mutable) | C-guided calibration: launcher firepower (J/s) = loaded warhead energy (J) ÷ this. Live-tune knob (no recompile) | `ShipCombatValueDB.cs` |
...
| `CombatEngagement.MinLandedFraction` | **0.05** (was 0.02, changed 2026-07-29) | floor on fire that lands — enough volume kills even a perfect dodger. **Kept equal to `1 − EvasionCap`** so the two ceilings on evasion AGREE: evasion is an effective-health MULTIPLIER (`EffToughness = Toughness ÷ landed`, no to-hit roll), and at 0.02 it capped at **×20 point-blank** (bound by `EvasionCap`) but **×50 at range** (bound by this floor). Now ×20 everywhere. ⚠ BALANCE — moves live numbers; only binds for weapons with `Saturation < ~2.6` (low-RoF ballistics). Gauge `CombatKernelTests.TheTwoCeilings_Agree_*`. Design: `docs/economy/DESIGNER-NORTH-STAR.md` §1b/§1c | `CombatEngagement.cs` → `CombatKernel.cs` |
...
build plan + locked decisions are `docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §14.4`. Each phase is behind a **default-OFF flag**
...
6. **Hostility = "different non-neutral faction" by DEFAULT, SUPPRESSED by diplomacy/treaties, but a DECLARED WAR overrides that suppression (2026-07-01; war-override added 2026-07-12, Phase-3.4a).** `AreHostile` still starts from the v1 rule (different non-neutral factions fight). It reads each faction's `DiplomacyDB` (`Pulsar4X.Factions`) and returns *not hostile* when **both** sides are at peace (`AtPeace` helper): a mutual Friendly/Allied **stance**, OR a signed **`NonAggressionPact`/`DefensivePact`** flag (so `Treaties.Propose(NonAggression)` actually stops a fight even at a neutral score — the treaty→combat loop is closed; gauge `DiplomacyIffTests.SignedPact_StopsTheFight`). **BUT a declared war wins over all of that:** `IsAtWar(a,fa,fb) || IsAtWar(b,fb,fa)` is checked FIRST and returns hostile regardless of any stance/pact — so if either side latches `RelationshipState.AtWar` (via `Diplomacy.DeclareWar` / `rel.DeclareWar()`), the fight is on even under a lingering pact. This is the developer's "alliance members are still allowed to shoot each other" rule and the teeth behind coalitions (an ally that joins your war on a shared threat becomes really hostile to it; a broken/betrayed pact instantly re-arms two former allies). `IsAtWar` mirrors `AtPeace`'s ledger resolution exactly (only a STORED relationship counts). The defensive pact's "drag me into your ally's wars" entanglement is Phase-3.4b (the NPC deciding to declare that war), separate from this combat-layer fact. Diplomacy can only SUPPRESS the default, never create it: an unmet stranger falls straight through to the old result, and a one-sided friendly declaration does NOT disarm you — so every existing combat fixture is byte-identical (nothing writes an AtWar+pact combo in those). `AreHostile` is `internal` (gauges: `DiplomacyIffTests` incl. `DeclaredWar_OverridesASignedPact`). Still-open v1 stub: the engine's `EntityFilter.Hostile` sensor-contact requirement is separate (fog is handled by `RequireDetectionToEngage`, not here). Next diplomacy-behavior slices (first-contact event, casus belli, commerce gating) are TESTING-TRACKER C6.
