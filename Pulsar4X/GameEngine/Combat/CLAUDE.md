...
>
> **Slice 1 ✅ LANDED 2026-07-08 — `CombatKernel.cs` (additive, byte-identical).** The shared home for the pure salvo math now exists: the neutral `CombatKernel.Combatant` view (what a ship OR a ground unit presents — value fields + `WeaponProfile`s + a 1-D `Position_m`, no hex/`Entity`) plus the pure functions `HitFraction` / `LandedFraction` / `SoakFractionOf` / `ResolveShield` / `ShieldSoakFraction` / `ArmourSoak`, copied byte-for-byte from the live ship math here and the ground `GroundDamageMatrix.ArmourSoak`. It is **UNWIRED** — nothing calls it this slice, so every ship and ground fixture is byte-identical (nothing existing was rewired). `CombatKernelTests` pins the outputs AND cross-checks them against the live functions so the one-slice duplication can't silently drift. **Next — slice 2:** route `StepEngagementGroup`'s inner damage math through `CombatKernel` on ship-derived `Combatant`s and delete the duplicated ship copies (ship fixtures = the byte-identity tripwire).
...
| File | Purpose | Status |
|------|---------|--------|
| `CombatKernel.cs` | **NEW (resolver merge, slice 1, 2026-07-08)** The shared, domain-neutral salvo KERNEL both a ship battle and a planetary battle will run through. `CombatKernel.Combatant` = the neutral view (FactionId · Health/MaxHealth · Evasion · Armour · Shield pool/capacity/regen · `List<WeaponProfile>` Weapons · 1-D `Position_m`) — no hex, no `Entity`. Pure static math copied byte-for-byte from the live ship resolver (`HitFraction`, `LandedFraction`, `SoakFractionOf`, `ResolveShield`, `ShieldSoakFraction`) + the ground matchup (`ArmourSoak` from `GroundDamageMatrix`). **UNWIRED this slice** — additive home only; nothing calls it, so ship + ground behaviour is byte-identical. Gauge `CombatKernelTests` (pins values + cross-checks vs the live functions). Design: `docs/RESOLVER-MERGE-DESIGN.md`. | ✅ slice 1 (additive) |
