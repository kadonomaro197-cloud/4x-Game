...
| **reactor-load** | folding "how hard the reactor runs" into the heat factor. **Deferred + flagged:** the obvious field `EnergyGenAbilityDB.Load` is **buggy** — its formula is `TotalOutputMax / batteryInflow` (inverted + unbounded; 1.0 at idle, →∞ under demand), NOT the "percent of max output" its comment claims. Needs that field fixed, or compute a clean `Demand/TotalOutputMax`. | ⏳ flagged | — |
