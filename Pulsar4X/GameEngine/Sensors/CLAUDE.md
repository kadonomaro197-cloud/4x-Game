...
**Band-match GATE fix — BUILT flag-gated (2026-08-21, `SensorTools.cs` `DetectonQuality`).** The band-match test at the top of the per-band loop had a bug the detection game *depended on*: `if (Math.Max(recvMin, sigMin) < Math.Max(sigMin, sigMax))` — the RHS `Math.Max(sigMin, sigMax)` always equals `sigMax`, so the **receiver's UPPER edge (`recvMax`) was never consulted**. A VISIBLE-light receiver (475–725 nm) therefore "detected" the reactor's ~1705 nm **infrared** emission — it let through *every* signal at a longer wavelength than the receiver's bottom edge. The correct overlap is `max(recvMin, sigMin) < min(recvMax, sigMax)` (both edges). Extracted to pure `BandsOverlapCorrect`/`BandsOverlapLegacy` and gated behind **`SensorTools.EnableBandMatchFix` (default OFF → the legacy test → byte-identical)**. ⚠ **Fixing it ALONE collapses detection** — with the upper edge honored, no base-mod VISIBLE receiver sees the IR reactor, so an engines-off ship goes invisible (the *"sat at Luna, saw nothing"* failure the detection rebalance exists to cure). So **going live is a coordinated developer step, verified live (combat detection is CI-blind): (1) add an INFRARED receiver to the base-mod sensor loadout (a new `SensorReceiverAtb` template tuned to ~1000–2000 nm, six-point registered, installed on ship/colony sensor designs), and (2) flip `EnableBandMatchFix` ON — together.** This is the same ruling the deferred FTL-band question waits on. Gauge: `BandMatchTests` (the bug vs the fix on the reactor-vs-visible case, an IR receiver restoring detection under the fix, genuine overlaps preserved, flag-default-off). The alternative route the mockup named — *compute the band and drop the band-centre dial* — was not taken; this is the "fix the overlap + IR receiver" route. Design: `docs/Actual HTMLs Of designers/sensorsderived.html`.

**Resolution → FIDELITY — BUILT flag-gated (2026-08-21, `SensorTools.cs` `DetectonQuality`).** `SensorReceiverAtb.Resolution` (MegaPixels) was stored but READ BY NOTHING — its only reader sat in a long-dead `/* */` block (the code's own comment: *"receiver resolution should play into how much gets detected"*). It now scales `SignalQuality` (the fidelity axis — "something is there" 0 → "full identification" 1) by a saturating factor `resFactor = Res/(Res + ResolutionFullIdKnee_MP)` (knee `1.0` MP), so a low-res sensor (~1 MP → ×0.5) caps at partial ID while a high-res one (100/200 MP → ~×0.99) reaches full ID. It scales quality only and NEVER changes detect-yes/no (that is strength/band-match), so it can only make a contact LESS resolved, never manufacture a detection. Gated behind **`SensorTools.EnableResolutionQuality` (default OFF → the pure band-alignment score → byte-identical)**, client-on via `NewGameMenu`. Unlike the band-match fix it is **safe to flip standalone** (it can't blind a ship). **Blast radius (Prime Directive):** the only threshold consumer of `SignalQuality` is SURVEY reveal (`SystemBodyInfoDB`/`StarInfoDB`, gates 0.20/0.80) — stock high-res homeworld sensors stay above 0.80, so survey is unaffected (gauge case #4 pins it). Gauge: `SensorResolutionQualityTests` (flag-default-off, flag-off byte-identical, higher-res-resolves-better, the survey-safety guard). This FINISHES the "forgotten consumer" Resolution dial (the dial audit's Bucket 3).
...
- **v1 behaviour:** the ship stops detecting NEW/refreshed contacts, but contacts it already put in the faction track table **persist until they age out** — so a faction doesn't instantly forget. **Contact age-out is now BUILT (Earthfall C2.1, 2026-07-18):** a stale track is removed after `SensorScan.ContactStaleSeconds`, and a lost-but-recent track flips to held-memory (LAGGED→FROZEN) before it expires (gauge `EfClientSensorAgeOutTests`).
