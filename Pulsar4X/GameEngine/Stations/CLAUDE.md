...
| `DeployStationOrder.cs` | **The player's FRONT DOOR (Slice A)** — the `EntityCommand` parallel to `Colonies/CreateColonyOrder`. `CreateCommand(faction, hostingBody, ...)` → `Execute` calls `StationFactory.CreateStation` (which only tests could reach before) and installs a **starter constructor loadout** (`default-design-factory`, guarded) so the deployed platform is immediately an in-situ builder ("deploy bare, build in-situ"). Issued from the UI via `OrderHandler.HandleOrder`. Client side: a **"Deploy Station" button** in `SystemWindow` (next to Colonize, on any surveyed body incl. asteroids) + a **`StationWindow`** reachable from the map context menu ("Manage Station") that embeds the host-agnostic `IndustryDisplay`. |
...
3. **The player can now REACH a station (Slice A front door, 2026-07-03).** `DeployStationOrder` + the `SystemWindow` "Deploy Station" button + the `StationWindow` context-menu entry make the whole chain reachable in normal play: deploy a platform at a body → it carries a starter constructor → select it → queue modules via the embedded `IndustryDisplay`. The **engine order is CI-tested** (`DeployStationOrder_DeploysAFunctionalPlatform`); the **UI is compile-checked only** (CI can't run the client — verify the deploy button, context menu, and StationWindow live). Remaining front-door polish (materials auto-supply via `LogiBaseDB`, a station designer #21) is deferred.
