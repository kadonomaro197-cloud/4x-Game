...
| `GroundForcesProcessor.cs` | The ground layer's **one** hotloop (`IHotloopProcessor` on `GroundForcesDB`, hourly). Does every surface job each tick — landmine L9 forbids multiple processors on one blob: **MOVE** (5b), **ENVIRONMENTAL ATTRITION** (E3 — a unit standing in a region with a damaging hazard bleeds health, the ground twin of a ship in a space hazard; **E4 — the unit's environmental GEAR (`GroundUnit.ResistanceTo`) negates a fraction of that attrition**, so a heat-shielded unit survives fire that kills an unprotected one), **FIGHT** (`ResolveRegionCombat` — TERRAIN + TYPE + FORTIFICATION aware: the triangle × terrain-affinity × the defender's cover × the defender's **fortification** (`FortificationDefenseMult` — the region's `InstallationIds` count, capped, so a built-up region is a fortress and a map-placed base has a combat payoff, 5h); mirrors the space `AutoResolve` salvo math over `GroundUnit`s, deterministic, 5c/5f/5g/5h), **CAPTURE** (region `OwnerFactionID` flip; all-regions → `TryCapturePlanet` flips the colony, 5d). Trivial ctor + `try/catch` body (never throws in a hotloop, L4). `SalvoScale` is the combat-pace dial. Gauge: `GroundForcesTests`. | ✅ 5b–5g, E3, E4 |
...
**Leader loss = reassign, no penalty** (the locked decision — exactly what a fleet does): `GroundForcesProcessor.MaintainFormations` (runs after casualties are removed each tick) passes leadership to a surviving member, or -1 if the formation was wiped. **Deliberately NOT built yet (each a follow-up formation slice):** a **doctrine/stance** with combat multipliers (the `FleetDoctrineDB` echo — will need tuned numbers, so it's a *ask-first* slice), **sub-formations** (the fleet tree), and the **client** (create/assign/move formations on the tactical map — the 5e-style client half). Gauge: `GroundForcesTests.Formation_MirrorsFleet_CreateAssignMoveAndLeaderReassign`.
