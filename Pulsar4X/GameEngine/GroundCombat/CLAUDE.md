...
| `GroundForcesProcessor.cs` | The ground layer's **one** hotloop (`IHotloopProcessor` on `GroundForcesDB`, hourly). Does all three surface jobs each tick — landmine L9 forbids three processors on one blob: **MOVE** (count down in-transit units, land them, 5b), **FIGHT** (`ResolveRegionCombat` — one salvo per contested region: each faction takes the combined attack of all others, focus-fired; mirrors the space `AutoResolve` math over `GroundUnit`s, deterministic, 5c), **CAPTURE** (a region's sole survivor takes it via `Region.OwnerFactionID`; when all regions are one invader's, `TryCapturePlanet` flips the colony's `FactionOwnerID` — the "take a planet" moment, 5d). Trivial ctor + `try/catch` body (never throws in a hotloop, L4). Combat-pace dial `SalvoScale` (mirrors the space `SalvoDamageScale`). Gauge: `GroundForcesTests`. | ✅ 5b/5c/5d |
