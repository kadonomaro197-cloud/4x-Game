...
| `GroundForcesProcessor.cs` | The ground layer's **one** hotloop (`IHotloopProcessor` on `GroundForcesDB`, hourly). Does every surface job each tick — landmine L9 forbids multiple processors on one blob: **MOVE** (5b), **ENVIRONMENTAL ATTRITION** (E3 — a unit standing in a region with a damaging hazard bleeds health, the ground twin of a ship in a space hazard), **FIGHT** (`ResolveRegionCombat` — now TERRAIN + TYPE aware via `GroundTerrain`: the triangle × terrain-affinity × the defender's cover; mirrors the space `AutoResolve` salvo math over `GroundUnit`s, deterministic, 5c/5f/5g), **CAPTURE** (region `OwnerFactionID` flip; all-regions → `TryCapturePlanet` flips the colony, 5d). Trivial ctor + `try/catch` body (never throws in a hotloop, L4). `SalvoScale` is the combat-pace dial. Gauge: `GroundForcesTests`. | ✅ 5b–5g, E3 |
