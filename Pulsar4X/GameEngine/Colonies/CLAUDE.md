...
| `EmploymentAtbDB.cs` | **NEW (M2)** Component attribute: jobs (worker slots) an installation provides. Summed via `ComponentInstancesDBExtensions.GetTotalJobs`. Morale: jobs-vs-pop is two-sided (unemployment debuff / full-employment buff). **⚠ INERT (2026-08-09): the attribute + the `GetTotalJobs` consumer + the `PopulationProcessor.cs:74` ±40 morale reader are all built, but NO base-mod installation template declares `Jobs` — so `GetTotalJobs` sums to 0 in every colony and the employment-morale term contributes 0 in every game. Producer missing; the loop never fires.** |
...
- Jobs/unemployment + housing comfort — **machinery built** (M2): `EmploymentAtbDB`, `HousingAtbDB`, summed and read into morale — **but no installation template declares `Jobs`, so the employment term reads 0 in every colony (producer missing; 2026-08-09).**
