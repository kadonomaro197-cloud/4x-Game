...
**Span-of-control RANK GATE — AdminLevel is now READ by a real rule (2026-08-18, OPERATION BLUEPRINT-TO-STEEL "wire it").** Until now the `AdminLevel` a command seat carries (`AdminSpaceAbilityState.SeatType`) was a *label* no rule read — `AssignAdministratorOrder.IsValidCommand` was hardcoded `return true`. It now gates on scope: a broader command demands a more SENIOR officer. Pure helpers on `AdminSpaceProcessor`: `AdminRankRequired(AdminLevel)` = `max(0, (int)level − AdminRankLevelOffset)` (the min `CommanderDB.Rank` for that scope; monotonic), and `CanOfficerHoldSeat(commander, level)` (`Rank ≥ required`). `AssignAdministratorOrder.IsValidCommand` resolves the target seat + officer (the same way `Execute` does) and returns `CanOfficerHoldSeat` — refusing ONLY a genuine under-ranked seating (an unresolved post/officer falls through to valid, since `Execute` already no-ops on it). **Flag-gated `AssignAdministratorOrder.EnableAdminRankGate` (default OFF → every assignment valid → byte-identical).** ⚠ The rank map is FLAGGED/tunable — the default offset `5` leaves routine posts (Ship…Colony, ordinals 0–5) ungated (required rank 0 = any officer) and only bites the strategic scopes (Planet 1 … Empire 5); start officers are rank 1–6 (`CommanderFactory`), so a green officer runs a colony but an empire-wide command needs a senior one. **Left off (not client-activated) until the rank map is tuned against the live officer scale** — CI can't run the client to see the distribution, so this is the developer's flip + tune (one line: `AssignAdministratorOrder.EnableAdminRankGate = true`). Gauge: `AdminRankGateTests` (the pure cap: required-rank rises with scope, junior blocked from strategic scope / senior allowed, null holds nothing; + the flag defaults off). *This is v1 — a rank-for-scope gate, NOT yet a per-administrator subordinate-COUNT cap (that needs an administrators-hold-many-posts model `CommanderDB.AssignedTo` doesn't have — a later slice).* `ConsoleSpace` remains a separate, still-unread dial.
...
| File | Role |
|------|------|
| `CommanderDB.cs` | DataBlob for an officer. Has Name, Rank (int), Type (`CommanderTypes` enum), Experience, ExperienceCap, CommissionedOn, RankedOn. **No bonus fields of its own** — a commander's skill bonuses live on the separate `BonusesDB` blob (Firepower/Toughness/ResearchPoints/Espionage), read via `CommanderBonuses.CombatMultiplier`. |
...
|---------|--------|-------|
| Naval academy generates officers | ✅ code works, ⚠️ unreachable | `NavalAcademyProcessor` fires on schedule *when installed* — but the naval academy is on no start colony's build list, so it's never instantiated in a default game. The reachable academy is the research academy (X.0-2). |
...
| Commander COMBAT bonuses | ✅ built & wired (CI-unit-tested; runtime unverified) | Bonuses live on `BonusesDB` (Firepower/Toughness), generated by the academy (`RollCombatCompetence`) and read via `CommanderBonuses.CombatMultiplier`. See the rung-4 notes above. **Navigation bonuses are still NOT built** (no navigation `BonusCategory`). |
| Ground force commanders | ⚠️ enum exists, no ground-specific loop | `CommanderTypes.Ground` IS in the enum (`Enums.cs:266`); what's missing is a ground-force skill/combat loop that reads a ground commander's bonuses (see Phase 4 below). |
| Commander death on ship destruction | Partial — `ShipFactory.DestroyShip()` kills the commander | See `Ships/CLAUDE.md` |
| Flagship commander bonus | ✅ built & wired (CI-unit-tested; runtime unverified) | `CombatEngagement.FleetCommanderMult` (`CombatEngagement.cs:1575`) resolves `FleetDB.FlagShipID` → commander `BonusesDB` and folds it into every ship's firepower/toughness mult (`:1545-1546`). Gauge: `CommanderCombatWireTests`. |
...
|----------------|--------|--------|
| Officers generated by academies | `NavalAcademyProcessor` | ✅ code functional, but ⚠️ UNREACHABLE — the naval academy is never instantiated on any start colony, so no class graduates in a default game. The reachable twin is the research academy (X.0-2). |
| Scientist research bonuses | `Scientist.Bonuses` | ✅ functional |
| Commander combat bonuses | `BonusesDB` (Firepower/Toughness) + `CombatMultiplier`, folded via `FleetCommanderMult` | ✅ built & wired (CI-unit-tested; runtime unverified). Navigation bonuses still not built. |
| Ground force commanders | `CommanderTypes.Ground` in the enum; no ground-combat skill loop yet | ⚠️ enum exists, loop not built |
| ~300 starting officers, 10/yr | NavalAcademy controls output rate | ✅ configurable via component (but see note: the naval academy is unreachable — never instantiated on any start colony, so no class actually graduates in a default New Game; the reachable sibling is the research academy) |
| Officer experience affects performance | Experience is stored; scientist experience is now read & grows (`GrowScientistExperience`); commander combat experience feeds `ExperienceCap`→academy competence roll | ⚠️ partial — scientist side wired, commander in-service experience growth not yet |

---

## Phase 4 Additions Needed

1. ~~Add `CommanderTypes.Ground` to the enum.~~ **DONE** — `CommanderTypes.Ground` already exists (`Enums.cs:266`).
2. ~~Add skill bonus fields to `CommanderDB`~~ **The bonus-carrier already exists** — commander skill bonuses live on the separate `BonusesDB` blob (Firepower/Toughness/ResearchPoints/Espionage categories). What remains is a *ground* `BonusCategory` + a ground-combat loop that reads it (below).
