...
**Commander combat competence now applies (2026-07-09, rung-4 wire) — but no commander carries a bonus yet.** A fleet's flagship commander's `BonusesDB` Firepower/Toughness competence now scales the fleet in the auto-resolver (see the rung-4 note below). The remaining gap is the *generator*: `CommanderDB` graduates get an **empty** `BonusesDB` (only `Scientist` bonuses are authored today), so the wire is live but there's nothing to fold until the academy competence generator (slice 3c) writes bonuses onto graduates.
