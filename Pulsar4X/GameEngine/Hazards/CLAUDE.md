...
| File | Role |
|------|------|
| `HazardEffect.cs` | The typed effect vocabulary — `HazardEffectType` (HeatDamage / RadiationDamage / KineticDamage / SensorJam / MovementDrag / WarpInhibit; + the appended Corrosive/EM/Gravimetric damage kinds; **+ the GROUND-ONLY `Vacuum` / `ToxicAtmosphere` surface kinds, appended 2026-07-17**) + the `HazardEffect` (type + magnitude + wavelength + scales-with-proximity). This is the SPINE: a hazard is a list of these (data); a counter resists by kind (data). **`Vacuum`/`ToxicAtmosphere` are deliberately NOT in `IsDamage`/`SignatureFor`** (a ship in space isn't harmed by planetary vacuum → the space hazard system is byte-identical); they're consumed only by the GROUND E4 attrition (`GroundCombat.GroundForcesProcessor.IsDamageEffect` + `PlanetEnvironmentFactory` generation) — an unsealed unit bleeds on an airless/toxic world, a sealed one (E4 `EnvResistance`) survives. **+ the keystone link (2026-06-28):** a derived `[JsonIgnore] Signature` maps each *damage* kind to the coarse shared `Damage.DamageSignature` (HeatDamage→Thermal, RadiationDamage→HardRadiation, KineticDamage→Kinetic; stat kinds → null) via `HazardEffect.SignatureFor`. That's the shared hazard↔weapon flavour word — see `Damage/CLAUDE.md` → "The DamageSignature keystone". |
