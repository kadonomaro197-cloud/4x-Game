...
| `MineResourcesProcessor.cs` | `IHotloopProcessor` (daily) **and** `IRecalcProcessor`. Two clocks: `CalculateActualMiningRates()` runs at **recalc time** (`CalcMaxRate`/`RecalcEntity`) and caches the result in `MiningDB.ActualMiningRate`; the **daily tick** (`MineResources`) reads that cached rate, scales it by infrastructure efficiency (`InfrastructureProcessor.GetEfficiency`), and transfers the minerals into cargo (see `MineResourcesProcessor.cs:64,71`). **+ R1b PER-HEX MINING (flag `IndustryTools.EnablePerHexMining` — lives on the public helper since this processor is internal; default OFF → the aggregate pool pass is byte-identical, `NewGameMenu`-on for a menu game; 2026-08-21):** when ON and the mined body carries a `SurfaceGrid`, `MineResources` routes through `MineResourcesPerComponent` (which REPLACES the aggregate pass, so no double-count): a mine PLACED ON A HEX (its `ComponentInstance.ID` in a `GroundHex.InstallationIds`) works THAT hex's located deposit into the hex's own `Stockpile` via the pure `MineHexDeposit` (deplete `DepositAmount` → fill the bucket, conserved) — so hex-mined ore SITS on the hex until HAULED (H1); a colony-level mine (on no hex) still mines the body-wide pool into colony cargo (the gradual-retrofit default). Client flips the flag ON only once the haul verb exists, so hex-mined ore is never stranded. Gauge: `PerHexMiningTests` (pure depletion/cap/conserve/no-op); the full component-routing integration gauge is a fast-follow (OFF path covered by `EconomyReadoutTests`). Design: `docs/ground/UNITS-ON-THE-MAP-DESIGN.md` §2 weld #1. |
| `HexMinerals.cs` | **NEW (located deposits, 2026-07-06)** Seeds a body's mineral deposits onto specific surface **hexes** (`GroundHex.DepositMineralId`/`DepositAmount`) so "there are resources HERE" is a PLACE you scan/see/build a mine on — the LOCKED PRINCIPLE (`docs/ground/GROUND-SURFACE-MAP-DESIGN.md`) applied to minerals. `SeedDeposits(body, grid)` picks hexes **terrain-flavored** (mountains/volcanic/highlands rich; never ocean/ice), one mineral per deposit hex, partitioning a share (`LocatableFraction` 0.6) of the body's real `MineralsDB` amount; deterministic (system RNG), idempotent, defensive. **Also seeds the per-faction masked `GroundHex.DepositAssay` (`Masked<long>`, ground-fog slice 1b, 2026-07-17)** at `AccessLevel.None` — the located tonnage is HIDDEN by default, closing the leak where the plain `DepositAmount` int let any faction read a deposit it never surveyed. `DepositAmount` still carries the omniscient server-truth (byte-identical); the assay is granted per-faction later (space survey → Partial, scout → Full). Design: `docs/ground/SURFACE-FOG-AND-RECON-DESIGN.md`. Hooked in `PlanetGridFactory.EnsureGridForBody` (runs when a body's surface grid is generated — post-survey), so **every planet, whole game**. **v1 is the LOCATED VIEW only — the colony still mines the body-wide pool (`MineResourcesProcessor` unchanged); per-hex mining (a mine works the deposit it SITS on, and that hex depletes) is the follow-up that promotes these to the source of truth.** Next slices: planet-view flags deposit hexes post-scan → build-a-mine-on-a-deposit → per-hex mining. Gauge: `HexMineralsTests`. |
...
- **✅ NOW LIVE ON THE START COLONY (C-POWER-LIVE, 2026-08-16).** The recon found no colony-scale plant existed (the fission reactor + steam-turbine LACKED `PlanetInstallation` in their `MountType`, and solar makes only ~10 kW/panel). Fix taken: **added `PlanetInstallation` to the fission `reactor`'s `MountType`** (`energy.json` — additive, so `PowerPlantGroundMountTests`' GroundUnit/ShipComponent flag checks still pass; `PowerJustificationTests` only pins solar's mount) and **installed ONE `default-design-fission-reactor` on Earth** (`earth.json` `Installations` — the design was already registered in Earth's ComponentDesigns + `reactor` in StartingItems, so no new registration). Earth now generates **75 MW** vs its ~52 MW installation demand → the throttle reads **1.0** (not throttled) with ~1.4× headroom, and bites once industry grows past that. Gauge: `PowerThrottleTests.PowerEfficiency_StartColony_IsPoweredAndSafe` (asserts supply ≥ demand on the real New-Game colony — CI is the calibration net). Grave rung live: bombard the reactor → generation falls below demand → production browns out.
