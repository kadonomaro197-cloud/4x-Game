...
- **⚠ LIVE only once a colony GENERATES power — and the base mod has NO colony-scale power plant yet (parked, C-POWER-LIVE).** No base-mod colony installs a reactor/solar (`earth.json` `Installations` has none), AND the recon found there's no component that both mounts on a colony (`PlanetInstallation`) and makes MW-scale power: the fission reactor (75 MW) and steam-turbine both LACK `PlanetInstallation` in their `MountType` (the turbine's description even says "colony plant" — a mount/description bug), and the only `PlanetInstallation` source, the solar array, makes ~10 kW/panel (Earth's ~52 MW demand ⇒ ~5,000 panels, absurd). So this is a byte-identical mechanism **inert on the stock start colony** until a colony-scale plant exists — a design gap parked in the campaign log (C-POWER-LIVE) with a recommendation (add `PlanetInstallation` to the turbine's mount, then install one on Earth). The mechanism is correct and ready; it waits on its supply.
