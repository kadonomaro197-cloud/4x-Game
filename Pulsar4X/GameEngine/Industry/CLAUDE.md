...
**ALPHA starting stockpiles (2026-07-03; completed 2026-07-10).** The start colony's stockpiles are stocked LARGE so building isn't resource-gated during testing (the developer's "picket couldn't build — stockpiles emptied"). In `GameData/basemod/ScenarioFiles/systems/sol/earth.json` the colony's `Cargo` block stocks every raw mineral **and** the common refined ship-build materials (`stainless-steel`/`electronics`/`plastic`/`space-crete`/`ree-magnetics`) at **50,000,000 each**, and the `default-design-warehouse` installation count is bumped **1 → 10** for storage headroom. **(2026-07-10 fix — the "every raw mineral" claim was aspirational, not true: the block silently OMITTED `regolith`, `water`, and `rare-earth-elements`. `regolith` + `water` are two of the five inputs to `space-crete`, so the start refining job sat at `MissingResources` forever — which is exactly why `EconomyReadoutTests.Economy_BaselineReadout_OverOneYear` was quarantined [Ignore]. All three are now stocked at 50M, so the refinery runs from tick 1 and the test is re-enabled. Lesson: when a doc says "all of X are stocked," diff the actual list against the definitions — this one was three short.)** **Why both:** cargo storage is **volume-capped** (`CargoMath.AddRemove…` clamps every add to the store's `FreeVolume`), so just cranking the amounts would be silently thrown away past one warehouse's ~1,000,000 volume — 10 warehouses give ~10M volume, ample for 50M units of ~17 general-storage goods. **Data-drift guard (gotcha #10):** every `Cargo` Id must be **unlocked** in that colony's `StartingItems` (the added refined materials already are) — `ColonyFactory.LoadCargo` hard-indexes `CargoGoods[id]`, so an un-unlocked id crashes New Game. Trim these back once the economy is balanced.
