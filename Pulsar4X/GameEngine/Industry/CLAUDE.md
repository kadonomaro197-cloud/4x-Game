...
**ALPHA starting stockpiles (2026-07-03) — ⚠️ THE 50M×17 STOCKPILE OVERFLOWS STORAGE (found 2026-07-10).** The start colony's stockpiles were stocked LARGE so building isn't resource-gated during testing (the developer's "picket couldn't build — stockpiles emptied"). In `GameData/basemod/ScenarioFiles/systems/sol/earth.json` the colony's `Cargo` block stocks 50,000,000 of ~17 general-storage goods, with the `default-design-warehouse` count bumped **1 → 10**. **The claim that "10 warehouses = ~10M volume, ample for 50M × 17 goods" is FALSE** — the `EconomyReadoutTests` CI readout (run #786) shows only the **first ~6 minerals** actually land at 50M; then Lithium is partial, **Titanium reads -3**, and Silicon/Regolith/Water/Space-Crete/REE-Magnetics/etc. are **clamped to 0**. Cargo storage is **volume-capped** (`CargoMath.AddRemove…` clamps every add to the store's `FreeVolume`), and 50M × 17 vastly exceeds 10 warehouses' volume — so the tail of the Cargo list is silently thrown away. **This jams the whole economy:** because general-storage is FULL, mined output has nowhere to go (deposits deplete only ~11 units/game-year in the readout), and the refinery starves (three of Space-Crete's five inputs sit at 0) so its job crawls (3/25 pts/year, never completes). This is why `EconomyReadoutTests.Economy_BaselineReadout_OverOneYear` is quarantined. **Fix (pending):** rebalance so the stockpile FITS — either far more warehouse volume (verify how much a warehouse adds via `CargoMath`/`CargoStorageDB` first) or smaller per-good amounts — then re-enable the test. **Data-drift guard (gotcha #10):** every `Cargo` Id must be **unlocked** in that colony's `StartingItems` — `ColonyFactory.LoadCargo` hard-indexes `CargoGoods[id]`, so an un-unlocked id crashes New Game.
