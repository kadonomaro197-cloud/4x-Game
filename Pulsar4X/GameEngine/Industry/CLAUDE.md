...
| `MineResourcesProcessor.cs` | `IHotloopProcessor` (daily) **and** `IRecalcProcessor`. Two clocks: `CalculateActualMiningRates()` runs at **recalc time** (`CalcMaxRate`/`RecalcEntity`) and caches the result in `MiningDB.ActualMiningRate`; the **daily tick** (`MineResources`) reads that cached rate, scales it by infrastructure efficiency (`InfrastructureProcessor.GetEfficiency`), and transfers the minerals into cargo (see `MineResourcesProcessor.cs:64,71`). **+ R1b PER-HEX MINING (flag `EnablePerHexMining`, default OFF → the aggregate pool pass is byte-identical; 2026-08-21):** when ON and the mined body carries a `SurfaceGrid`, `MineResources` routes through `MineResourcesPerComponent` (which REPLACES the aggregate pass, so no double-count): a mine PLACED ON A HEX (its `ComponentInstance.ID` in a `GroundHex.InstallationIds`) works THAT hex's located deposit into the hex's own `Stockpile` via the pure `MineHexDeposit` (deplete `DepositAmount` → fill the bucket, conserved) — so hex-mined ore SITS on the hex until HAULED (H1); a colony-level mine (on no hex) still mines the body-wide pool into colony cargo (the gradual-retrofit default). Client flips the flag ON only once the haul verb exists, so hex-mined ore is never stranded. Gauge: `PerHexMiningTests` (pure depletion/cap/conserve/no-op); the full component-routing integration gauge is a fast-follow (OFF path covered by `EconomyReadoutTests`). Design: `docs/ground/UNITS-ON-THE-MAP-DESIGN.md` §2 weld #1. |
