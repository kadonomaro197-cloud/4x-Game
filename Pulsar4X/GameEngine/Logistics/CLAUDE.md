...
| File | Role |
|------|------|
| `LogiBaseDB.cs` | DataBlob on a colony/station acting as a trade hub. Holds `DesiredLevels` (what it wants to maintain), `ListedItems` (what it has available), `ItemsInTransit`, `ItemsWaitingPickup`, `TradeShipBids`. **Clone bug fixed (B-orders RANK-1):** the copy-ctor silently dropped `Capacity`, `DesiredLevels` and `ItemsInTransit`, so a cloned hub (entity moved between managers) forgot its capacity + per-item targets + in-transit set; now it copies every field (save/load was unaffected — it serialises fields directly). Gauge: `SetLogisticsOrderTests.Clone_PreservesCapacityDesiredLevelsAndTransit`. |
...
| `SetLogisticsOrder.cs` | Player-issued order to configure a ship's/base's logistics participation. **+ `CreateCommand_SetDesiredLevels(entity, changes)` (OPERATION BLUEPRINT-TO-STEEL B-orders RANK-1):** the write path for a trade hub's stockpile MIN/MAX targets (`LogiBaseDB.DesiredLevels`) — the reader (`LogisticsCycle.UpdateListings` shortfall/surplus pricing) was already live; this is the missing setter. A `(0,0)` pair removes an item's target. Deferred into `Execute` (the sim thread) **on purpose** — `DesiredLevels` is iterated by `LogiBaseProcessor`, so a UI-thread mutation mid-iteration throws "collection was modified" (why the order exists rather than a direct field write, unlike `TaxRate`). Valid on a **colony OR a station** (gated on `LogiBaseDB` in `IsValidCommand`). One verb, both seats (the AI sets targets through the same order). Gauge: `SetLogisticsOrderTests`. |
...
> **Build-state caveat (verified 2026-07-13):** the **base/bidding half is wired** — bases recompute listings and pick bids, ships find bases, estimate profit, and place bids (`UpdateListings` / `LogiBaseBidding` / `LogiShipBidding` all run on the 1-hour hotloop). But the **ship state machine past `Bidding` is NOT driven.** `LogiShipBidding` only flips `Waiting → Bidding`; `LogiBaseBidding` accepts a bid and issues the cargo-transfer + movement commands directly, but nothing ever sets the ship's `CurrentState` to `MoveToSupply`/`Loading`/`MoveToDestination`/`Unloading`/`ResuplySelf` — those enum states are declared, referenced only by `ShipLogisticsOrders.UpdateDetailString` for display, and otherwise unused. And `ShipLogisticsOrders.Execute()` is an **empty stub** (`ShipLogisticsOrders.cs` ~line 34). So the order-driven per-ship lifecycle is scaffolding; the working path is the base-side bid/accept loop calling `CargoTransferOrder.CreateCommands` + `Manuvers` inline. **A4 (OPERATION BLUEPRINT-TO-STEEL, 2026-08-13) DE-CRASHED it:** `Clone()` used to `throw new NotImplementedException()` (a sim-thread [FATAL] if ever standing-ordered); it now clones cleanly. `Execute` stays a documented display shim (the base-side loop does the real work); driving the per-ship `CurrentState` machine is a parked design decision (campaign log ADJUDICATION QUEUE). Gauge `OrderStubSafetyTests`. Runtime behaviour is unverified here (CI can't run the client).
