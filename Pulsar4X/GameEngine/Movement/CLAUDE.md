...
6. **A CO-LOCATED warp arrival threw "Speed Result is NaN" → a background-thread `[FATAL]` (fixed 2026-07-23).** Same failure *shape* as #5 (an unobserved throw on the sim thread kills the clock), different math. Under `StrictNewtonion` (the **default**, `GameSettings.cs`), a warp completion runs `WarpMoveProcessor.SetOrbitHereSimpleNewt`. A fleet "move to body" order sets the exit offset to `Vector3.Zero`, so `WarpMovingDB.ExitPointrelative` is a **literal `(0,0,0)`** — the ship arrives co-located with its target. That zero position feeds `OrbitMath.KeplerFromPositionAndVelocity`, whose `SemiMajorAxis` collapses to **+0** (a zero position makes specific orbital energy `-Inf` → `SMA = -sgp/(2·-Inf) = +0`; only an *exactly*-zero position does this — a tiny non-zero offset stays finite). Downstream, `GetStateVectors` → `ObjectLocalVelocityPolar` → `InstantaneousOrbitalSpeed(sgp, distance=0, sma=0)` computes `|2/0 − 1/0| = |Inf − Inf| = NaN` and threw at `OrbitalMath.cs`. On the background sim thread that's an unobserved `[FATAL]` (it fired 7+ times in a committed `game_logs/` session, killing the clock). **Trigger:** any warp to a body when the exit is co-located — e.g. the Operation-Earthfall UMF fleet warping to Earth. **Fix (two layers, mirroring #5):** (a) **primary** — `SetOrbitHereSimpleNewt` now detects the degenerate geometry SOI-INDEPENDENTLY (a literal-zero `ExitPointrelative`, OR a derived orbit with `SemiMajorAxis == 0`/non-finite/non-finite eccentricity) and settles into a clean circular orbit via the proven `SetOrbitHereNoNewt` (the `!StrictNewtonion` twin) instead of building the crashing `NewtonSimpleMoveDB`. **Do NOT gate this on `GetSOI_m()`** — it returns `+Infinity` for a root/`OrbitUpdateOftenDB` target, so a SOI-relative threshold would divert *every* arrival (not byte-identical). (b) **defensive backstop** — `OrbitalMath.InstantaneousOrbitalSpeed` and `GetStateVectors` now **bail to a finite value** (0 / zero-velocity) with a `[ORBIT]` breadcrumb instead of throwing on non-finite, so no degenerate orbit can ever again become a `[FATAL]`. Byte-identical on every physical input (both changed only a branch that fired on NaN). Gauge: `WarpArrivalNaNGuardTests` (pure math — `InstantaneousOrbitalSpeed(sgp, 0, 0)` no longer throws + a zero-position Kepler solve reads degenerate). **Lesson reaffirmed: `2/r − 1/a` with `r` and `a` both zero is `∞ − ∞ = NaN`; any vis-viva on a possibly-degenerate orbit needs a finite-guard, and a sim-thread throw is a clock-killer, not an error.**
