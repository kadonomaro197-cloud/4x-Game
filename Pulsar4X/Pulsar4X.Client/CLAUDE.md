...
| `ShipDesignWindow` | `ShipDesignWindow.cs` | ✅ Functional | **The ENTITY ASSEMBLER** (window title + toolbar tooltip retitled from "Ship Design" 2026-07-14 — the developer's naming: the Component Designer makes the PIECES, this ASSEMBLES them into any buildable entity — ship / station / building / ground unit). Assembles a chassis + a list of components; stats emerge from the sum. v1 assembles ship-class designs; generalising to every entity kind is in-progress (**same window, never a new per-domain one** — `docs/economy/UNIVERSAL-ASSEMBLY-DESIGN.md` §0). Class name still `ShipDesignWindow`. |
