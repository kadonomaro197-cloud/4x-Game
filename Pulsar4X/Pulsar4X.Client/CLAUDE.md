...
| `StationWindow` | `StationWindow.cs` | ✅ New 2026-07-03 (Slice A — compile-checked, runtime-unverified) | The space-station front door's management half — a per-entity window opened from the map context menu ("Manage Station", gated on `StationInfoDB`). Header (hosting body / structural-integrity durability pool / population) + the **host-agnostic `IndustryDisplay`** so a deployed platform with a constructor can queue+install modules in-situ. Thin defensive draw: `Window.Begin/End` wrapper + try/catch body so a throw can't skip `End`. Deploy side: `SystemWindow` "Deploy Station" button → `DeployStationOrder`. |
