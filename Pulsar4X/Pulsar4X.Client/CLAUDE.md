...
| `ComponentDesignWindow` | `ComponentDesignWindow.cs` | ✅ Functional | Component designer with NCalc formulas. **Left panel reorganised 2026-07-14 into "Make New Component"** — collapsible category headers (the ComponentType "Doors": Weapons/Sensors/Power/…), each listing its templates as "＋ New \<template\>" that start a BLANK design from scratch (`SetTemplate`, not a copy). Replaced the single-category filter combo. The middle "Current Component Designs" list edits an existing design (`SetFromComponent`) and is reset when a category with no designs is picked. **Crash-hardened:** the `Description` getter for a component/property can no longer take the whole client down — an NCalc-unparseable description (an apostrophe) falls back to raw text (`ComponentDesignProperty`/`ComponentDesigner` in the engine); the window's tooltip Description lookup is `TryGetValue`-guarded. |
