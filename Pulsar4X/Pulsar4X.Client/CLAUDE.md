...
**Selecting a fleet froze the game — an unbounded parent-walk hit the SELF-PARENTED root (2026-07-03).** Live `[HANG] … wedged in stage: 'FleetWindow'` the instant the player clicked the 1st fleet — the watchdog above named the culprit window, which is the ONLY reason this was diagnosable (no exception: it's a freeze, not a throw). Root cause: `FleetWindow.GetVisibleParent` (called in the **Summary tab**, the default tab, to show the fleet's "Orbiting" body) walks up the flagship's `PositionDB.Parent` chain in a `while (parent != null)` loop with **no cycle guard**. But the engine **self-parents root bodies** — a root star's `PositionDB.Parent` points at **itself**, not null (`MoveState`/`PositionDB.AbsolutePosition` special-case `Parent == OwningEntity`; the engine guards this in its own position math). The loop only stops on null / a visible parent / a parent with no PositionDB — so when the walk reaches the self-parented star (or ANY parent-chain cycle) and that node isn't in `systemState.AllEntities`, `parent = parentPositionDB.Parent` returns the same entity forever → the main loop wedges. **Fix:** a `HashSet<int> visited` guard — `while (parent != null && visited.Add(parent.Id))` breaks the instant an id repeats. **Rule: any UI walk up `PositionDB.Parent` (or any entity tree) MUST carry a visited-set / cap — the engine's roots are self-parented, so an unguarded `while (parent != null)` never terminates.** (Pre-existing latent bug, not new; the scenario's extra fleets just made it easy to hit. The gauge — the hang breadcrumb — is what turned an un-tebuggable freeze into a one-line diagnosis.)
