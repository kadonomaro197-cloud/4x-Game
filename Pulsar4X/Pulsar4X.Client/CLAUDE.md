...
| `FleetWindow` | `FleetWindow.cs` | ✅ Functional | Fleet listing, selection, basic orders. **+ Combat tab (2026-06-25)** — see "Fleet Combat tab" below. **+ Force Management retitle + Battalions tab (Earthfall C3.1, 2026-07-18)** — the window title is now **"Force Management"** (the class stays `FleetWindow` — renaming it orphans `LoadedWindows`/save refs, R1 ledger), and its content splits across a top-level tab bar: **Fleets** (the entire existing fleet manager, verbatim, inside its own tab — byte-identical) and **Battalions** (the new cross-body ground-formation manager). See "Battalions tab" below. **+ Embark / Land Troops order (Earthfall C5.1, 2026-07-19)** — a new `IssueOrderType.Troops` in the Fleets ▸ Issue Orders tab (gated on the fleet having a troop-bay ship): per bay ship, embark the player's ground units standing on the body it orbits (`LoadTroopsOrder`) and land loaded units onto a region-picked target once it holds the orbit (`LandTroopsOrder`). See "Embark / Land Troops" below. **+ Toggle Inherit Orders (B-orders C1, 2026-08-15)** — the Fleets ▸ Issue Orders ▸ *Order List* section shows an **"Inherit Orders"** checkbox ONLY for a **sub-fleet** (parented to another fleet, not the faction root) → `FleetOrder.ToggleInheritOrders`. Un-commented + repaired the dead wire (the old `FleetDB.Parent.Guid` check is stale — entities key by int `Id`, parent lives on `TreeHierarchyDB.Parent`; the client replicates the engine-internal `CombatEngagement.IsSubFleet` test off the public `TreeHierarchyDB.Parent`). Checkbox seeds from `FleetDB.InheritOrders` (default ON) on selection. Client-only; engine byte-identical. **+ Categorized order menu (B-orders C9, 2026-08-15)** — the Fleets ▸ Issue Orders LEFT list is reorganized from a flat 6-item list into Movement/Survey/Logistics `CollapsingHeader`s (`DrawCategorizedOrderList`); capability-gated orders (Geo/Grav survey, troop lift) now **DIM** (`BeginDisabled` + a greyed "— needs …" reason) instead of hiding — cradle-to-grave made legible. `IssueOrdersDisplay` (right detail panel) is byte-identical (same `selectedIssueOrderType` targets). Engine untouched; F2/F3 follow-ups re-shelve battalion orders + route every force kind through the tree. **+ Intercept a hostile fleet (B4a, Operation Blueprint-to-Steel, 2026-08-17)** — a new `IssueOrderType.Intercept` in the **Movement** category (always applicable to a fleet, so never dimmed): the detail panel lists the FOG-AWARE detected hostile fleets (`Combat.CombatEngagement.DetectedHostileFleets(SelectedFleet)`), one button per enemy fleet → warp the fleet's ships toward that enemy's representative (flag)ship (`FleetTools.RepresentativeShip(enemy)`) via `Movement.WarpFleetTowardsTargetOrder.CreateCommand(SelectedFleet, repShip)` → `HandleOrder` (a `FleetDB` has no `PositionDB`, so we target a positioned ship). Closes the distance so the auto-trigger/closing model starts the battle at weapon range — the "close on that enemy" primitive that was missing (only the `MoveToSystemBodyOrder` idiom, one verb over). Empty list → a "no detected hostile fleets" note (send a scout first). The SAME order the AI can drive (one verb, both seats). Engine byte-identical (a new client order type + the additive `DetectedHostileFleets`/`RepresentativeShip` read-only helpers, CI-gauged by `InterceptTargetingTests`). Runtime is the developer's build. **+ Rearm ordnance at a base (B-orders C7, 2026-08-15)** — a new `IssueOrderType.RearmAt` in the Logistics & Cargo category: mirrors "Refuel at ..." (warp the fleet to a picked base + issue `CargoTransferOrder.CreateCommands` WaitTillFull) but lets the player pick BASE **and** ORDNANCE (`OrdnanceDesign : ICargoable`; the right panel enumerates each colony's ordnance-in-stock via `GetCargoables().OfType<OrdnanceDesign>()`, transfers per ship with `GetFreeUnitSpace > 0`), so a magazine never fills with rounds the launchers can't fire. Two-stage: fills the ship's ordnance HOLD; the launcher's internal magazine reloads from it (`GenericFiringWeaponsProcessor`). No engine change. Stations-as-base + auto-pick-launcher-ordnance are follow-ups. **+ Dock / undock vessels (B-orders C8, 2026-08-15)** — a new `IssueOrderType.Dock` in the Logistics & Cargo category (dimmed via `HasAnyCarrier` when no ship in the fleet carries a docking bay): per carrier ship (`DockTools.Capacity > 0`) the panel shows berth used/door, an **Undock** per `DockTools.DockedShips`, and a **Dock** per other fleet ship (greyed with `CanDock`'s human-readable reason on refusal), issuing the RANK-3 `DockOrder.Dock/Undock`. **Makes docking cradle-to-grave playable** — the DockBay component is already offered in the Entity Assembler, so you design a carrier → build it → dock/undock here. Content-gated (byte-identical until a hull carries a bay). `+ using Pulsar4X.Docking`. |
...
| `ShipDesignWindow` | `ShipDesignWindow.cs` | ✅ Functional (+ GROUND-unit branch 2026-07-15, compile-checked, runtime-unverified) | **The ENTITY ASSEMBLER** (window title + toolbar tooltip retitled from "Ship Design" 2026-07-14 — the developer's naming: the Component Designer makes the PIECES, this ASSEMBLES them into any buildable entity — ship / station / building / ground unit). Assembles a chassis + a list of components; stats emerge from the sum. **Now assembles BOTH ships AND ground units in the SAME window, branching on the CHOSEN CHASSIS** (rides the additive `Pulsar4X.Interfaces.IChassisAtb` seam — `ShipHullAtb` → Ship, `GroundChassisAtb` → Ground). Detection: `SelectedChassis()` scans the design's components for a part carrying an `IChassisAtb`; `IsGroundAssembly()`/`ActivePartMount()` read its `BudgetKind`/`PartMount`. An **"Assembling: Ship / Ground Unit" combo** (default Ship) bootstraps an EMPTY design (a chassis, once mounted, LOCKS the kind). The parts-list filter now uses the chassis's `PartMount` (Ship → `ShipComponent` as before; Ground → `GroundUnit`), so a ground chassis shows ground frames/parts. Ground SAVE (`SaveGroundDesign`) routes to `GroundUnitAssembly.RegisterAssembledDesign(faction, id, name, frame, parts)` (registers a buildable `GroundUnitDesign` on `IndustryDesigns`, NOT a `ShipDesign`); ground STATS/validity come from `GroundUnitAssembly.Compute` (`DisplayGroundStats` — carry-budget readout + emergent Attack/Defense/HP/etc. + assembler `Problems`), skipping the ship thrust/warp/energy math + the armor block + the armor-null throw. **The SHIP path is byte-identical** (combo defaults to Ship, no ground chassis → old behaviour: same filter, same stats, same save). Crash-hardened: `RefreshExistingClasses`/`DisplayExistingDesigns` no longer hard-index `ShipDesigns[id]` (all `TryGetValue`-guarded, L14); ground draw wrapped so a throw logs `[RenderError]` once and can't skip `Window.End`. Logs `[ACTION] ground-unit design saved…`. v1: generalising to every entity kind is in-progress (**same window, never a new per-domain one** — `docs/economy/UNIVERSAL-ASSEMBLY-DESIGN.md` §0). Class name still `ShipDesignWindow`. **+ C4.1 (Operation Earthfall, 2026-07-19):** `DisplayGroundStats` gained three readout rows off `GroundUnitAssembly.Compute` — **Training** (`TrainingMultiplier`, cadre veterancy, previously invisible), and ALWAYS-ON **Power (draw / supply)** (`EnergyDemand_W`/`ReactorSupply_W`, red UNDER on shortfall) + **Ammo Capacity** (`AmmoCapacity_kg`); the power/ammo gates were violation-only red text before. Display-only (no new numbers, Problems verdict unchanged). **+ "Units per build" dial (squad size, 2026-07-21):** an `InputInt` in the ground Details section (`_groundUnitsPerBuild`, clamped ≥1) passed to `RegisterAssembledDesign(..., unitsPerBuild)` — one build of the design raises that many units; the engine scales the design's build cost + time by it (no free multiplication). Ground-assembly only; default 1 → byte-identical (ship path untouched). **+ "Mass Budget (hull)" readout (scorecard #2, Operation Blueprint-to-Steel 2026-08-21):** `DisplayStats` (the SHIP path) gained one row after "Mass (Dry)" showing the mounted hull's `IChassisAtb.StructuralBudget` vs the design's current `_massDry` (`current / budget`, with a red **OVER BUDGET** flag when `_massDry > budget`), read FRESH off `SelectedChassis()` (no stale design state). This closes the mockup's "Budgets" panel gap on the ship side — the ground/station/building panels already showed their carry/structure/footprint budget, so the ship path was the last one missing its cap readout. Shown only once a hull is mounted (`BudgetKind == Mass`); a hull-less ship has no cap so nothing draws → byte-identical for legacy hull-less designs. **The Entity Assembler is functionally complete across all four host kinds** (`_assemblyKindNames = { "Ship", "Ground Unit", "Station", "Building" }`, each with its own stats method + save path); the mockup's remaining delta is cosmetic panel-organization, deliberately NOT rewritten blind (CI can't run the client; this is a working critical window with the Window.End/cascade/printf landmines). Client-only, engine byte-identical. Runtime is the developer's local build. |
| `ComponentDesignWindow` | `ComponentDesignWindow.cs` | ✅ Functional | Component designer with NCalc formulas. **Left panel = the "Make New Component" taxonomy (reorganised 2026-07-14 to the DESIGN doc).** Two-level tree: **Category (11) → Door (37) → templates**, per `docs/archive/economy/COMPONENT-DESIGNER-CATEGORIES.md` (§2), via the `ComponentDoors` map (`ComponentDoors.cs`: template-id → Category ▸ Door, with an "Other"/raw-ComponentType fallback so nothing vanishes). Click **Weapons** → Energy/Ballistic/Melee/Guided/Exotic; a cargo hold and a fuel tank both sit under **Logistical ▸ Storage**; a hull is **Chassis ▸ Hull**. Each template is a "＋ New \<template\>" that starts a BLANK design (`SetTemplate`, not a copy). Middle "Current Component Designs" list edits an existing design (`SetFromComponent`), reset when a door with no designs is picked. **NOTE:** the window only shows templates the VIEWED faction has UNLOCKED (`Data.ComponentTemplates`); the DevTest player now has all design templates unlocked (uef-devtest `startingItems`) so every door is populated — the earlier "can't build a weapon" was the trimmed weapon templates being locked. **Crash-hardened:** the engine `Description` getter falls back to raw text on an NCalc-unparseable description (apostrophe); the tooltip Description lookup is `TryGetValue`-guarded. **+ Slice-2 HYBRID form (OPERATION BLUEPRINT-TO-STEEL, 2026-08-22, compile-checked / runtime-unverified):** `ComponentDesignDisplay.GuiDesignUI` now DISPATCHES — the door (tree) + the "Type" dropdown are the two CHOICES, then a template's **CORE sliders** show up front and its fine dials tuck behind a default-CLOSED **"Advanced settings"** `CollapsingHeader` (`GuiDesignUIHybrid`). **WHICH dials are fine is DATA, not client code (slice 2b):** a per-property **`"Advanced": true`** marker in the template JSON (`ComponentTemplatePropertyBlueprint.Advanced` → the public `ComponentDesignProperty.IsAdvanced`, byte-identical — no atb/save change). A template that marks ≥1 settable dial Advanced gets the split; one that marks NONE renders the OLD flat list (`GuiDesignUIFlat`) → byte-identical. So the split lives next to the dial (per DESIGNER-NORTH-STAR "the door is derived from the data") — **no client code per door**, a modder marks a dial and its layout follows. The tags are now authored across ALL doors (2026-08-22): `weapons.json` (laser's 7 fine physics dials; Tracking on railgun/flak/plasma; Tracking+Recoil on siege), `installations.json` (ground units' Penetration/PerShotEnergy; ground weapons' CarryMass/Penetration/PerShotEnergy; plating's CarryMass + 4 per-nature soaks; augments' off-purpose sibling stats; command-berth Support/Span; infra/habitat storage+comfort+tolerance; bunker footprint), `ordnance.json` (missile-payload shaped-charge geometry; the two seekers' EM wavelength/bandwidth), `electronics.json` (passive-sensor wavelength/bandwidth). **Power/Propulsion/Logistical/Docking need nothing — every template there has ≤3 all-meaningful dials, so they render flat by default.** Most templates game-wide stay flat; only the ~26 with a genuine primary/secondary split carry tags. Both layouts call the SHARED `RenderDesignProperty` (the exact per-GuiHint switch, PushID/PopID balanced), so it re-groups the SAME render calls — a saved design is identical to the flat layout, and the `CreateDesign` (tech + `IndustryDesigns` + save/load) path is untouched (cradle-to-grave intact). The SAVE path is NOT wired to the slice-1 `WeaponsDesignModel` calculator (that would be a parallel non-cradle-to-grave path) — a design still saves through the template's own atb path. **+ B2 Weapon Profile READOUT (2026-08-23):** `ComponentDesignDisplay.DisplayWeaponProfileReadout` (called from `GuiCostText`) draws, for the generic parametric weapon ONLY (content-gated on the presence of the Delivery / Nature / Damage-Per-Second dials → every other template byte-identical), the weapons HTML's "ten numbers reach the fight" panel: the RESOLVED `WeaponProfile` (the delivery FORCES the velocity + reach the raw dials don't show — a Beam knifes at light-speed, a Slug takes the railgun class range), the emergent weapon-triangle CORNER (`WeaponProfile.Class`, computed not chosen), and a "does it land?" read via `CombatKernel.HitFraction` vs a nimble (evasion 0.9) / sluggish (0.1) target. It reads the SAME `WeaponsDesignModel` the fidelity gauge proves reproduces every base-mod gun — but READ-ONLY (display), so the cradle-to-grave save path is untouched; fully defensive (a value caught mid-edit omits the panel rather than throwing into the design panel — the cascade rule), `%` via `TextUnformatted`. **+ Path C "Design reads as" readout for ALL 12 doors (2026-08-23, 2 commits — build then check):** `ComponentDesignDisplay.DisplayDoorReadout` (also called from `GuiCostText`) dispatches by `ComponentDoors.Classify(Template.UniqueID, ComponentType).category` and shows, per door, the two things the design HTMLs foreground that the stat table did NOT — the CONNECTIONS (a per-door "Feeds →" line: Power→power budget, Sensors→fog of war, Defense→survivability, Civic→society, …) and a door's headline DERIVED behaviour read from its REAL dials: **Defense** nature-matchup (strongest/weakest vs Kinetic/Energy/Explosive/Exotic, off the `Vs*` dials — the armour/shield analog of the weapon triangle), **Enhancers** caliber `×`-mult (the `Firepower/Toughness Caliber` dials, confirmed `×`-mults by `ShipCaliberTests`), **Civic** support/comfort→morale, **Power** reactor endurance in hours. The plain numeric readouts were already shown as the templates' `GuiTextDisplay` stats, so this adds the missing connection + behaviour layer, not duplicates. Fully defensive (static text can't fail; the derived switch reads dials via `TryGetValue` inside a guard, so a bad/mid-edit read omits a line, never crashes); display-only → design/save byte-identical. The other 11 doors' one-form parametric collapse (like Weapons) + the shelved-feature doors (aura Rally/Dread, civic recreation/market) remain adjudication-queue items. Engine byte-identical (the JSON field + computed accessor only). Runtime is the developer's local build. |
...
| `SiteWindow` | `SiteWindow.cs` | ✅ New 2026-07-13 (Site Engine SE-6b — compile-checked, runtime-unverified) | The player-facing **Field Sites** panel — toolbar button (next to Diplomacy/Intelligence). Lists every discovered `FieldSiteDB` across the player's known systems (`_uiState.StarSystemStates` → each `StarSystem.GetAllEntitiesWithDataBlob<FieldSiteDB>()`): per site a collapsing header (name / space-or-surface / status) + Role/Shape/Yield, work banked + understanding accrued, who's working it, and — for a **branched** site (`FieldSiteDB.HasBranches`) — one **Commit: \<branch\>** button per branch (enabled only when `SiteMachine.IsBranchUnlocked` AND the player's faction worked it), issuing the CI-tested `Sites.CommitSiteBranchOrder` via `Game.OrderHandler.HandleOrder` — the piece that makes the SE-5 branch CHOICE reachable by a player (a branched site waits at Worked until you pick here). Single-path demo sites show a progress readout (watch understanding climb → resolve). Thin defensive draw, mirrors `DiplomacyWindow` (GetInstance + `Window.Begin`/try/`End`, no hard-index). Placeholder icon (`Img_Select`). **Runtime feel is the developer's local build (CI compiles the client, can't run it).** |
| `AIInspectorWindow` | `AIInspectorWindow.cs` | ✅ New 2026-07-14 (B4b — the observability spine; compile-checked, runtime-unverified) | The live view of the **AI flight recorder** — toolbar button (next to Diplomacy/Intelligence). For every NPC faction: its current objective + why (`PlanReadout.Faction`) and a scrolling **decision tape** (each cycle's `AIDecisionRecord` as a line — what it SENSED/DECIDED/ACTED, via `PlanReadout.DecisionLine`), read off the per-faction `AIDecisionRecordDB`. The SAME data the client flushes to `game_logs/` as `[AI]` lines (`SessionLog.AiDecisionSnapshot`, from the ~3 s heartbeat) — the log tape is primary, this is the convenience view. Thin/defensive, mirrors `DiplomacyWindow` (GetInstance + `Window.Begin`/try/`End`, unconditional `EndChild`, no hard-index). **The decision tape's child now carries `ImGuiWindowFlags.HorizontalScrollbar` (2026-07-17)** — each `[AI]` line is ~300 chars (sensed \| decided \| acted) drawn UNWRAPPED via `TextUnformatted`, so without it the right half was clipped; the scrollbar lets you scroll right to read the whole line (the developer's "see what the AIs are doing in their entirety"). **Runtime is the developer's local build (CI compiles, can't run).** |
...
internal Entity? Faction                  // the currently VIEWED faction entity (internal + nullable; SM mode swaps it to the Game Master — see gotcha #10). GlobalUIState.cs:65
...
the only way to know the real state is to run it (the live state dashboards are `docs/SYSTEM-CONNECTION-MAP.md` for connections, `docs/TESTING-TRACKER.md` for gauges, `docs/DOCS-INDEX.md` for currency, and `docs/CLIENT-TEST-CHECKLIST.md` for the runtime backlog — the old `SYSTEMS-STATUS-AND-TEST-PLAN.md` was RETIRED 2026-07-27). If something
...
A **third top-level tab, "All Forces"**, beside Fleets and Battalions — the design's "front door" (`docs/combat/FORCES-WINDOW-DESIGN.md` §4.2/§4.3): ONE flat, common-column table over EVERY force the player owns, ships AND battalions, space AND ground, so "what do I have and where" is answered in one place. `DisplayAllForces()`:
1. **Gather** — ships by recursing `PlayerFaction`'s root `FleetDB` (`AllShipsUnder`/`CollectShips`, cycle-guarded by fleet id — CombatFleetTreeSafetyTests discipline — collecting the `IsValid && !HasDataBlob<FleetDB>()` leaves, the exact filter the Fleets tab uses); battalions via `GroundFormationTools.AllFormationsFor(game, PlayerFaction.Id)` (the same cross-body helper the Battalions tab uses). Each folds into a `RosterEntry` (a nested readonly struct) keyed by a `ForceRef` (the S4 selection abstraction — `OfShip`/`OfBattalion`).
2. **Common columns (§4.2)** — Unit / Domain (Space·Ground) / Kind (Ship·Battalion) / **Class** (`ShipRoleTools.ClassifyRole` for a ship = Warship/Survey/Freighter/Transport/Tender/Hauler/Utility; `FormationClass` = the plurality `GroundRoleComposer.ClassifyRole` across a formation's members = Line/Artillery/Screen/Support/Mixed) / **Mil-Civ** (`ShipRoleTools.IsMilitary` — only a warship is military; a battalion is always military) / Location (`ShipLocation` = system / orbited body via `GetVisibleParent`; `BattalionLocation` = world + R#) / Strength (`ShipCombatValueDB.Firepower` · `FormationStrength`). **Filters:** Domain / Role(Mil-Civ) / search (name·class·location).
3. **Kind-swapping detail panel (§4.3)** off the new `_selRoster` `ForceRef` (`DrawRosterDetail`) — a **battalion** reuses the SAME self-contained `DrawBattalionOrders` surface the Battalions tab gives (march/queue/stance/ROE/rename/infra); a **ship** shows a compact combat readout (Firepower/Toughness/Evasion) + a **Select on map** button (`_uiState.EntityClicked`). Ship movement/fleet orders stay in the Fleets tab.

**Connections (Prime Directive):** reads `FleetDB`/`ShipCombatValueDB`/`ShipRoleTools` (space) + `GroundForcesDB`/`GroundFormation`/`GroundFormationTools`/`GroundRoleComposer` (ground); writes nothing except through the CI-tested `GroundForces`/`GroundFormationDoctrine` order APIs `DrawBattalionOrders` already uses. **Thin + defensive:** the whole tab body is try/catch-wrapped (logs `[RenderError]` once via `_rosterErrorLogged`, still runs `EndTabItem`), nothing hard-indexed, evasion rendered as `F2` (not `P0`) through `TextUnformatted` to dodge the ImGui `%` printf trap. **The Fleets + Battalions tabs are byte-identical** (the class stays `FleetWindow`). CI compiles the client but can't run it — live render/feel is the developer's build (CLIENT-TEST-CHECKLIST "B-S5").

**+ B-S6 (2026-08-13) — per-individual ground-unit rows (§S6).** A **"Show individual units"** checkbox (`_rosterShowUnits`, default OFF → the S5 formation-level view is byte-identical) that, when on, interleaves per-unit rows into the roster: each battalion is followed by its member units (`GroundFormationTools.MembersOf`) as indented `└` child rows, and the **loose (formation-less) units** — the piece a formation-walk can't reach — are listed as `•` rows from the new engine helper `GroundFormationTools.AllUnitsFor(game, factionId)` (filtered to `FormationId < 0`; a formed unit is already under its battalion). A unit row is a new `ForceKind.GroundUnit` (`ForceRef.OfGroundUnit(body, u)`, keyed by body+unitId), Class = `GroundRoleComposer.ClassifyRole(u)`, Mil/Civ = `Attack > 0`; selecting it swaps in `DrawGroundUnitDetail` — the unit's own stats (type/class/health/attack/defense/range/veterancy/location) + an Open-planet-view jump (per-unit orders are a flagged follow-up; the battalion is the command surface). Also this slice hardened the roster's detail/table against the **`%` printf trap** the B-S5 adversarial verification flagged: user-renamable ship/body **names + locations** now render through a printf-safe `RosterDetailHeader` + `TextUnformatted` (a ship named `Foo %s` could otherwise garble/crash ImGui's `Text`). Engine gauge: `EfGroundFormUpTests.AllUnitsFor_EnumeratesAcrossBodies_IncludesUnformed_FactionFiltered`. Runtime is the developer's build (CLIENT-TEST-CHECKLIST "B-S6").

**+ B-S7 (2026-08-15) — civilian-ship detail (§4.3 "manifest / route / trade-space / survey").** The roster's ship detail (`DrawRosterDetail` ship branch) now branches on `e.Military`: a **warship** keeps just the combat sheet (unchanged); a **civilian** ship (Freighter/Hauler/Tender/Transport/Survey/Utility) gains `DrawCivilianShipReadout` below the combat line — its **cargo manifest** (REUSES `CargoStorageDBDisplay.Display(es, _uiState, ImGuiTreeNodeFlags.None)`, the verbatim call `EntityWindow.cs:1272` makes; needs the ship's `EntityState`, resolved by `ResolveEntityState` = the `JumpToPlanetView` walk-the-`StarSystemStates` idiom, degrades to a one-line note if the ship isn't in the active view), its **trade route + state** if it carries a `LogiShipperDB` (`StateString`/`CurrentState` + each `ActiveCargoTasks` item From→To), and a **survey-vessel** note for a `ShipRole.Survey` ship (per-target survey progress lives on the surveyed body — a flagged follow-up). DATA-grade (the numbers already exist — the Logistics window renders them; this promotes them into the roster). Thin/defensive: reads only, `TextUnformatted` for every user-renamable name, no hard-index, own-ships-only (roster is `PlayerFaction`-scoped, so the cargo panel's `GetFactionOwner` is safe). Runtime is the developer's build (CLIENT-TEST-CHECKLIST "B-S7").

**+ B-S9b-1 (2026-08-15) — colonies + stations as HOLDINGS (§S9).** A new **`ForceDomain.Holding`** (+ `ForceKind.Colony`/`Station` and `ForceRef.OfColony`/`OfStation`): the roster now lists the player's colonies + stations as rows, gathered through the engine **`FactionAssets.OwnedColonies`/`OwnedStations`** helpers (B-S9a). Those filter the **capture-STALE** `FactionInfoDB.Colonies`/`Stations` registry (added on creation but NOT cleared on capture — `GroundForcesProcessor` flips a taken planet's live owner but leaves it in the old list) to the assets whose LIVE `Entity.FactionOwnerID` still == the faction — so a colony lost to invasion drops off the roster (the §S9 live-owner cross-check, done engine-side + CI-tested by `FactionAssetsTests`). A holding row: Class = its host body, Location = the host's system (reused `ShipLocation`), Strength = population (both `ColonyInfoDB` and `StationInfoDB` carry a `Population` dict), Mil/Civ = Civilian, Health = the **station's structural-integrity** fraction (`StationInfoDB.StructuralIntegrity/BaseStructuralIntegrity`) or "—" for a colony (no aggregate HP — its strength is population). Selecting one → `DrawHoldingDetail`: host + population + (station) integrity + the **installed infrastructure** reusing `ComponentInstancesDBDisplay.Display` (resolved via the B-S7 `ResolveEntityState` walk) + an Open-planet-view jump. The holding entity rides the `RosterEntry.Ship` field, but `DrawRosterDetail`/`RowHealthFraction` dispatch on `Domain==Holding` FIRST so no ship logic ever sees it. **Assign-commander (B-S9b-2)** is BUILT — `DrawHoldingAdminPosts(holding)` reads the holding's `AdminSpaceDB.CommanderSeats` (a holding with an admin-complex has them) and per seat renders a combo of `FactionInfoDB.Commanders` + an **Assign** button (`AssignAdministratorOrder.Create(holding, commanderId, seat.ComponentName)`) + an **Unassign** button (`UnassignAdministratorOrder`), issued via `_uiState.Game.OrderHandler.HandleOrder`. The wire is verified to actually seat the officer (not just compile): both `ColonyFactory` + `StationFactory` attach `OrderableDB`, so `HandleOrder` enqueues → `OrderableProcessor` runs `Execute`; the whole path is try/catch-wrapped (`[OrderError]`) and never calls the order's throwing `Clone()`. Thin/defensive: reads only, `TextUnformatted` for every user-renamable name (the `%` trap), no hard-index. **Engine byte-identical** (client-only; `+ using GameEngine.People`/`Pulsar4X.People`/`Pulsar4X.People.Orders`). Runtime is the developer's build (CLIENT-TEST-CHECKLIST "B-S9").
...
A live freeze (`[HANG] main loop STALLED ~5s`) hit right as the **combat interrupt auto-opened `BattleReportWindow`** on a heavy frame (36 ships / 286 orbits / 20 contacts). Root defect found: **`BattleReportWindow.Display()` called `ImGui.End()` only INSIDE its `if(ImGui.Begin(...))` block** (plus an early `return` that did the same). Dear ImGui requires `End()` for **every** `Begin()` — *even when `Begin()` returns false* (window collapsed/clipped) — unlike `BeginTable`/`BeginChild`/`BeginMenu` (End only if it returned true). So a collapsed Battle Report window skipped `End()` → the `[imgui-error] … 'Battle Report': Missing End()` spam seen in the logs → an unbalanced ImGui window stack on the combat frame. **Fixed:** `End()` is now unconditional (moved after the `if`), the empty case is an `else` (no early return), `EndTable` stays inside its own `if`. **Rule: a top-level `if (ImGui.Begin(...)) { … ImGui.End(); }` is a bug — put `End()` after the block, always.** ⚠ **CORRECTED 2026-07-28 — the sentence that used to sit here was FALSE and it hid a live 16-window bug.** It claimed *"windows that use the `Window.Begin`/`Window.End` wrapper are safe (the wrapper calls `End` unconditionally)"*. **The wrapper does NOT call `End` for you** — `Interface/Widgets/Window.cs:57-69` is a plain `ImGui.End()` plus a `#if DEBUG` counter, and **the caller must call it.** Worse: `Window.cs:22` `_beginCount` is **static**, incremented in `ValidateBeginCall` and decremented only in `End()`, **with no per-frame reset**. So in a **Debug build** (`dotnet run`'s default), any wrapper window that puts `Window.End()` *inside* its `if (Window.Begin(...))` block will, the first time it is collapsed or throws, leave the counter stuck — and **every subsequent `Window.Begin` in the process throws `InvalidOperationException` forever, blanking all ~52 wrapper windows for the rest of the session.** In Release the counter compiles out and ImGui's `ConfigErrorRecovery` cleans up at frame end. **16 windows have this shape**, including `ComponentDesignWindow.cs:120`, `ComponentsWindow.cs:203`, `ColonyManagementWindow.cs:108` and `ToolBarWindow.cs:277`. `ShipDesignWindow.cs:372` and `FleetWindow.cs:243` do it correctly (End outside the `if`). **The rule is the same for BOTH the raw `ImGui.Begin` and the `Window.Begin` wrapper: `End()` goes AFTER the block, always.** See `docs/COMBAT-DESIGNER-GROUND-TRUTH-2026-07-28.md` §7.2 R-3. **Follow-up audit flagged** (same pattern, not yet verified): `ColonyHexMapWindow`, `ManeuverNodePanel`, `Debug/DataViewerWindow`, `Debug/DebugGUIWindow`, `EntityUIWindowSelector`, `ColonyPanel`, and the `ModFileEditing/*` editors — check each puts `End()` after the `if`.
...
`docs/ground/GROUND-SURFACE-MAP-DESIGN.md` → "LOCKED PRINCIPLE." **Next: the "40k" ground-combat depth pass + a per-unit combat
readout** (the ground echo of the Fleet Combat tab).

**Build a MINE on a located deposit hex (2026-07-07).** The Build panel gained a second action beside the region "Build here": the last hex you click on the globe is remembered (`_selGQ/_selGR`), and if it holds a mineral deposit (`GroundHex.DepositMineralId >= 0`, seeded by `Industry.HexMinerals`) the panel shows **"Build \<Mine\> on this \<mineral\> deposit (Q,R)"** → `PlaceInstallationOnHexOrder.CreateCommand(colony, gq, gr, mineDesignId)` through `Game.OrderHandler.HandleOrder` (the CI-tested engine path). The mine design is auto-picked from the placeable installs by `HasAttribute<MineResourcesAtbDB>()`; the mineral name comes from the slice-2 `BuildMineralNames()`. This is "build a mine ON that deposit" — the mine sits on the ore hex. **✅ PER-HEX MINING + HAUL LANDED (R1b/H1, 2026-08-21):** a mine built on a hex now works THAT hex's deposit into the hex's local `Stockpile` (engine `IndustryTools.EnablePerHexMining`, `NewGameMenu`-on), and a **"Haul \<amount\> \<good\> to colony"** button appears under the mine-build action for each good sitting in the selected hex's bucket (`hex.Stockpile`) → `HexHaulOrder.CreateHexToColony(colony, _selGQ, _selGR, goodId, 0)` (all-on-hand) through `Game.OrderHandler.HandleOrder` — the CI-tested haul verb both seats issue. So the loop is playable: build a mine on a deposit → ore piles on the hex → haul it home. Inert until per-hex mining fills a bucket (byte-identical row otherwise). Thin defensive draw (try/catch, `##haul<id>` unique button ids, no hard-index). CI compiles; live feel is the developer's build.

**The CONTINUOUS-GLOBE window is now the ONLY surface view (G6a, 2026-07-05).** The developer validated the globe view live and asked to **drop the old per-region "band" view**. `DrawTacticalMap` now always calls **`DrawGlobalHexWindow`** (G5) — a sliding window over the ONE cylinder grid (`PlanetRegionsDB.SurfaceGrid`): centred on a longitude column (`_centerCol`, ~2 region-bands wide — centre band full + neighbours bleeding in), **wrapping at the seam** so any place shows in every window whose longitude reaches it. Units draw at their **GLOBAL (Q,R)**; click a hex to select your group / march via `GroundForces.OrderMoveToGlobalHex` (wrapping A\*, no edge gates); ◀/▶ pans by band. Uses an **odd-r OFFSET** layout (`HexCenterOffset`) so the wide window reads rectangular. **The band view is GONE:** the `◑ Globe / ▦ Band` toggle, `DrawThreeRegionHexMap`, the H4 `DrawHexMap` drill-in, and their dead helpers (`HandleHexClick` / `MoveSelectedToHex` / `HexCenter`) are **removed**. A **detailed collapsible legend** now sits at the bottom (`DrawLegend` + `LegendRow`): unit ownership + type initials + selection ring, and every terrain colour grouped (Water/Lowland/Upland/Cold/Gas, Ocean flagged impassable) + the seam/label/pan cues. The region-based side panels (selection/build/formation/detail) still drive off `_centerRegion`, which the globe keeps in sync (◀/▶ + clicks recentre the band). **Engine disks are still generated** (dormant scaffold; G6b retires them). Design: `docs/ground/GROUND-SURFACE-MAP-DESIGN.md` (G5/G6a). **Runtime render/feel is the developer's local build (CI compiles, can't run).** The superseded 3-region band view is described below for history.

> **CLICK-TO-MARCH now goes through the ONE queued FORMATION verb (D-units, OPERATION BLUEPRINT-TO-STEEL, 2026-08-16 — the developer's One-Verb-Both-Seats ruling: "the battalion/fleets is how you move units").** The two loose-unit bypass methods (`MoveSelectedToGlobalHex` / `MarchSelectedTo`) used to call `GroundForces.OrderMoveToGlobalHex` / `OrderMove` **directly** per unit — a bypass the AI can't issue (no order queue, no issuer marker). They now AUTO-WRAP the region+type selection into a formation (`WrapSelectionIntoFormation` — reuses the formation the selection already shares, else forms a new one exactly as the "Form up" button does) and issue `GroundForces.SetFormationOrder(formation, GroundOrder.MoveHex/MoveRegion)` — the SAME queued verb the AI drives (`GroundTacticalBrain` → `SetFormationOrder`). So the player and AI move ground units through the identical primitive; a loose unit marched becomes a one-member formation (b1), and to move a subset you split the formation (b3), mirroring fleets. `SetFormationOrder` (replace) = "march now" semantics; the order is processed on the next ground tick (like every fleet/formation order). **Client-only, engine byte-identical.** ✅ **The formation-march buttons are now ALSO queued (D-units follow-up DONE, Operation Blueprint-to-Steel 2026-08-19):** `PlanetViewWindow.MarchFormation` + `FleetWindow.MarchBattalion` swapped the direct `GroundForces.OrderFormationMove(body, f, target)` for `GroundForces.SetFormationOrder(f, GroundOrder.MoveRegion(target))` — the SAME queued verb the AI drives, so the last formation-level bypass is closed (immediate→queued: the order now validates + executes on the next ground tick, so the status line reads "ordered to march to Region N" instead of an immediate moved-count). Runtime is the developer's build (CLIENT-TEST-CHECKLIST "D-units").

> **The HOMEWORLD (Earth) rendered BLANK — survey-gate regression FIXED (2026-07-17).** Live symptom: the surface window opened on Earth but drew no hexes, only "This world isn't surveyed yet" — while the control strip read "surveyed 4/4" (the two survey concepts disagreeing was the tell). Root cause: `DrawTacticalMap` gates the globe view on `body.TryGetDataBlob<GeoSurveyableDB>(out geoDB) && geoDB.IsSurveyComplete(myFaction)` (`PlanetViewWindow.cs:190`), but **Earth carries NO `GeoSurveyableDB` at all** — its blueprint (`earth.json`) is the ONLY Sol body that omits `GeoSurveyPointsRequired`, because the homeworld was never meant to need surveying (`SystemBodyFactory.cs:217` only attaches the gauge when that node is present). With no gauge, `IsSurveyComplete` can never read true → the `&&` fogged Earth forever. This was a **regression** from moving the surface view off the retired band view (which fog-gated on the always-set faction-agnostic `Region.Surveyed`) onto the globe view (which reads the per-faction `GeoSurveyStatus`). **Fix (one line, client-only):** `bool playerSurveyed = !body.TryGetDataBlob<GeoSurveyableDB>(out var geoDB) || geoDB.IsSurveyComplete(myFaction);` — a body with NO survey gauge is treated as "no gate applies / already known" (renders immediately), while a body that HAS the gauge still requires the viewing faction to complete it (the anti-leak rule holds — every rival body carries the gauge, so it stays fogged until you scan/settle it). The `||` short-circuits, so `geoDB` is only read when `TryGet` succeeded — cannot NRE. **Runtime-unverified (CI compiles the client, can't run it) — the developer's local build confirms Earth's hexes now draw.**
...
**RANGE OVERLAY on the globe (Operation Earthfall C4.1, 2026-07-19) — the ground echo of the space map's range rings.**
`DrawGlobalHexWindow` gained a highlight pass (right after the terrain loop, before the seam lines + unit tokens): when
the player has a unit group selected on the globe, it **tints the hexes that group can reach** — faint **red** = weapon
reach (`GroundUnit.Range` in hexes), faint **green** = radar reach (the unit's best `GroundSensorAtb.Range_km` ÷ the
region's `GroundRangeTools.HexPitchKm`, the `GroundSensors.cs:42` km→hex conversion, read through
`GroundUnitEntity.TryGetBacking` → `ComponentInstancesDB` exactly like `GroundSensors.RevealFromUnits`). The disk shape is
the **odd-r OFFSET** hex distance the globe is DRAWN in — new private helpers `GlobalHexDistance`/`OddRToAxial` convert the
...
selected units (`_selFaction == myFaction` — you never see an enemy's ranges, the same rule the space rings use), and only
the visible window's hexes; a unit with no radar component (garrison / DevTools-raised — no backing store) shows no green.
**Toggle MIRRORS the space map's global switch** (`GlobalUIState.ShowAllRangeRings`, default on; DevTools › Detection /
Fog of War). Thin/defensive (the radar read is its own try/catch; no state mutation, no engine value changed). Engine adds:
NONE (the optional `GroundSensors.RadarReachHexes` helper R1 flagged is a GROUND follow-up — the client computes km÷pitch
inline until it merges). CLIENT-TEST-CHECKLIST: "OPERATION EARTHFALL — C4.1." Runtime feel is the developer's local build.

**K-track real-distance client draws (compile-only, 2026-07-22) — units-on-mini-hexes + fog-honest enemy reach + formation plan lines.** Three additive draws over the K-track engine (`docs/AUTO-RESOLVER-GROUND-TRUTH-2026-07-29.md §12`; weapon reach already reads the REAL `GroundUnit.Range_m` via `WeaponReachHexesFor`→`GroundRangeTools.HexesForMetres`):
- **C1** `DrawCityZoom` now draws the UNITS standing in the zoomed operational hex (`GlobalQ/GlobalR == _zoomQ/_zoomR`) on the mini-hex tiles — at their `MiniQ/MiniR` tile (the same axial space the tiles use) plus the K2 sub-tile real offset (`MiniOffX_km`/`MiniOffY_km` → px via the mini pitch), coloured by `OwnerColor`, fanned out when stacked. Fog-honest (own always; a SCOUTED enemy too; an un-detected enemy hidden). Own try/catch.
- **C2** the C4.1 range-overlay pass broadened: it now ALSO tints every SCOUTED enemy unit's weapon (orange) / radar (teal) reach — distinct from own (red/green) — drawn without needing a selection, bounded to the visible window's hexes; and the enemy TOKENS (section 3) are gated to the detected set (an un-scouted enemy garrison never draws). New engine accessor `GroundThreat.DetectedEnemyUnits(body, viewer)` (owned/scouted/**in-contact**) is the fog seam.
- **C3** `DrawGlobalHexWindow` draws the SELECTED formation's queued PLAN as path lines: the leader's `GlobalQ/R` → its in-flight `GlobalPath` → each queued `MoveToHex` waypoint (global cylinder coords), via the new instance helper `GlobalHexToScreen` (odd-r offset + seam-wrap) + `drawList.AddLine`. Per-unit fire-control target lines deferred (no engine state). All thin/defensive; CI compiles the client but can't run it — the look is the developer's local build.

**Entity Assembler readout rows (C4.1, same slice) — see the `ShipDesignWindow` row.** `DisplayGroundStats` gained three
rows off `GroundUnitAssembly.Compute`: **Training** (`TrainingMultiplier`, the cadre veterancy — previously computed but
...
2. **`PlanetaryWindow.old.cs` is a dead legacy file — kept out of the build by being wholly commented out, NOT by a `.csproj` exclusion.** It lives at `Interface/Windows/PlanetaryWindow.old.cs`, and its entire contents sit inside a single `/* … */` block comment (verified 2026-07-13), so the compiler skips it. There is **no `<Compile Remove …>` line** in `Pulsar4X.Client.csproj` for it. Do not reference it — it may hold stale API usage.

3. **There is no `GalaxyMap.cs` in the client** (verified 2026-07-13 — no such file exists anywhere in `Pulsar4X.Client/`, and no `<Compile Remove …>` for it in the `.csproj`). The live galaxy-level window is `GalaxyWindow.cs`; don't confuse an imagined `GalaxyMap.cs` with it.
...
13. **⚠ `[FleetCombat]` IS A PLAYER-INPUT ECHO, NOT A BATTLE READOUT (verified 2026-07-27).** The tag has
    **exactly three emitters, all client button handlers** (set doctrine / set EMCON / set engagement posture).
    Seeing `[FleetCombat]` proves *a button worked*; it can **never** confirm a battle formed, resolved, or
    was lost. **The battle channel is `[Combat]`** (`CombatEngagement.CombatLog`, gated on `NarrateToLog`
    which this client sets at startup) — and on the GROUND side there is no channel at all yet
    (`docs/ground/PLANETARY-FUNCTIONAL-PLAN-2026-07-27.md` slice S1). Two further gauge traps found in the
    same pass, both from real play logs (`docs/DOCS-AUDIT-2026-07-27.md` §9): the clean-exit
    `faults=` number counts **render/input faults only** (`PulsarMainWindow.cs:485`) and read `faults=0` on a
    session with 7 `[FATAL]`s and a dead clock; and the `[ENGINE]` heartbeat counters increment **before**
    their processors' early-returns, so they climb on an empty galaxy and prove only that a hotloop is
    scheduled.

14. **Session recorder — the "flight recorder" for live play (`SessionLog`, built 2026-06-26).** `Pulsar4X.Client.SessionLog` (`SessionLog.cs`) writes a readable, greppable play-by-play of the player's actions + periodic state to the captured log, so a bug report **is** the log instead of "reproduce it and send a log." Every line is **flushed immediately** (`Console.Out.Flush()`), so a freeze or hard crash still leaves the full trail up to that instant. Categories: `[ACTION] [VIEW] [TIME] [CAMERA] [SELECT] [DRAG] [STATE] [DETECT] [EMCON] [ENGINE]` (plus engine-side `[Combat]`/`[FleetCombat]`/`[DevTools]`). Toggle the whole thing with `SessionLog.Enabled`.
...
> **Extended to CONTACT BLIPS (2026-07-18, Operation Earthfall C1.1 — the 2050-04-22 freeze).** The gotcha-#15 cull was applied only to `OrbitEllipseIcon`; the fog-of-war **contact blips** (`SensorContactIcon`, drawn from the engine's last-known `SensorPositionDB`) were the ONE unculled path left, and they're exactly the one that goes degenerate — a lost track anchor gives a NaN/infinite `AbsolutePosition`, and a `fogLag` drift (the log showed it jump to ~25,000 km right before the real session froze) parks the blip astronomically off-screen. The base `Icon.Draw` **clamps** such a coordinate to `int.Min/MaxValue` rather than **throwing**, so the per-item `SafeDraw` try/catch never fires — instead `SDL.RenderLine` chokes rasterising the astronomically off-screen line, frame time climbs, and the client FREEZES with no exception (findings/A1-freeze.md H1). **Fix:** `SensorContactIcon.OnFrameUpdate` now computes the blip's world + on-screen position and, if either is non-finite or the screen coordinate is beyond `_maxBlipScreenCoordPx` (1,000,000 px — matches `SimpleCircle.MaxSafeCoordPx`), sets `_offScreenSkip` and returns early (skips transform AND draw; `Draw` checks the flag first). Recomputed every frame, so a contact that becomes valid/on-screen again redraws next frame — a per-frame "worth drawing right now?" decision, same as the orbit cull. A normal on-screen finite blip takes the unchanged base path. **Companion breadcrumb: `SystemMapRendering.Draw` now stamps a per-LIST sub-stage** (`SessionLog.CurrentStage = "GalacticMap.Draw/contacts"` | `/orbits` | `/ships` | `/labels` …) before each icon list, so if a native hang ever survives this it names the exact list instead of the coarse `'GalacticMap.Draw'`. And the hang watchdog threshold dropped **5000 → 3500 ms** (`SessionLog.HangThresholdMs`) — the worst legit frame ever seen was ~2100 ms (startup), so a freeze is now named ~1.5 s sooner without false positives. Runtime-unverified (CI can't run the client); the local check lives in `docs/CLIENT-TEST-CHECKLIST.md` under "OPERATION EARTHFALL — C1.1".
