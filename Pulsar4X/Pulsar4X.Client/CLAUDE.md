...
`docs/GROUND-SURFACE-MAP-DESIGN.md`); `PlanetViewWindow` is the player's window onto it. It's the developer's "flat
...
`docs/GROUND-SURFACE-MAP-DESIGN.md` → "LOCKED PRINCIPLE." **Next: the "40k" ground-combat depth pass + a per-unit combat
...
the developer's local build (CI compiles the client, can't run it).** Design: `docs/GROUND-SURFACE-MAP-DESIGN.md` (H4).
