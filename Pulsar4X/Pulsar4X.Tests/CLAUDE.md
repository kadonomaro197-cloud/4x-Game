...
| &nbsp;&nbsp;`WeaponClassifierTests` | **weapon-unification step 1** — `WeaponClassifier.Classify(delivery, vel, trk, sat)` derives the triangle corner from the axes+specs (each corner + the disambiguating edges), AND THE INVARIANT on real built ships: every base-mod weapon's `WeaponProfile.ComputedClass` equals its authored `Class` (laser/railgun/flak/ion-disruptor) — the green light to later make `Class` a pure read-out |
