...
| &nbsp;&nbsp;`WeaponClassifierTests` | **weapon-unification step 1** — `WeaponClassifier.Classify(delivery, vel, trk, sat)` derives the triangle corner from the axes+specs (each corner + the disambiguating edges), THE INVARIANT on real built ships (every base-mod weapon's `WeaponProfile.ComputedClass` equals its authored `Class` — laser/railgun/flak/ion-disruptor), AND that the invariant survives `BuildFireMix` **aggregation** (the mix now buckets by class+nature+delivery, so a real Vanguard's plasma keeps Energy/Bolt, not the Slug default) — the green light to later make `Class` a pure read-out |
