...
| `GroundAmmoGateTests` | **weapon-unification P2c-a** — the AMMO GATE (the ammo-axis twin of P2b): `GroundUnitAssembly` refuses an Ammo/Both weapon (flak, railgun) with no `GroundMagazineAtb` mounted (hard gate; exposes `AmmoCapacity_kg`). Isolates it with FLAK (pure Ammo, draws no power) — flak needs a magazine, a laser (Energy) doesn't. No base-mod magazine exists yet, so the gate bites for every ammo weapon (correct until P2c-b). The engine gauge for the ammo half of `WeaponSupply` |
